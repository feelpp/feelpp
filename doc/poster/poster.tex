...
auto( E , 0.5*(gradv(u)+trans(gradv(u)))
         +0.5*trans(gradv(u))*gradv(u)); @\vspace{0.3cm}@
//Loi de comportement St-Venant-Kirchhoff : $S = \lambda tr\left(E\right) Id + 2 \mu E$
auto( S, lambda*trace(E)*Id + 2*mu*E );		@\vspace{0.3cm}@ 
//Force volumique
auto( eta, 1000*(Px()-0.4)*(Px()-0.45)
               *chi(Px()>=0.4 && Px()<=0.45) ); @\vspace{0.3cm}@
//$-\int_{\Omega} FS:\nabla v + \int_{\Omega} \rho \ \eta \vec{n_{y}} \cdot v$ + C.L.
form1( Xh, R ) =
 integrate( elements(mesh), im,
     - trace((F*S)*trans(grad(v))) 
     + rho*trans(eta*oneY())*id(v) ) +
 on( markedfaces(mesh,"clamped"),
     u, F, constant(0)*one() ); @\vspace{0.3cm}@
