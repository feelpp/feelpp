...
[source,cpp]
----
{% include "/QuickReference/Models/Examples/solid_model.cpp" %}
----
...
[source,cfg]
----
{% include "/QuickReference/Models/Examples/csm3.cfg" %}
----
...
After that, we define the link:#_material[material properties]. In our case, we define the solid we will study, named beam here, by $$E$$, $$\nu$$ and $$\rho$$, respectively its Young's modulus ( $$kg/ms^2$$ ), its Poisson's ratio ( dimensionless ) and its density (in $$kg/m^3$$)
...
The link:#_boundary_conditions[boundary conditions] are the next aspect we define. Here, we impose on the displacement several conditions :
...
The link:#_post_process[post process] aspect is the last one to define. We want to export displacement values as well as measure displacement and velocity on point $$A$$ along with the maximum of this values on all the free wall.
...
[source,cfg]
----
{% include "/QuickReference/Models/Examples/csm3.json" %}
----
