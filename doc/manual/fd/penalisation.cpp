...
void Penalisation<Dim>::initStokes()
{
    ...
    LOG(INFO)<<"check mesh\n";
    LOG(INFO)<<"H1 = "<<integrate( _range=markedfaces( mesh, "Inflow" ), _expr=vf::cst( 1. ) ).evaluate()( 0,0 )<<"\n";
    LOG(INFO)<<"Wall = "<<integrate( _range=markedfaces( mesh, "Wall" ), _expr=vf::cst( 1. ) ).evaluate()( 0,0 )<<"\n";
    LOG(INFO)<<"H2 = "<<integrate( _range=markedfaces( mesh, "Outflowtop" ), _expr=vf::cst( 1. ) ).evaluate()( 0,0 )<<"\n";
    ...
    form1( _test=Xh, _vector=F ) =
        integrate( _range=elements( mesh ), _expr=trans( vf::one()-vf::one() ) * id( v ) ) ;
    ...
    C = M_backend->newMatrix( _test=Xh, _trial=Xh );

    //    def(u):def(v) = trace ( def(u)* trans( def(v) ) )
    form2( _test=Xh, _trial=Xh, _matrix=C )= integrate( _range=elements( mesh ),
                                                        _expr=nu * vf::trace( deft * trans( def ) ) );
    form2( _test=Xh, _trial=Xh, _matrix=C )+= integrate( _range=elements( mesh ),
                                                         _expr=- div( v ) * idt( p ) +id( q ) * divt( u ) );
    form2( _test=Xh, _trial=Xh, _matrix=C )+= integrate( _range=elements( mesh ),
                                                         _expr=- epsilonpress * idt( p ) * id( q ) );
    form2( _test=Xh, _trial=Xh, _matrix=C )+= integrate( _range=elements( mesh ),_expr=idt( lambda )*id( q ) );
    form2( _test=Xh, _trial=Xh, _matrix=C )+= integrate( _range=elements( mesh ),_expr=id( lambda )*idt( q ) );
    C->close();
...
void Penalisation<Dim>::stokes()
{
    ...
    local_chrono.start();
    D=M_backend->newMatrix( _test=Xh, _trial=Xh );
    form2( _test=Xh, _trial=Xh, _matrix=D )= integrate( _range=marked3elements( mesh,1 ),
                                                        _expr=idv( carac ) * nu * trace( def*trans( deft ) ) / epsilon );
    ...
    double Q0, Q1, Q2;
    Q0 = integrate( _range=markedfaces( mesh,mesh->markerName( "Inflow" ) ),_expr=trans( idv( u ) )*vf::N() ).evaluate()( 0,0 );
    Q1 = integrate( _range=markedfaces( mesh,mesh->markerName( "Outflowtop" ) ),_expr=trans( idv( u ) )*vf::N() ).evaluate()( 0,0 );
    Q2 = integrate( _range=markedfaces( mesh,mesh->markerName( "Outflowbottom" ) ),_expr=trans( idv( u ) )*vf::N() ).evaluate()( 0,0 );

    LOG(INFO)<<"Q0 = "<<Q0<<"\n";
    LOG(INFO)<<"Q1 = "<<Q1<<"\n";
    LOG(INFO)<<"Q2 = "<<Q2<<"\n";
    LOG(INFO)<<"div(u)="<<integrate( _range=elements( mesh ),
                                     _expr=divv( u ) ).evaluate()( 0,0 )<<"\n";
...
void Penalisation<Dim>::addCL()
{
    ...
    if ( Dim==2 )
    {
        ...
        form2( _test=Xh, _trial=Xh, _matrix=D )+=
            on( _range=markedfaces( mesh, "Wall" ),
                _element=u, _rhs=F, _expr=cst( 0. ) * vf::N()  );
        form2( _test=Xh, _trial=Xh, _matrix=D )+=
            on( _range=markedfaces( mesh, "Inflow" ),
                _element=u, _rhs=F, _expr=inflow * vf::N() );
        form2( _test=Xh, _trial=Xh, _matrix=D )+=
            on( _range=markedfaces( mesh, "Outflowtop" ),
                _element=u, _rhs=F, _expr=- outflowtop * vf::N() );
        form2( _test=Xh, _trial=Xh, _matrix=D )+=
            on( _range=markedfaces( mesh, "Outflowbottom" ),
                _element=u, _rhs=F,  _expr=- outflowbottom * vf::N() );
    }
...
void Penalisation<Dim>::updateChi()
{
    ...
    double aire = integrate( _range=marked3elements( mesh, 1 ), _expr=idv( carac ) ).evaluate()( 0,0 );
...
void Penalisation<Dim>::updatePosition()
{
    ...
    double aire = integrate( _range=marked3elements( mesh,1 ),
                             _expr=idv( carac ) ).evaluate()( 0,0 );
    LOG(INFO)<<"aire = "<<aire<<"\n";

    auto Velo = integrate( _range=marked3elements( mesh,1 ),
                           _expr=idv( carac )*idv( u ) ).evaluate() / aire ;
