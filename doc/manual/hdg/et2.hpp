...
ElectroThermal<Dim, OrderP>::run()
{
    ...
    hdg_graph(3,3) = stencil( _test=Ch,_trial=Ch, _diag_is_nonzero=false, _close=false)->graph();
                                               ...
                                               )
{
    ...
    auto rhs5 = form1( _test=Ch, _vector=F,
                       _rowstart=3);
    double meas_bottom = integrate( _range=markedfaces(mesh,"bottom"), _expr=cst(1.0)).evaluate()(0,0);
    rhs5 += integrate(_range=markedfaces(mesh,"bottom"),
                      _expr=doption("I_target")*id(nu)/meas_bottom);
    ...
    a13 += integrate(_range=markedfaces(mesh,{"R","top"}),
    ...
    a23 += integrate(_range=markedfaces(mesh,{"R","top"}),
