...
ElectroThermal<Dim, OrderP>::run()
{
    ...
    hdg_graph(3,3) = stencil( _test=Ch,_trial=Ch, _diag_is_nonzero=false, _close=false)->graph();//stencil( _test=Ch,_trial=Ch, _diag_is_nonzero=false, _close=false,_pattern=(size_type)Pattern::ZERO)->graph();
          ...
          bdfT->next(T) )
    {
        ...
        do
        {
            assemble_A_and_F( A, F, Vh, Wh, Mh, Xh, up, pp, Tp );
            tic();
            backend(_rebuild=true)->solve( _matrix=A, _rhs=F, _solution=U );
            hdg_sol.localize(U);
            toc("solve",false);
                                               ...
                                               )
{
    ...
    auto a51 = form2(_trial=Vh, _test=Xh,_matrix=A,
                     _rowstart=Vh->nLocalDofWithGhost()+Wh->nLocalDofWithGhost()+Mh->nLocalDofWithGhost()+Ch->nLocalDofWithGhost(),
                     _colstart=0);
    a51 += integrate( _range=elements(mesh), _expr=trans(idt(u))*id(v) );
