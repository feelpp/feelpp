...
#ifndef LC_MODEL2_HPP
#define	LC_MODEL2_HPP
...
public:
    ...
    // Vh
    using Vh_t = typename super_type::Vh_t;
    using Vh_ptr_t = typename super_type::Vh_ptr_t;
    using Vh_element_t = typename super_type::Vh_element_t;
    using Vh_element_ptr_t = typename super_type::Vh_element_ptr_t;
    // Wh
    using Wh_t = typename super_type::Wh_t;
    using Wh_ptr_t = typename super_type::Wh_ptr_t;
    using Wh_element_t = typename super_type::Wh_element_t;
    using Wh_element_ptr_t = typename super_type::Wh_element_ptr_t;
    // Vh
    using Mh_t = typename super_type::Mh_t;
    using Mh_ptr_t = typename super_type::Mh_ptr_t;
    using Mh_element_t = typename super_type::Mh_element_t;
    using Mh_element_ptr_t = typename super_type::Mh_element_ptr_t;
    // Ch
...
private:
	...
	std::vector<std::string> M_odeMarkersList;
	int M_odeInterfaces;
...
template<int Dim, int Order>
void
LaminaCribrosa<Dim, Order>::assemble()
{
    super_type::assemble();
	this->assembleLCmatrix();
...
LaminaCribrosa<Dim, Order>::initModel(){
	...
	// To manage
	M_odeMarkersList.push_back(this->M_integralMarkersList[0]);
	M_odeInterfaces = M_odeMarkersList.size();
...
LaminaCribrosa<Dim, Order>::initSpaces(){
	...
	// I added one integral condition more to allocate one row and one column more for the ODE system coupling.
    this->M_integralCondition += M_odeInterfaces;
	
	// cout << "Number of integral condition before initSpaces: " << this->integralCondition() << std::endl;
	super_type::initSpaces();

	// I return to the right number of integral boundary condition
	this->M_integralCondition += -M_odeInterfaces;

    M_Y = this->constantSpace()->element( "yy" );
...
template<int Dim, int Order>
void
LaminaCribrosa<Dim, Order>::assembleLCmatrix( ) 
{

	cout << "Assembling LC matrix . . ." << std::endl;

	// Assembling the outer matrix (ODE system coupling
	auto bbf = blockform2( *(this->M_ps), this->M_A_cst );
	...
	auto N_IBC = this->integralCondition();


	for ( int i=0; i < M_odeInterfaces; i++)
	{
		auto marker = M_odeMarkersList[i];
		// cout << "MARKER: " << marker << std::endl;
		auto meas = integrate(_range=markedfaces(this->mesh(),marker),_expr=cst(1.0)).evaluate()(0,0);
		// - <j.n, mu3>_Gamma_I			// a51
		bbf( 3_c, 0_c, N_IBC+i, 0 ) += integrate( _range=markedfaces(this->mesh(),marker), _expr= -trans(idt(u))*N()*id(nu) );
		// - <tau p, mu3>_Gamma_I 		// a52
		bbf( 3_c, 1_c, N_IBC+i, 0 ) += integrate( _range=markedfaces(this->mesh(),marker), _expr= -tau_constant*( pow(idv(H),this->tau_order())*idt(p) )*id(nu) );
		// + <tau u_I, mu3>_Gamma_I 	// a54
		bbf( 3_c, 3_c, N_IBC+i, i ) += integrate( _range=markedfaces(this->mesh(),marker), _expr= tau_constant * (pow(idv(H),this->tau_order())*idt(uI)*id(nu)) );
		...
		for( auto const& pairMat : this->M_modelProperties->materials() )
		{
			...
			// -1/(R |Gamma_I|) <u_I, mu2>_Gamma_I 		// a44
			bbf( 3_c, 3_c, i, i ) += integrate( _range=markedfaces(this->mesh(),marker), _expr = - idt(uI)*id(nu)/RR/meas ) ;
			// +1/(R |Gamma_I|) <Y,mu2>_Gamma_I			// a45
			bbf( 3_c, 3_c, i, N_IBC+i) += integrate( _range=markedfaces(this->mesh(),marker), _expr = idt(uI)*id(nu)/RR/meas ) ;
			// < C/|Gamma_I| Y/dt, mu3>_Gamma_I			// a55
			bbf( 3_c, 3_c, N_IBC+i, N_IBC+i) += integrate(_range=markedfaces(this->mesh(),marker), 
										_expr= CC*this->timeStepBDF_statevar()->polyDerivCoefficient(0) * idt(uI)*id(nu)/meas );
		}
	}
}
...
template<int Dim, int Order>
void
LaminaCribrosa<Dim, Order>::assembleLCrhs()
{

	cout << "Assembling LC rhs . . ." << std::endl;
	...
	for ( int i = 0; i < M_odeInterfaces; i++ )
	{
		for( auto const& pairMat : this->M_modelProperties->materials() )
		{
			...
			auto marker = M_odeMarkersList[i];
			auto meas = integrate(_range=markedfaces(this->mesh(),marker),_expr=cst(1.0)).evaluate()(0,0);
	 		// < C/|Gamma_I| Yold/dt, mu3> 		// rhs5
	 		blf( 3_c, i+N_IBC ) += integrate( _range = markedfaces(this->mesh(),marker),
							   _expr = CC*idv(this->timeStepBDF_statevar()->polyDeriv()) * id(nu)/meas);
		}
	}
...
LaminaCribrosa<Dim, Order>::solve()
{
	...
	U.localize(this->M_U);
	// for (int i = 0; i < M_odeInterfaces; i++)
	M_Y =  U( 3_c, this->M_integralCondition) ;
}
...
LaminaCribrosa<Dim, Order>::second_step(){
	...
	for( auto marker : this->M_integralMarkersList)
    {
		...
		cout << "Integral value of the flow on '" << marker << "' : \t " << j_integral << std::endl;
    }
	
	// Feel::cout << "Pi1: \t" << M_statevar_solution[0] << " (before) " << std::endl;
	// Feel::cout << "Pi2: \t" << M_statevar_solution[1] << " (before) " << std::endl;
	// Feel::cout << "Pi3: \t" << M_statevar_solution[2] << " (before) " << std::endl;
	// solve the problem
	...
	Feel::cout << "Pi1: \t" << M_statevar_solution[0] /*<< " (after) "*/ << std::endl;
	Feel::cout << "Pi2: \t" << M_statevar_solution[1] /*<< " (after) "*/ << std::endl;
	Feel::cout << "Pi3: \t" << M_statevar_solution[2] /*<< " (after) "*/ << std::endl;
