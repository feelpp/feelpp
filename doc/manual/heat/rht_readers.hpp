...
namespace Feel
{
    ...
    // Read sky temperature information from all meteo stations, as specified in the json
        template<int Dim, int Order>
        void RHT<Dim,Order>::read_Tsky()
        {        
            // Json will have a sun radiation section in the boundary conditions
            for ( auto& [meteo_station_name, value] : specs["/BoundaryConditions/heat/solar_radiation"_json_pointer].items() )
            {
                auto T_sky_status = value["Tsky"]["status"];
                // Tsky is read from the CSV file that has been output by the weather tool
                // One value of Tsky is available per meteo station (it will be the same for all the surfaces)
                if(T_sky_status=="load")
                {
                    readTskyFromCSV(meteo_station_name,value["Tsky"]["directory"]);                
                }
                else
                {
                    throw std::logic_error( "Sky temperature not loaded" );
                }
            }
            
        }
    // Read sky temperature information for one meteo station from a CSV file
        template<int Dim, int Order>
        void RHT<Dim,Order>::readTskyFromCSV(std::string const& meteo_station_name, std::string const& directory)
        {
            std::fstream f;
            auto filename = directory + "/" + meteo_station_name + ".Tsky.csv";
            f.open(Environment::expand(filename));
            if(!f.is_open())
            {
                std::cout << "file Tsky not opened" << std::endl;
                std::cout << fmt::format("File {} not found",Environment::expand(filename)) << std::endl;
            }
            std::string line, entry, temp;
            std::vector<double> TSky;

            while (std::getline(f, line,'\n')) 
            {     
                std::stringstream s(line);        
                while (std::getline(s, entry, ',')) {    
                        TSky.push_back( stod(entry) );
                    }                
            }
            M_station_Tsky.insert(std::make_pair(meteo_station_name,TSky));
        }

    template<int Dim, int Order>
        double RHT<Dim,Order>::accessTsky(std::string const& meteo_station_name,double const& time, double const& dt)
        {
            int index = time/dt;
            return M_station_Tsky[meteo_station_name][index];
        }

    // Read external temperature information from all meteo stations, as specified in the json
        template<int Dim, int Order>
        void RHT<Dim,Order>::read_Text()
        {        
            // Json will have a sun radiation section in the boundary conditions
            for ( auto& [meteo_station_name, value] : specs["/BoundaryConditions/heat/meteo"_json_pointer].items() )
            {
                auto T_ext_status = value["Text"]["status"];
                // Text is read from the CSV file that has been output by the weather tool
                // One value of Text is available per meteo station (it will be the same for all the surfaces)
                if(T_ext_status=="load")
                {
                    readTextFromCSV(meteo_station_name,value["Text"]["directory"]);                
                }
                else
                {
                    throw std::logic_error( "External temperature not loaded" );
                }
            }
            
        }
        
    // Read external temperature information for one meteo station from a CSV file
        template<int Dim, int Order>
        void RHT<Dim,Order>::readTextFromCSV(std::string const& meteo_station_name, std::string const& directory)
        {
            std::fstream f;
            auto filename = directory + "/" + meteo_station_name + ".Text.csv";
            f.open(Environment::expand(filename));
            if(!f.is_open())
            {
                std::cout << "file Text not opened" << std::endl;
                std::cout << fmt::format("File {} not found",Environment::expand(filename)) << std::endl;
            }
            std::string line, entry, temp;
            ...
            {     
                std::stringstream s(line);        
                while (std::getline(s, entry, ',')) {    
                        Text.push_back( stod(entry) );
                    }                
            }
            M_station_Text.insert(std::make_pair(meteo_station_name,Text));
        }

    template<int Dim, int Order>
        double RHT<Dim,Order>::accessText(std::string const& meteo_station_name,double const& time, double const& dt)
        {
            int index = time/dt;
            return M_station_Text[meteo_station_name][index];
        }


    // Read infrared solar radiation for all buildings and all surfaces, as specified in the json from a CSV file, specified in the json
        template<int Dim, int Order>
        void RHT<Dim,Order>::read_solarRadiation()
        {
            // Json will have a sun radiation section in the boundary conditions
            for ( auto& [meteo_station_name, value] : specs["/BoundaryConditions/heat/solar_radiation"_json_pointer].items() )
            {
                ...
                for(auto const& [building_name,building_structure] : specs[json::json_pointer(buildings_specs_string)].items() )
                {
                    ...
                    {
                        // solar_radiation is read from the CSV file that has been output by the weather tool
                        // The meteo tool outputs one CSV per surface, with the total value of incident solar radiation
                        readSolarRadiationFromCSV(meteo_station_name,building_name,surface_name,building_structure["OpaqueSurfaces"]["directory"]);  
                    }
                    for(std::string surface_name : building_structure["TransparentSurfaces"]["surfaces"])
                    {
                        // solar_radiation is read from the CSV file that has been output by the weather tool
                        // The meteo tool outputs one CSV per surface, with the total value of incident solar radiation
                        readSolarRadiationFromCSV(meteo_station_name,building_name,surface_name,building_structure["TransparentSurfaces"]["directory"]);  
                    }
                }
    ...
    // Read sky temperature information for one meteo station from a CSV file
        template<int Dim, int Order>
        void RHT<Dim,Order>::readSolarRadiationFromCSV(std::string const& meteo_station_name, std::string const& building_name, std::string const& surface_name, std::string const& directory)
        {
            std::fstream f;
            std::string filename = directory + "/" + surface_name + ".solar_radiation.csv";
            f.open(Environment::expand( filename));
            if(!f.is_open())
            {
                std::cout << fmt::format("File solar radiation of surface {} in building {} not opened",surface_name,building_name);
            }
            std::string line, entry, temp;
            ...
            {     
                std::stringstream s(line);        
                while (std::getline(s, entry, ',')) {    
                        solar_radiation.push_back( stod(entry) );
                    }                
            }            
            M_solarRadiation_structure.insert(std::make_pair( meteo_station_name + building_name + surface_name ,solar_radiation));
        }

    template<int Dim, int Order>
        double RHT<Dim,Order>::accessSolarRadiation(std::string const& meteo_station_name, std::string const& building_name, std::string const& surface_name, double const& time, double const& dt)
        {
            int index = time/dt;
            
            return M_solarRadiation_structure[ meteo_station_name + building_name + surface_name ][index];
        }
