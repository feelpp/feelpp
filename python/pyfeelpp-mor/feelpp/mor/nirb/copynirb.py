...
class nirbOffline(ToolboxModel):
    ...
    def checkConvergence(self,Ns=10, Xi_test=None, regulParam=1.e-10):
        ...
        for j in range(Ntest):
            ...
            for k in range(self.N):
                tabcoef[j,k] = self.l2ScalarProductMatrix.energy(uHint,self.reducedBasis[k])

        
        Unh = self.Xh.element()
        UnhRect = self.Xh.element()

        errorS = np.zeros((2,Ntest))
        Error = {'N':[], 'w/o rect':[], 'w/ rect':[]}

        nb = 0
        for i in tqdm(range(self.N), desc=f"[NIRB] Compute convergence error:", ascii=False, ncols=100):
            nb +=1
            ...
            for j in range(Ntest):
                ...
                for k in range(nb): # get reduced sol in i basis function space 
                    Unh = Unh + float(tabcoef[j,k])*self.reducedBasis[k]
                    UnhRect = UnhRect + float(tabRect[k])*self.reducedBasis[k]

                Unh = Unh - soltestFine[j]
                UnhRect = UnhRect - soltestFine[j]
                errorS[0,j] = np.sqrt(abs(self.l2ScalarProductMatrix.energy(Unh,Unh)))
                errorS[1,j] = np.sqrt(abs(self.l2ScalarProductMatrix.energy(UnhRect,UnhRect)))
            Error["w/o rect"].append(errorS[0,:].mean())
            Error["w/ rect"].append(errorS[1,:].mean())
            Error["N"].append(i)
    ...
    def orthonormalizeL2(self, nb=0):
        ...
        while not (self.checkL2Orthonormalized() ) and nb < 10:
            nb +=1
            for n in range(1, self.N):
                s = self.Xh.element()
                s.setZero()
    ...
    def checkL2Orthonormalized(self, tol=1e-5):
