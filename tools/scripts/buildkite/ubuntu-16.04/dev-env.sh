...
sudo apt-get -y --force-yes install software-properties-common python-software-properties
...
sudo curl http://ubuntu.openturns.org/openturns.org-repo.key | apt-key add -
...
sudo apt-get -y --force-yes install \
        xauth cmake flex g++-4.9 clang++-3.8 gfortran git ipython openmpi-bin pkg-config \
            wget bison sudo \
            libbz2-dev \
