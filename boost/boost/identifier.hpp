...
public:
    ...
    typedef identifier<T,D> self;
    self& operator=(const self& p ) { m_value = p.m_value; return *this;  }
    self& operator++() { ++m_value; return *this; }
    //self operator++(int) { ++m_value; return *this; }
...
#if 0
template <typename Ostream, typename Id>
typename enable_if< is_base_of< identifier< typename Id::value_type, Id >, Id >, Ostream & >::type
operator<<( Ostream & os, const Id & id )
{
    return os << id.value();
}

template <typename Istream, typename Id>
typename enable_if< is_base_of< identifier< typename Id::value_type, Id >, Id >,Istream & >::type
operator>>( Istream & is, Id & id )
{
