...
if [ $DIST = "jammy" -o  $DIST = "focal" -o $DIST = "bookworm" ]; then
    ...
    wget -O - http://apt.feelpp.org/apt.gpg 2>/dev/null | gpg --dearmor - | sudo tee /usr/share/keyrings/feelpp-archive-keyring.gpg >/dev/null
    echo 'deb [signed-by=/usr/share/keyrings/feelpp-archive-keyring.gpg] http://apt.feelpp.org/$FLAVOR/$DIST $DIST $CHANNEL' | sudo tee /etc/apt/sources.list.d/feelpp.list >/dev/null
fi
