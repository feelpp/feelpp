...
# Locate PETSc Python bindings (works for both system and wheel installs)
_petsc_dir = os.getenv('PETSC_DIR', '' or '/usr/lib/petsc')
_petsc_pydir = os.path.join(_petsc_dir, 'lib/python3/dist-packages')
if os.path.isdir(_petsc_pydir) and _petsc_pydir not in sys.path:
    sys.path.append(_petsc_pydir)
