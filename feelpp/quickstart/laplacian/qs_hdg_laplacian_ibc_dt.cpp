...
makeOptions()
{
    ...
    hdgoptions.add_options()
        ...
        ( "time-step", po::value<double>()->default_value( 1. ), "time step" )
...
int hdg_laplacian()
{
    ...
    // for (auto time : myBdg->priorTimes() )
    // {
    ...
    double t = 0;
    double tmax = doption( "time-final" );
    double dt = doption( "time-step" );
    int status_time = 0;
    ...
    for (t=dt; t < tmax+dt; t += dt)
    {
        ...
        toc("a(0,3)",Environment::logVerbosityLevel()>0);
        ...
        toc("a(1,3)", Environment::logVerbosityLevel()>0);
        ...
        for( auto const& [ibc_type,ibc_data]  : ibcs )
        {
            ...
            toc("a(3,3)",Environment::logVerbosityLevel()>0);
        }
        toc("matrices",true);
        ...
        toc("postprocessing.solve",Environment::logVerbosityLevel()>0);
