...
using shell_voigt = Eigen::Matrix<double, 6, 1>;
...
struct BenchmarkConfig
{
    ...
    {
        shell_voigt epsilon = shell_voigt::Zero();
        shell_voigt sigma = shell_voigt::Zero();
    };
    ...
    struct FieldReference
    {
        std::string name;
        std::string description;
        shell_vec point = shell_vec::Zero();
        bool hasAbsoluteTolerance = false;
        double absoluteTolerance = 0.0;
        bool hasEpsilonAbsoluteTolerances = false;
        shell_voigt epsilonAbsoluteTolerances = shell_voigt::Zero();
        bool hasSigmaAbsoluteTolerances = false;
        shell_voigt sigmaAbsoluteTolerances = shell_voigt::Zero();
        bool hasRelativeTolerance = false;
        double relativeTolerance = 0.0;
        std::map<std::string, FieldReferenceTarget> targets;
    };
...
bool reportFieldReferenceComparison( BenchmarkConfig::FieldReference const& reference,
