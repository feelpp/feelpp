...
remesh_options( std::string const& prefix )
{
    ...
    mmgoptions.add_options()
        ...
        ( prefixvm( prefix, "remesh.verbose").c_str(),Feel::po::value<int>()->default_value(6), "[-1..10], Tune level of verbosity")
        ( prefixvm( prefix, "remesh.debug").c_str(),Feel::po::value<int>()->default_value(0), "Turn on/off debug mode")

        ( prefixvm( prefix, "remesh.hmin").c_str(),Feel::po::value<double>()->default_value(1), "[val], Minimal mesh size")
        ( prefixvm( prefix, "remesh.hmax").c_str(),Feel::po::value<double>()->default_value(1), "[val], Maximal mesh size")
        ( prefixvm( prefix, "remesh.hsiz").c_str(),Feel::po::value<double>()->default_value(1), "[val], Constant mesh size")

        ( prefixvm( prefix, "remesh.nosizreq").c_str(),Feel::po::value<int>()->default_value(0), "[0/1], Allow/avoid overwritten of sizes at required points (advanced usage)")
        ( prefixvm( prefix, "remesh.hgradreq").c_str(),Feel::po::value<double>()->default_value(1), "[val], Control gradation on required entites (advanced usage)")
        ( prefixvm( prefix, "remesh.hausd").c_str(),Feel::po::value<double>()->default_value(1), "[val], Control global Hausdorff distance (on all the boundary surfaces of the mesh)")
        ;
    // clang-format on
    return mmgoptions;
}
