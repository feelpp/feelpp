...
/**
   ...
   @date 2005-09-03
   @brief Faces container class (C++20/23 modernized)
 */
 ...
 * @brief Faces container class (C++20/23 modernized)
 *
 * Uses std::conditional_t instead of boost::mpl::if_ for face type selection.
 *
 * @author Christophe Prud'homme <christophe.prudhomme@feelpp.org>
...
private:
    // Helper constants for face type selection
    static constexpr uint16_type nDim = EntityType::nDim;
    static constexpr uint16_type nRealDim = EntityType::nRealDim;
    static constexpr bool is_codim_one = (nDim == nRealDim - 1);

    // Face type for SubFaceOf (boundary faces connecting to one element)
    using subface_of_type = std::conditional_t<
        nDim == 0,
        GeoElement0D<nRealDim, SubFaceOf<ElementType>, value_type, index_type>,
        std::conditional_t<
            nDim == 1,
            GeoElement1D<nRealDim, EntityType, SubFaceOf<ElementType>, value_type, index_type>,
            GeoElement2D<nRealDim, EntityType, SubFaceOf<ElementType>, value_type>
        >
    >;

    // Face type for SubFaceOfMany (internal faces connecting to multiple elements)
    using subface_of_many_type = std::conditional_t<
        nDim == 0,
        GeoElement0D<nRealDim, SubFaceOfMany<ElementType>, value_type, index_type>,
        std::conditional_t<
            nDim == 1,
            GeoElement1D<nRealDim, EntityType, SubFaceOfMany<ElementType>, value_type, index_type>,
            GeoElement2D<nRealDim, EntityType, SubFaceOfMany<ElementType>, value_type, index_type>
        >
    >;

public:
    //! @brief Face type - determined by dimension and co-dimension using C++20 type selection
    using face_type = std::conditional_t<is_codim_one, subface_of_type, subface_of_many_type>;

    using faces_type = std::unordered_map<index_type, face_type, std::hash<index_type>>;

    using face_iterator = typename faces_type::iterator;
    using face_const_iterator = typename faces_type::const_iterator;

    using faces_reference_wrapper_type = std::vector<boost::reference_wrapper<face_type const>>;
    using faces_reference_wrapper_ptrtype = std::shared_ptr<faces_reference_wrapper_type>;
    using face_reference_wrapper_iterator = typename faces_reference_wrapper_type::iterator;
    using face_reference_wrapper_const_iterator = typename faces_reference_wrapper_type::const_iterator;

    using ordered_faces_reference_wrapper_type = std::vector<boost::reference_wrapper<face_type>>;
    using ordered_face_reference_wrapper_iterator = typename ordered_faces_reference_wrapper_type::iterator;
    using ordered_face_reference_wrapper_const_iterator = typename ordered_faces_reference_wrapper_type::const_iterator;
     ...
     */
    faces_type const& faces() const noexcept
    {
        return M_faces;
    }

    [[nodiscard]] WorldComm const& worldCommFaces() const noexcept
    {
        return *M_worldComm;
    }
    ...
    [[nodiscard]] bool isBoundaryFace( face_type const& e ) const
    {
        return e.isOnBoundary();
    }

    [[nodiscard]] bool isBoundaryFace( size_type const& id ) const
    {
     ...
     */
    [[nodiscard]] bool hasFace( size_type i ) const
    {
        return M_faces.find( i ) != M_faces.end();
    }

    [[nodiscard]] face_type const& face( size_type i ) const
    {
    ...
    [[nodiscard]] face_const_iterator faceIterator( size_type i ) const
    {
    ...
    [[nodiscard]] face_const_iterator faceIterator( face_type const& face ) const
    {
    ...
    [[nodiscard]] face_const_iterator beginFace() const
    {
        return M_faces.begin();
    }
    ...
    [[nodiscard]] face_const_iterator endFace() const
    {
        return M_faces.end();
    }

    [[nodiscard]] ordered_faces_reference_wrapper_type const& orderedFaces() const noexcept
    {
    ...
    [[nodiscard]] ordered_face_reference_wrapper_const_iterator beginOrderedFace() const
    {
        return M_orderedFaces.begin();
    }
    ...
    [[nodiscard]] ordered_face_reference_wrapper_const_iterator endOrderedFace() const
    {
        return M_orderedFaces.end();
    }
     ...
     * with \c Id \p m
     */
    [[nodiscard]] std::tuple<face_reference_wrapper_const_iterator, face_reference_wrapper_const_iterator, faces_reference_wrapper_ptrtype>
    ...
    {
        std::pair<face_iterator, bool> ret = M_faces.emplace( std::make_pair( f.id(), f ) );
