...
public:
    ...
    // point set type associated with the functionals
    typedef PointSet<convex_type, value_type> pointset_type;
    typedef PointSetType<convex_type, (nDim==2)?2:3, value_type> equispaced_pointset_type;
    ...
    // CR DOFs: 1 per edge in 2D, 1 per face in 3D — hardcoded math def, not from reference convex
    static inline const uint16_type nbPtsPerEdge = (nDim == 2) ? 1 : 0;
    static inline const uint16_type nbPtsPerFace = (nDim == 3) ? 1 : 0;
        ...
        M_fset( primal )
    {
        ...
        std::cout << "\n o- ref nbPtsPerFace = " << ( int )reference_convex_type::nbPtsPerFace << "\n";
        std::cout << " o- ref nbPtsPerEdge = " << ( int )reference_convex_type::nbPtsPerEdge << "\n";
        std::cout << " o- ref numFaces     = " << ( int )reference_convex_type::numFaces << "\n";
        std::cout << " o- ref numEdges     = " << ( int )reference_convex_type::numEdges << "\n";
        std::cout << " o- nFacesInConvex   = " << ( int )nFacesInConvex << "\n";
        std::cout << " o- topoDim          = " << ( int )M_convex_ref.topologicalDimension() << "\n";
        std::cout << " o- d (loop dim)     = " << ( int )(M_convex_ref.topologicalDimension()-1) << "\n";
#endif
        equispaced_pointset_type epts;
        // in d-dimension, consider only the d-1 entity mid-points
        int d = M_convex_ref.topologicalDimension()-1;
                ...
                ++e )
        {
            if constexpr ( nDim == 3 )
            {
                // compute face centroid manually from face vertices
                auto const& face_vertices = M_convex_ref.faceVertices( e );
                points_type centroid( nDim, 1 );
                centroid.clear();
                for ( int v = 0; v < face_vertices.size2(); ++v )
                    ublas::column( centroid, 0 ) += ublas::column( face_vertices, v );
                ublas::column( centroid, 0 ) /= face_vertices.size2();
                M_points_face[e] = centroid;
            }
            else
            {
                M_points_face[e] = epts.pointsBySubEntity( nDim-1, e, 0 );
