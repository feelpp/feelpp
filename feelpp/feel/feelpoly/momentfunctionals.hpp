...
*/
/**
   \file momentfunctionals.hpp
   \author Christophe Prud'homme <christophe.prudhomme@feelpp.org>
   \date 2006-01-22
 */
#ifndef FEELPP_FEELPOLY_MOMENTFUNCTIONALS_HPP
#define FEELPP_FEELPOLY_MOMENTFUNCTIONALS_HPP 1
...
public:
        ...
        super( p )
    {
        ...
        const uint16_type quadratureOrder = static_cast<uint16_type>( 2*( p.order()+1 ) );
        IMGeneral<nDim, value_type, Simplex> im( quadratureOrder );
        auto const basisAtQuadPts = detail::basisEvaluateAtPoints( p, im.points() );
        auto const polynomialAtQuadPts = q.evaluate( im.points() );

        typename space_type::matrix_type coefficients( space_type::nComponents,
                                                       basisAtQuadPts.size1() );
        coefficients.clear();
        for ( uint16_type component = 0; component < space_type::nComponents; ++component )
        {
            for ( uint16_type basis = 0; basis < basisAtQuadPts.size1(); ++basis )
            {
                value_type value = 0;
                for ( uint16_type quad = 0; quad < im.nPoints(); ++quad )
                    value += im.weight( quad ) *
                             polynomialAtQuadPts( component, quad ) *
                             basisAtQuadPts( basis, quad );
                coefficients( component, basis ) = value;
            }
        }
...
class [[deprecated( "IntegralMomentsOnFace is incomplete; use an entity-moment builder from the appropriate FE-family functional header." )]] IntegralMomentsOnFace
    :
...
#endif // FEELPP_FEELPOLY_MOMENTFUNCTIONALS_HPP
