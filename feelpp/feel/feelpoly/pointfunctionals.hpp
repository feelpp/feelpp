...
*/
/**
   \file pointfunctionals.hpp
   \author Christophe Prud'homme <christophe.prudhomme@feelpp.org>
   \date 2006-01-22
 */
#ifndef FEELPP_FEELPOLY_POINTFUNCTIONALS_HPP
#define FEELPP_FEELPOLY_POINTFUNCTIONALS_HPP 1
...
public:
        ...
        : super()
    {
        ...
        VLOG(4) << "[SecondDirectionalComponentPointEvaluation] m = " << m << "\n";
        //this->push_back( functional_type( b, m ) );
        this->push_back( functional_type( b, m ) );
    }
}; // SecondDirectionalComponentPointEvaluation
...
/**
 * Build the canonical point-evaluation dual block for scalar or
 * vector/tensor-valued polynomial spaces.
 *
 * @tparam Space primal polynomial space
 * @param space primal space on which the point functionals act
 * @param points functional support points
 * @return ordered scalar values or component-wise values
 */
template<typename Space>
[[nodiscard]] std::vector<Functional<Space>>
makePointEvaluationFunctionals( Space const& space,
                                typename Space::points_type const& points )
{
    if constexpr ( Space::is_scalar )
    {
        auto evaluations = PointsEvaluation<Space>( space, points );
        return { evaluations.begin(), evaluations.end() };
    }
    else
    {
        auto evaluations = ComponentsPointsEvaluation<Space>( space, points );
        return { evaluations.begin(), evaluations.end() };
    }
}
...
#endif // FEELPP_FEELPOLY_POINTFUNCTIONALS_HPP
