    ...
    if ( FEELPP_APP_TESTS )
      ...
      if ( EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/.tests.${FEELPP_APP_NAME} )
        ...
        foreach(TEST ${TESTS_${FEELPP_APP_NAME}})
          ...
          set(FEELPP_APP_TEST_HAS_DIRECTORY OFF)
          foreach(FEELPP_APP_TEST_ARG ${TEST})
            if ( "${FEELPP_APP_TEST_ARG}" MATCHES "^--directory($|=)" )
              set(FEELPP_APP_TEST_HAS_DIRECTORY ON)
            endif()
          endforeach()
          set(FEELPP_APP_TEST_SKIP_RETURN_CODE)
          foreach(FEELPP_APP_TEST_ARG ${TEST})
            if ( "${FEELPP_APP_TEST_ARG}" MATCHES "^--.*pc-type=hpddm$" )
              set(FEELPP_APP_TEST_SKIP_RETURN_CODE 77)
            endif()
          endforeach()
          # Keep the leaf repository case short. The repository layer appends np_<N>,
          # and --directory already carries suite/case isolation for generated tests.
          if ( FEELPP_APP_TEST_HAS_DIRECTORY )
            set(FEELPP_APP_TEST_REPOSITORY_CASE "${FEELPP_APP_TEST_EXPR_SUBDIR}")
          else()
            set(FEELPP_APP_TEST_REPOSITORY_CASE "${execname}-${FEELPP_APP_TEST_EXPR_SUBDIR}")
          endif()
          if ( FEELPP_ENABLE_VERBOSE_CMAKE )
          ...
          IF(NOT FEELPP_APP_NO_MPI_TEST AND NProcs2 GREATER 1)
            add_test(NAME ${execname}-${TEST_NAME}-np-${NProcs2} COMMAND ${MPIEXEC} ${MPIEXEC_NUMPROC_FLAG} ${NProcs2} ${MPIEXEC_PREFLAGS} ${CMAKE_CURRENT_BINARY_DIR}/${execname} ${TEST} --repository.case=${FEELPP_APP_TEST_REPOSITORY_CASE} ${MPIEXEC_POSTFLAGS} )
            set_property(TEST ${execname}-${TEST_NAME}-np-${NProcs2} PROPERTY PROCESSORS ${NProcs2})
            if ( FEELPP_APP_TEST_SKIP_RETURN_CODE )
              set_tests_properties(${execname}-${TEST_NAME}-np-${NProcs2} PROPERTIES SKIP_RETURN_CODE ${FEELPP_APP_TEST_SKIP_RETURN_CODE})
            endif()
            list(APPEND APP_TESTS ${execname}-${TEST_NAME}-np-${NProcs2})
          endif()

          IF(NOT FEELPP_APP_NO_SEQ_TEST)
            add_test(NAME ${execname}-${TEST_NAME}-np-1 COMMAND ${CMAKE_CURRENT_BINARY_DIR}/${execname} ${TEST} --repository.case=${FEELPP_APP_TEST_REPOSITORY_CASE})
            if ( FEELPP_APP_TEST_SKIP_RETURN_CODE )
              set_tests_properties(${execname}-${TEST_NAME}-np-1 PROPERTIES SKIP_RETURN_CODE ${FEELPP_APP_TEST_SKIP_RETURN_CODE})
            endif()
...
macro(feelpp_add_test)
  ...
  get_directory_property( FEELPP_TEST_LABELS_DIRECTORY LABELS )
  ...
  set( FEELPP_TEST_ALL_LABELS ${FEELPP_TEST_LABELS_DIRECTORY} ${FEELPP_TEST_LABEL_DIRECTORY} ${FEELPP_TEST_LABEL} )
  list(FILTER FEELPP_TEST_ALL_LABELS EXCLUDE REGEX "^$")
  list( REMOVE_DUPLICATES FEELPP_TEST_ALL_LABELS )
  if ( NOT FEELPP_TEST_ALL_LABELS )
    set( FEELPP_TEST_ALL_LABELS testsuite )
  endif()
  set_property(TARGET ${targetname} PROPERTY LABELS ${FEELPP_TEST_ALL_LABELS})

  foreach( _label ${FEELPP_TEST_ALL_LABELS} )
    if ( TARGET ${_label} )
      add_dependencies( ${_label} ${targetname} )
    endif()
  endforeach()
  if ( TARGET ${FEELPP_TEST_TARGET_AGGREGATE} )
    if ( FEELPP_TEST_LABEL_DIRECTORY )
      if ( TARGET ${FEELPP_TEST_LABEL_DIRECTORY} AND NOT FEELPP_TEST_TARGET_AGGREGATE STREQUAL FEELPP_TEST_LABEL_DIRECTORY )
        add_dependencies( ${FEELPP_TEST_TARGET_AGGREGATE} ${FEELPP_TEST_LABEL_DIRECTORY} )
      elseif( NOT FEELPP_TEST_TARGET_AGGREGATE IN_LIST FEELPP_TEST_ALL_LABELS )
        add_dependencies( ${FEELPP_TEST_TARGET_AGGREGATE} ${targetname} )
      endif()
    elseif( NOT FEELPP_TEST_TARGET_AGGREGATE IN_LIST FEELPP_TEST_ALL_LABELS )
      add_dependencies( ${FEELPP_TEST_TARGET_AGGREGATE} ${targetname} )
    endif()
  elseif ( TARGET testsuite AND NOT "testsuite" IN_LIST FEELPP_TEST_ALL_LABELS )
  ...
  if ( NOT FEELPP_TEST_NO_TEST )
      ...
      IF(NOT FEELPP_TEST_NO_MPI_TEST AND NProcs2 GREATER 1)
        if ( FEELPP_TEST_SKIP_TEST OR FEELPP_TEST_SKIP_MPI_TEST )
          ...
          add_test(NAME ${FEELPP_TEST_EXEC}-np-${NProcs2} COMMAND ${MPIEXEC} ${MPIEXEC_NUMPROC_FLAG} ${NProcs2} ${MPIEXEC_PREFLAGS} ${CMAKE_CURRENT_BINARY_DIR}/${targetname} --log_level=all ${BOOST_TEST_SEPARATOR} ${MPIEXEC_POSTFLAGS} ${FEELPP_TEST_CFG_CLI} ${FEELPP_TEST_CLI} --directory=testsuite/test_${FEELPP_TEST_NAME}_np_${NProcs2} --rm )
        endif()
        set_property(TEST ${FEELPP_TEST_EXEC}-np-${NProcs2}  PROPERTY LABELS ${FEELPP_TEST_ALL_LABELS} )
      ...
      if ( FEELPP_TEST_SKIP_TEST OR FEELPP_TEST_SKIP_SEQ_TEST )
         ...
         add_test(NAME ${FEELPP_TEST_EXEC}-np-1 COMMAND ${CMAKE_CURRENT_BINARY_DIR}/${targetname} --log_level=all ${BOOST_TEST_SEPARATOR} ${FEELPP_TEST_CFG_CLI} ${FEELPP_TEST_CLI} --directory=testsuite/test_${FEELPP_TEST_NAME}_np_1 --rm )
      endif()
      set_property(TEST ${FEELPP_TEST_EXEC}-np-1  PROPERTY LABELS ${FEELPP_TEST_ALL_LABELS} )
      if(CMAKE_BUILD_TYPE MATCHES Debug)
