...
makeAbout()
{
    AboutData about( "test_hdiv3D_oneelt" ,
                     "test_hdiv3D_oneelt" ,
                     "0.1",
...
TestHDiv3DOneElt::shape_functions( std::string one_element_mesh )
{
    ...
    auto refine_level = std::floor(1 - math::log( 0.05 )); //Deduce refine level from meshSize (option)
    mesh_ptrtype mesh = loadMesh( _mesh=new mesh_type,
                                      _filename=mesh_name,
    ...
    auto F = M_backend->newVector( Xh->dof() );
