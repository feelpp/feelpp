...
TestHCurlLag::exampleProblem1()
{
  ...
  mixed_space_s_ptrtype    Dh = mixed_space_s_type::New(mesh)   ;
  mixed_space_v_ptrtype    Eh = mixed_space_v_type::New(mesh)   ;
  ...
  A1 = vf::project(Ah, elements(mesh), idv(A2));
  B1 = vf::project(Bh, elements(mesh), idv(B2));
  C1 = vf::project(Ch, elements(mesh), idv(C2));
  D1 = vf::project(Dh, elements(mesh), idv(D2));
  E1 = vf::project(Eh, elements(mesh), idv(E2));
}
}
