...
makeOptions()
{
    ...
    laplacianoptions.add_options()
        ...
        ("ft", Feel::po::value<double>()->default_value( 1000 ), "final time value")
        ...
        ("hsize", Feel::po::value<double>()->default_value( 0.08 ), "first h value to start convergence")
        ("bctype", Feel::po::value<int>()->default_value( 1 ), "0 = strong Dirichlet, 1 = weak Dirichlet")
        ("export-directory", Feel::po::value<std::string>()->default_value( "" ), "export directory")
    ...
    for( t = dt; t < ft; t += dt )
        {
            ...
            backend_solve->solve( _matrix=M, _solution=uEx, _rhs=L );
            backend_solve.reset();
            system( cmd.str().c_str() );
