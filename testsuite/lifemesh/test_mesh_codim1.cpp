...
struct test_mesh_filters
{
    ...
    void operator()()
    {
        ...
        // elements
        {
            ...
            Life::MeshTraits<detail::mesh_type>::element_const_iterator it = mesh->beginElement();
            Life::MeshTraits<detail::mesh_type>::element_const_iterator en = mesh->endElement();
            ...
            for( ; it != en; ++it )
                {
                    ...
                    LIFE_ASSERT( ublas::norm_frobenius( __c->xReal() - it->G() ) < 1e-15 )( it->id() )( __c->xReal() )( it->G() ).error( "invalid element" );
                    LIFE_ASSERT( it->marker().value() == 3 )( it->id() )( it->marker().value() ).error( "invalid element marker" );
#if defined(USE_BOOST_TEST)
                ...
                // location faces
        {
            ...
            Life::MeshTraits<detail::mesh_type>::location_face_const_iterator it = mesh->beginFaceOnBoundary();
            Life::MeshTraits<detail::mesh_type>::location_face_const_iterator en = mesh->endFaceOnBoundary();
            ...
            for( ; it != en; ++it )
                {
                    ...
                    Life::node<double>::type n10 = it->element(1).point( it->element(1).fToP( face_1, 0 ) ).node();
...
#endif
                    //LIFE_ASSERT( it->element(0).nGeometricFaces() == 2 )( it->element(0).nGeometricFaces() ).error( "invalid number of faces" );
                    Life::node<double>::type n01 = it->element(0).point( it->element(0).fToP( face_0, 0 ) ).node();
