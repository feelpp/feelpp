...
BOOST_AUTO_TEST_CASE( test_specx_21 )
{
    ...
    {
        auto const& idk = k;
        TsK.add( _param( idk, valuesVec.at( idk ) ), _tasks = FC1 );
    }
...
BOOST_AUTO_TEST_CASE( test_specx_23 )
{
    ...
    int writeVal = 0;
    TsK.add(
        _param( initVal1, writeVal ),
        _tasks = []( const int& initValParam, int& writeValParam )
...
BOOST_AUTO_TEST_CASE( test_specx_25 )
{
    ...
    TsK.for_each( Vec.begin(), Vec.end(), _param( idk ), _tasks = FC1 );
