...
BOOST_AUTO_TEST_CASE( test_polynomial1D )
{
    ...
    pts(0,1 ) = -1;
    pts(0,2 ) = 1;
    detail::OrthonormalPolynomialSet<1,2,Scalar> ps;
    Polynomial<detail::OrthonormalPolynomialSet<1,2,Scalar>, Scalar> p1 = project( ps, x2(), IM<1,3>() );
    PolynomialSet<detail::OrthonormalPolynomialSet<1,2,Scalar>, Scalar> p(ps);
...
BOOST_AUTO_TEST_CASE( test_polynomial2D )
{
    ...
    pts(0,3 ) =  0;
    pts(1,3 ) =  0;
    detail::OrthonormalPolynomialSet<2,2,Scalar> ps;
    Polynomial<detail::OrthonormalPolynomialSet<2,2,Scalar>, Scalar> p1 = project( ps, x2(), IM<2,3>() );
    PolynomialSet<detail::OrthonormalPolynomialSet<2,2,Scalar>, Scalar> p(ps);
    p.insert( p1.toSet(true) );
    p.insert( p1.toSet(true) );
    std::cout << "p=" << p.coeff() << "\n"
              << "grad(p)(0)=" << p.gradient().evaluate( pts ) << "\n";
...
BOOST_AUTO_TEST_CASE( test_polynomial3D )
{
    ...
    pts(1,3 ) =  0;
    pts(2,3 ) =  0;
    detail::OrthonormalPolynomialSet<3,3,Scalar> ps;
    Polynomial<detail::OrthonormalPolynomialSet<3,3,Scalar>, Scalar> p = project( ps, x2(), IM<3,5>() );
