...
template<typename T>
T two( typename Life::node<T>::type const& /*t*/ )
{
  return 2.0;
}

template<typename T>
T one( typename Life::node<T>::type const& /*t*/ )
{
  return 1.0;
}

template<int P, typename T>
T xp( typename Life::node<T>::type const& t )
{
   if ( P ==0 ) return 1.0;
   T res = t[0];
   for(int i = 1;i < P; ++ i )
...
struct P2N
{
    ...
    typedef typename Life::node<T>::type node_type;
    ...
    {
        value_type res = Life::math::pow( t[0], M_alpha1 )* Life::math::pow( t[1], M_alpha2 );
        return res;
    }
    value_type integral() const
    {
        value_type a1 = Life::math::pow( -1.0, M_alpha2+1 )/(M_alpha2 +1);
        value_type a2 = 1./(M_alpha1+M_alpha2+2)-1./(M_alpha1+1);
        value_type a3 = Life::math::pow( -1.0, M_alpha1+M_alpha2+2 )/(M_alpha1 + M_alpha2+2);
        value_type a4 = Life::math::pow( -1.0, M_alpha1+1 )/(M_alpha1 + 1);
...
struct P3N
{
    ...
    value_type operator()( node_type const& t )
    {
        value_type res = Life::math::pow( t[0], M_alpha1 )* Life::math::pow( t[1], M_alpha2 )* Life::math::pow( t[2], M_alpha3 );
        return res;
    }
    value_type integral( int face )
    {
        ...
        switch( face )
            ...
            case FACE_2:
                return 0;
            case FACE_3:
                return -(Life::math::pow(-1.,a)+1)*Life::math::pow(-1.,c)/((a+1)*(a+3));
            }
    ...
    {
        value_type a = M_alpha1;
        return (2*Life::math::pow(a,3.)*Life::math::pow((-1.),a)+21*Life::math::pow(a,2.)*Life::math::pow((-1.),a)+
                67*a*Life::math::pow(-1.,a)+63*Life::math::pow(-1.,a)+3*Life::math::pow(a,2.)+21*a+33)/(3*(a+1)*(a+2)*(a+3)*(a+4)*(a+5));
    }
    int M_alpha1;
    int M_alpha2;
    int M_alpha3;
...
template<int P, typename T>
T xpdim( typename Life::node<T>::type const& t )
{
   if ( P ==0 ) return 1.0;
   T res = t[0];
   for(int i = 1;i < P; ++ i )
...
template<typename T>
T sint( typename Life::node<T>::type const& t )
{
  return sin(t[0]);
}
template<typename T>
T cost( typename Life::node<T>::type const& t )
{
  return cos(t[0]);
}

template<typename T>
T xp1( typename Life::node<T>::type const& t )
{
//   return pow(t[0]*t[1],T(P));
  return t[0];
}
template<typename T>
T xp2( typename Life::node<T>::type const& t )
{
//   return pow(t[0]*t[1],T(P));
  return t[0]*t[0];
}
template<typename T>
T xp3( typename Life::node<T>::type const& t )
{
//   return pow(t[0]*t[1],T(P));
  return t[0]*t[0]*t[0];
}
template<typename T>
T xp4( typename Life::node<T>::type const& t )
{
//   return pow(t[0]*t[1],T(P));
  return t[0]*t[0]*t[0]*t[0];
}
template<int P, typename T>
T exp2( typename Life::node<T>::type const& t )
{
//   return pow(t[0]*t[1],T(P));
  return exp(t[0]+t[1]);
}
template<typename T>
T sint2( typename Life::node<T>::type const& t )
{
  return sin(t[0])*sin(t[1]);
}
template<typename T>
T coscos( typename Life::node<T>::type const& t )
{
  return cos(t[0])*cos(t[1]);
}

template<Life::uint16_type D,
         Life::uint16_type N,
         typename T,
         template<class Convex, Life::uint16_type O, typename T2> class QPS = Life::Gauss>
class TestImPK
{
public:
    typedef T value_type;
    typedef typename Life::node<value_type>::type node_type;
    ...
    TestImPK( value_type res,
              ...
              value_type tol = Life::type_traits<T>::epsilon() )
        :
        _M_res( res ),
        _M_tol( tol ),
        _M_face( -2 ),
    ...
    value_type operator()() const
    {
        ...
        if ( _M_face != -2 )
            {
                integral = im.integrateAtPoints( Life::IntegrationFaceEnum(_M_face), _M_func );
            }
...
{
    TestImPK<1,1, double> t6( Life::FACE_0, -1.0 , xp<1,double> );
    t6();
}
BOOST_AUTO_TEST_CASE( im1d_test7 )
{
    TestImPK<1,1, double> t7( Life::FACE_1, 1.0 , xp<1,double> );
    t7();
}
BOOST_AUTO_TEST_CASE( im1d_face_test1 )
{
    TestImPK<1,1, double> t0( Life::FACE_0, -1.0 , xp<1,double> ); t0();
    TestImPK<1,1, double> t1( Life::FACE_1, 1.0 , xp<1,double> ); t1();
}
BOOST_AUTO_TEST_CASE( im1d_face_test2 )
{
    TestImPK<1,2, double> t0( Life::FACE_0, -1.0 , xp<1,double> ); t0();
    TestImPK<1,2, double> t1( Life::FACE_1, 1.0 , xp<1,double> ); t1();
}
BOOST_AUTO_TEST_CASE( im1d_face_test3 )
{
    TestImPK<1,3, double> t0( Life::FACE_0, -1.0 , xp<1,double> ); t0();
    TestImPK<1,3, double> t1( Life::FACE_1, 1.0 , xp<1,double> ); t1();
}

BOOST_AUTO_TEST_CASE( im2d_test0 )
{

    TestImPK<2,1, double> t1( P2N<double>( 0, 0).integral(), P2N<double>( 0, 0 ) );t1();
}
...
{
    TestImPK<2,1, double> t0( Life::FACE_1, 2.0 , one<double> ); t0();
    TestImPK<2,1, double> t1( Life::FACE_0, Life::math::sqrt( double( 8.0 ) ) , one<double> ); t1();
    TestImPK<2,1, double> t2( Life::FACE_2, 2.0 , one<double> ); t2();
}
BOOST_AUTO_TEST_CASE( im2d_face_test2 )
{
    TestImPK<2,2, double> t0( Life::FACE_1, 2.0 , one<double> ); t0();
    TestImPK<2,2, double> t1( Life::FACE_0, Life::math::sqrt( double( 8.0 ) ) , one<double> ); t1();
    TestImPK<2,2, double> t2( Life::FACE_2, 2.0 , one<double> ); t2();
}
BOOST_AUTO_TEST_CASE( im2d_face_test3 )
{
    TestImPK<2,3, double> t0( Life::FACE_1, 2.0 , one<double> ); t0();
    TestImPK<2,3, double> t1( Life::FACE_0, Life::math::sqrt( double( 8.0 ) ) , one<double> ); t1();
    TestImPK<2,3, double> t2( Life::FACE_2, 2.0 , one<double> ); t2();
}
BOOST_AUTO_TEST_CASE( im2d_face_test4 )
{
    TestImPK<2,4, double> t0( Life::FACE_1, 2.0 , one<double> ); t0();
    TestImPK<2,4, double> t1( Life::FACE_0, Life::math::sqrt( double( 8.0 ) ) , one<double> ); t1();
    TestImPK<2,4, double> t2( Life::FACE_2, 2.0 , one<double> ); t2();
}
BOOST_AUTO_TEST_CASE( im2d_face_test5 )
{
    TestImPK<2,5, double> t0( Life::FACE_1, 2.0 , one<double> ); t0();
    TestImPK<2,5, double> t1( Life::FACE_0, Life::math::sqrt( double( 8.0 ) ) , one<double> ); t1();
    TestImPK<2,5, double> t2( Life::FACE_2, 2.0 , one<double> ); t2();
}
BOOST_AUTO_TEST_CASE( im2d_face_test6 )
{
    TestImPK<2,6, double> t0( Life::FACE_1, 2.0 , one<double> ); t0();
    TestImPK<2,6, double> t1( Life::FACE_0, Life::math::sqrt( double( 8.0 ) ) , one<double> ); t1();
    TestImPK<2,6, double> t2( Life::FACE_2, 2.0 , one<double> ); t2();
}
...
{
    TestImPK<3,1, double> t1( Life::FACE_2, 2.0 , one<double> ); t1();
    TestImPK<3,1, double> t2( Life::FACE_1, 2.0 , one<double> ); t2();
    TestImPK<3,1, double> t3( Life::FACE_0, 2.0*Life::math::sqrt( double( 3.0 ) ), one<double> ); t3();
    TestImPK<3,1, double> t4( Life::FACE_3, 2.0 , one<double> ); t4();
    ...
    for( int alpha1 = 0; alpha1 < N-1; ++ alpha1 )
        {
            ...
            TestImPK<3,N, double> t10( Life::FACE_3, P3N<double>( alpha1, alpha2, alpha3).integral( FACE_3), P3N<double>( alpha1, alpha2, alpha3 ) );
            t10();
        }
...
{
    const int N = 2;
    TestImPK<3,N, double> t1( Life::FACE_2, 2.0 , one<double> ); t1();
    TestImPK<3,N, double> t2( Life::FACE_1, 2.0 , one<double> ); t2();
    TestImPK<3,N, double> t3( Life::FACE_0, 2.0*Life::math::sqrt( double( 3.0 ) ), one<double> ); t3();
    TestImPK<3,N, double> t4( Life::FACE_3, 2.0 , one<double> ); t4();
...
// 1D quadrature on segment [-1;1]
  ...
  test->add( BOOST_TEST_CASE( ( TestImPK<1,1, value_type>( Life::FACE_0, -1.0 , xp<1,value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<1,1, value_type>( Life::FACE_1, 1.0 , xp<1,value_type> ) )  ) );

    // 2D quadrature on triangle [-1;1]x[-1;-x]
  test->add( BOOST_TEST_CASE( ( TestImPK<2,1, value_type>( 2.0 , one<value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<2,1, value_type>( Life::FACE_1, 2.0 , one<value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<2,1, value_type>( Life::FACE_0, Life::math::sqrt( value_type( 8.0 ) ) , one<value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<2,1, value_type>( Life::FACE_2, 2.0 , one<value_type> ) )  ) );
  ...
  test->add( BOOST_TEST_CASE( ( TestImPK<2,1, value_type>( Life::ALL_FACES, value_type(4.0)+sqrt(value_type(8.0)), one<value_type> ) )  ) );

    // 3D quadrature [-1;1]x-[-1;-x]x[-1;-x]
  test->add( BOOST_TEST_CASE( ( TestImPK<3,1, value_type>( 8.0/value_type(6.0) , one<value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<3,1, value_type>( Life::FACE_2, 2.0 , one<value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<3,1, value_type>( Life::FACE_1, 2.0 , one<value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<3,1, value_type>( Life::FACE_0, 2.0*Life::math::sqrt( value_type( 3.0 ) ), one<value_type> ) )  ) );
  test->add( BOOST_TEST_CASE( ( TestImPK<3,1, value_type>( Life::FACE_3, 2.0 , one<value_type> ) )  ) );
...
init_unit_test_suite( int /*argc*/, char** /*argv*/ )
{
    ...
    add_tests<qd_real>( test );
     ...
     std::cout << "[lifepoly::testsuite::test_im] test with arbitrary precision\n";
     mp::mp_init( 200 );
...
int main()
{
    ...
    TestImPK<1,1, double> im0( Life::FACE_0, 1.0 , one<double> ); im0();

    TestImPK<1,1, double> im1( Life::FACE_1, 1.0 , one<double> );im1();
    TestImPK<1,1, double> im11( Life::FACE_0,-1.0 , xp<1,double> );im11();
    TestImPK<1,1, double> im2( Life::FACE_0, 1.0 , xp<2,double> );im2();
    TestImPK<1,1, double> im3( Life::FACE_1, 1.0 , xp<2,double> );im3();
