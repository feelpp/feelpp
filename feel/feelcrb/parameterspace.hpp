...
class ParameterSpace : public boost::enable_shared_from_this<ParameterSpace<P>>
{
  public:
    ...
    class Element : public Eigen::Matrix<double, Dimension, 1>
    {
      ...
      public:
        ...
        void check() const
        {
            ...
            {
                std::cout << "Parameter not identical on all processors: "
                          << "current parameter on proc " << proc_number << " : [";
                for ( int i = 0; i < this->size(); i++ )
                    std::cout << std::setprecision( 15 ) << this->operator()( i ) << ", ";
    ...
    {
        element_ptrtype e( new element_type( this->shared_from_this() ) );
        *e = element();
        return e;
    }
    bool check() const
      ...
      public:
        ...
        void mixEquiLogEquidistributeProduct( std::vector<int> Nlogequi, std::vector<int> Nequi )
        {
            ...
            if ( Environment::worldComm().globalRank() == Environment::worldComm().masterRank() )
            {
                ...
                FEELPP_ASSERT( number_of_directions_equi == number_of_directions_logequi )
                ( number_of_directions_equi )( number_of_directions_logequi )
    ...
    static std::vector<double> logEquidistributeInDirection( parameterspace_ptrtype space, int direction, int N )
    {
        ...
        FEELPP_ASSERT( space_dimension > direction )
        ( space_dimension )( direction ).error( "bad dimension of vector containing number of samples on each direction" );
        FEELPP_ASSERT( direction >= 0 )
        ( direction ).error( "the direction must be positive number" );
...
template <int P>
const uint16_type ParameterSpace<P>::Dimension;
                                                     ...
                                                     std::vector<int>& index_vector )
{
    ...
    if ( Environment::worldComm().globalRank() == Environment::worldComm().masterRank() )
    {
        ...
        for ( size_type i = 0; i < this->size(); ++i )
        {
            ...
            FEELPP_ASSERT( data_pts[i] != 0 )
            ( i ).error( "invalid pointer" );
        }
