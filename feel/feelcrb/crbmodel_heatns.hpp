...
*/
/**
   \file crbmodel_heatns.hpp
   \author Christophe Prud'homme <christophe.prudhomme@feelpp.org>
   \date 2009-08-09
 */
#ifndef __CRBModel_heatns_H
#define __CRBModel_heatns_H 1
...
namespace Feel
{
enum class CRBModelMode_heatns
{
...
public:
    ...
    CRBModel_heatns()
        ...
        M_mode( CRBModelMode_heatns::PFEM ),
    ...
    CRBModel_heatns( po::variables_map const& vm, CRBModelMode_heatns mode = CRBModelMode_heatns::PFEM  )
        :
        M_Aq(),
        M_Mq(),
        M_Fq(),
     ...
     */
    CRBModel_heatns( model_ptrtype & model )
        :
        M_Aq(),
        M_Mq(),
        M_Fq(),
     ...
     * copy constructor
     */
    CRBModel_heatns( CRBModel_heatns const & o )
        :
        M_Aq( o.M_Aq ),
        M_Mq( o.M_Mq ),
        M_Fq( o.M_Fq ),
    ...
    FEELPP_DONT_INLINE void init()
    {
        ...
        if ( M_mode != CRBModelMode_heatns::CRB_ONLINE &&
                M_mode != CRBModelMode_heatns::SCM_ONLINE )
        {
    ...
    //! copy operator
    CRBModel_heatns& operator=( CRBModel_heatns const & o )
    {
        if ( this != &o )
        {
            M_Aq = o.M_Aq;
    ...
    {
        return computeThetaq( mu , mpl::bool_<false>(), time  );
    }
    ...
    {
        std::cout <<"-------->CRBmodel::offline_merge_type update( parameter_type const& mu,  double time=0 )\n";
    ...
    {
        return computeAffineDecomposition( mpl::bool_<false>() );
    }
    ...
    {
        return thetaMq( q,mpl::bool_<false>() );
    }
    ...
    {
        return scalarProductForPod( X, Y ,mpl::bool_<false>() );
    }
    ...
    {
        return computeNumberOfSnapshots( mpl::bool_<false>() );
    }
    ...
    double timeStep()
    {
        return timeStep( mpl::bool_<false>() );
    }
    ...
    double timeInitial()
    {
        return timeInitial( mpl::bool_<false>() );
    }
    ...
    double timeFinal()
    {
        return timeFinal( mpl::bool_<false>() );
    }
    ...
    int timeOrder()
    {
        return timeOrder( mpl::bool_<false>() );
    }
    ...
    bool isSteady()
    {
        return isSteady( mpl::bool_<false>() );
    }
    ...
    {
        return initializationField( initial_field,mu,mpl::bool_<false>() );
    }
...
void
CRBModel_heatns<TruthModelType>::initB()
{
    M_B = M_model->innerProduct();
}

template<typename TruthModelType>
typename CRBModel_heatns<TruthModelType>::offline_merge_type
CRBModel_heatns<TruthModelType>::offlineMerge( parameter_type const& mu )
{
    std::cout << "-------->CRBmodel::CRBModel_heatns<TruthModelType>::offlineMerge( parameter_type const& mu )\n";
...
template<typename TruthModelType>
void
CRBModel_heatns<TruthModelType>::run( const double * X, unsigned long N, double * Y, unsigned long P )
{
    M_model->run( X, N, Y, P );
}
...
CRBModel_heatns<TruthModelType>::run()
{
    
    std::cout << "------->CRBModel_heatns<TruthModelType>::run()\n";
...
template<typename TruthModelType>
std::map<double, boost::tuple<double,double,double,int,double,double> >
CRBModel_heatns<TruthModelType>::runq()
{
    // parameter space
    parameterspace_ptrtype Dmu = M_model->parameterSpace();
