...
CRBAero<TruthModelType>::buildRbMatrixTrilinear( int number_of_added_elements, parameter_type& mu )
{
    ...
    for ( int q=0; q<q_tri; q++ )
    {
        ...
        for ( int k=0; k<N0; k++ )
        {
            ...
            this->blockTriqm(0,0)[q][k].resize( N0, N0 );
        ...
        for ( int k=0; k<N2; k++ )
        {
            ...
            this->blockTriqm(2,0)[q][k].resize( N0, N2 );
            auto trilinear_operator = model->assembleTrilinearOperator( Ut, q );
            for ( int i=0; i<N0; i++ )
            {
                ur = XN0->primalBasisElement(i);
                for ( int j=0; j<N2; j++ )
                {
                    Tc = XN2->primalBasisElement(j);
                    this->blockTriqm(2,0)[q][k](i,j) = trilinear_operator->energy( Ur, Uc );
                }
            }
        }
    }
    ...
    if ( computeOutput )
    {
        ...
        for ( size_type q=0; q<model->Ql(out_index); q++ )
        {
            for ( size_type m=0; m<model->mMaxF(out_index,q); m++ )
            {
                ...
                L.segment(N1,N2) += betaFqm[out_index][q][m]*blockLqm(2)[q][m].head(N2);
            }
        }
...
CRBAero<TruthModelType>::updateJacobianOnline( const map_dense_vector_type& X, map_dense_matrix_type& J , parameter_type const& mu , int N ) const
{
    ...
    for ( int q=0; q<model->Qa(); q++ )
    {
        for ( int m=0; m<model->mMaxA(q); m++ )
        {
            ...
            J.block( 0, N1, N0, N2) += betaJqm[q][m]*blockAqm(0,2)[q][m].block(0,0,N0,N2);
            // second row
            ...
            J.block(N0, N1, N1, N2) += betaJqm[q][m]*blockAqm(1,2)[q][m].block(0,0,N1,N2);
            // third row
            J.block(N1,  0, N2, N0) += betaJqm[q][m]*blockAqm(2,0)[q][m].block(0,0,N2,N0);
            J.block(N1, N0, N2, N1) += betaJqm[q][m]*blockAqm(2,1)[q][m].block(0,0,N2,N1);
            J.block(N1, N1, N2, N2) += betaJqm[q][m]*blockAqm(2,2)[q][m].block(0,0,N2,N2);
        }
    }
    ...
    for ( int q=0; q<model->QTri(); q++ )
    {
        ...
        for ( int k=0; k<N2; k++ )
        {
            for( int i=0; i<N0; i++ )
            {
                J(N1+k,i) += betaTri[q]*(blockTriqm(2,0)[q][k].row(i)).dot(X.segment(N1,N2));
            }
            for( int i=0; i<N2; i++ )
            {
                J(N1+k,N1+i) += betaTri[q]*(blockTriqm(2,0)[q][k].col(i)).dot(X.segment(0,N0));
            }
        }
    }
...
CRBAero<TruthModelType>::updateResidualOnline( const map_dense_vector_type& X, map_dense_vector_type& R , parameter_type const& mu , int N ) const
{
    ...
    for ( int q=0; q<model->Ql(0); q++ )
    {
        for ( int m=0; m<model->mMaxF(0,q); m++ )
        {
            ...
            R.segment(N1,N2) += betaRqm[0][q][m]*blockFqm(2)[q][m].head(N2);
        }
    }
    ...
    for ( int q=0; q<model->Qa(); q++ )
    {
        if ( model->mMaxA(q)==1 )
        {
            int m=0;
            // first row
            temp.block( 0,  0, N0, N0) += betaJqm[q][m]*blockAqm(0,0)[q][m].block(0,0,N0,N0);
            temp.block( 0, N0, N0, N1) += betaJqm[q][m]*blockAqm(0,1)[q][m].block(0,0,N0,N1);
            temp.block( 0, N1, N0, N2) += betaJqm[q][m]*blockAqm(0,2)[q][m].block(0,0,N0,N2);
            // second row
            temp.block(N0,  0, N1, N0) += betaJqm[q][m]*blockAqm(1,0)[q][m].block(0,0,N1,N0);
            temp.block(N0, N0, N1, N1) += betaJqm[q][m]*blockAqm(1,1)[q][m].block(0,0,N1,N1);
            temp.block(N0, N1, N1, N2) += betaJqm[q][m]*blockAqm(1,2)[q][m].block(0,0,N1,N2);
            // third row
            temp.block(N1,  0, N2, N0) += betaJqm[q][m]*blockAqm(2,0)[q][m].block(0,0,N2,N0);
            temp.block(N1, N0, N2, N1) += betaJqm[q][m]*blockAqm(2,1)[q][m].block(0,0,N2,N1);
            temp.block(N1, N1, N2, N2) += betaJqm[q][m]*blockAqm(2,2)[q][m].block(0,0,N2,N2);
        }
    }
    ...
    for ( int q=0; q<model->QTri(); q++ )
    {
        ...
        for ( int k=0; k<N2; k++ )
            for ( int i=0; i<N0; i++ )
                temp(N1+k,i) = (blockTriqm(2,0)[q][k].row(i)).dot(X.segment(N1,N2));
        R += betaTri[q]*temp*X;
    }
