...
public:
    ...
    GeimFunctionalModel( space_ptrtype space,
...
public:
    ...
    typedef GeimFunctionalModel<Space> element_type;

    typedef Space space_type;

    typedef boost::shared_ptr<space_type> space_ptrtype;

    DictionnaryGeim(  space_ptrtype space,
                     std::vector<std::vector<double>> Centers = std::vector<std::vector<double>>(),
                     std::vector<double> Radiuss = std::vector<double>(),
                     int size = 0):
        M_Centers(Centers),
        M_Radiuss(Radiuss),
        M_size(size)
    ...
    std::vector<double> radiuss() const
    {
        return M_Radiuss;
    }
    void run()
    {
        this->resize( M_size, element_type( M_space ) );
    ...
    void addElement(  space_ptrtype space, std::vector<double> center, double radius )
    {
        M_size++;
        element_type newElement( space, center, radius );
...
public:
    ...
    typedef typename data_type::element_type form_type;
    ...
    GEIM( std::string const& name= "default", WorldComm const& worldComm = Environment::worldComm() )
        :
        ...
        {}
    GEIM( model_type* model, data_type* data, sampling_ptrtype sampling, double __tol = 1e-8, bool offline_done=false )
        :
        ...
        M_correct_RB_SER( false )
        {
            ...
            if ( do_offline )
            {
                LOG(INFO) << "construct GEIM approximation...\n";
    ...
    size_type nDOF() const {  FEELPP_ASSERT( M_model != 0 ).error( "Invalid GEIM model" ); return M_model->functionSpace()->nLocalDof(); }
...
template <typename ModelType, typename DataType>
typename boost::tuple< double, typename GEIM<ModelType,DataType>::element_type >
GEIM<ModelType, DataType>::interpolationErrorEstimation( parameter_type const & mu, model_solution_type const& solution , int M ) const
{
    ...
    auto geim_approximation = this->operator()(mu, solution, M);
    auto geim_approximation_at_points = M_model->evaluateElementAtInterpolationPoints( geim_approximation , M+1 );
    double geim = geim_approximation_at_points(M);

    double coeff = math::abs( projected_expression - geim );
...
template <typename ModelType, typename DataType>
double
GEIM<ModelType,DataType>::errorEstimationLinf( parameter_type const & mu, model_solution_type const& solution , int M ) const
{
    double max = M_model->mMax();
    CHECK( M <= max ) << "Invalid number M for errorEstimation: " << M << " Mmax : " << max << "\n";
    auto projected_expression = M_model->operator()( solution  , mu );
    auto geim_approximation = this->operator()(mu, solution, M);
    auto diff = idv( projected_expression ) - idv( geim_approximation );
...
template<typename ModelType, typename DataType>
void
GEIM<ModelType, DataType>::orthonormalize( std::vector<element_type> & __Z )
{
...
template<typename ModelType, typename DataType>
typename GIM<ModelType, DataType>::parameter_residual_type
GEIM<ModelType, DataType>::computeBestFit( sampling_ptrtype trainset, int __M)
{
...
template<typename ModelType, typename DataType>
void
GEIM<ModelType, DataType>::offline()
{
    using namespace vf;
    ...
    if( M_restart )
    {
        ...
        if( this->worldComm().isMasterRank() )
        {
            std::cout<<" ******************offline GEIM for expression "<<M_model->name()<<" start "<<std::endl;
        }
        ...
        M_model->addInterpolationPoint( sig );
        ...
        M_greedy_rbmaxerr = 0;
        toc("GEIM offline initialization (M=1) - " + M_model->name(), FLAGS_v>0);
    }//if M_restart
    else
    {
        M_M = M_model->mMax()+1;
        ...
        {
            // Cobuild : If the first group (FEM solve) of GEIM basis has already been built, go to loadDB (crb)
            if( M_model->mMax()-1 >= cobuild_freq && !M_model->rbBuilt() )
            {
                if( this->worldComm().isMasterRank() )
                    std::cout << "First group of GEIM has already been built, start to load rb..." << std::endl;
                return;
            }
        }
    ...
    */
    ...
    {
        if( M_restart ) // First GEIM basis with SER : do only initialization step (require only one FEM approx)
            Mmax = 0;
    ...
    std::string geim_greedy_file_name = "cvg-geim-"+M_model->name()+"-Greedy-max-error.dat";
    std::string geim_greedy_inc_file_name = "cvg-geim-"+M_model->name()+"-Greedy-max-error-inc.dat";
    std::ofstream greedy_maxerr, greedy_maxerr_inc;
    if( this->worldComm().isMasterRank() )
    {
        greedy_maxerr.open(geim_greedy_file_name, std::ios::app);
        if( doption(_name="ser.radapt-eim-rtol") != 0 )
            greedy_maxerr_inc.open(geim_greedy_inc_file_name, std::ios::app);
        if( M_M == 2 )
        {
    ...
    for( ; M_M <=Mmax; ++M_M ) //err >= this->M_tol ) //Mmax == 1 : the basis has already been built at init step
    {
        ...
        // Print summary of GEIM iterations
        ...
        if( ioption(_name="ser.eim-frequency") != 0  ) // SER
        {
            ...
            if( M_M > 2 ) // Ensure M_greedy_maxerr (error for previous GEIM approx.) is initialized
            {
                ...
                // increment > 1e-10 and inc_relative > 0 avoid to take into account GEIM used for constants
                ...
                {
                    if( this->worldComm().isMasterRank() )
                        std::cout << " -- Relative increment < tol : RB approx used for next GEIM will be corrected " << std::endl;
    ...
    time=timer.elapsed();
    if( this->worldComm().isMasterRank() )
        std::cout<<"Total time for offline step of GEIM "<<M_model->name()<<" : "<<time<<"s\n"<<std::endl;
...
template<typename ModelType, typename DataType>
void
GEIM<ModelType, DataType>::studyConvergence( parameter_type const & mu , model_solution_type & solution , std::vector<std::string> all_file_name ) const
{
    ...
    for(int N=1; N<=max; N++)
    {
        ...
        auto geim_approximation = this->operator()(mu , solution, N);
        diffl2norm = M_model->projDiffL2Norm( solution , mu , geim_approximation );
        double absolute_linf_error = M_model->projDiffLinfNorm( solution , mu , geim_approximation );
...
template<typename SpaceType, typename ModelSpaceType, typename ParameterSpaceType, typename DataType>
class GEIMFunctionBase : public CRBDB
...
public:
    ...
    typedef DataType data_type;
    typedef DataType::element_type dataspace_type;


    typedef ParameterSpaceType parameterspace_type;
    typedef boost::shared_ptr<parameterspace_type> parameterspace_ptrtype;
    typedef typename parameterspace_type::element_type parameter_type;
    typedef typename dataspace_type::super_type node_type;
    ...
    typedef GEIM<EIMFunctionBase<SpaceType,model_functionspace_type, dataspace_type, ParameterSpaceType>, DataType > geim_type;
    typedef typename geim_type::vector_type vector_type;

    typedef boost::shared_ptr<geim_type> geim_ptrtype;
    ...
    GEIMFunctionBase( parameterspace_ptrtype const& pspace,
                     sampling_ptrtype const& sampling,
                     std::string const& modelname,
                     std::string const& name,
                     uuids::uuid const& uid )
        :
        super_type( name, "geim", uid ),
        M_fspace(),
        M_pspace( pspace ),
        M_trainset( sampling ),
    ...
    GEIMFunctionBase( functionspace_ptrtype const& fspace,
                     ...
                     uuids::uuid const& uid )
        :
        GEIMFunctionBase( pspace, sampling, modelname, name, uid )
        {
            M_fspace = fspace;
        }
    virtual ~GEIMFunctionBase()
        {}
    ...
    value_type operator()( node_type const& sig, parameter_type const& mu )
        {
            VLOG(2) << "calling GEIMFunctionBase::operator()( sigma=" << sig << ", mu=" << mu << ")\n";
            element_type v = this->operator()( mu );
            value_type res = sig(v);
            VLOG(2) << "EIMFunctionBase::operator() sig(v)=" << res << "\n";
            return res;
        }

    // evaluate geim expansion at interpolation points in space and mu in parameter where T provides the coefficient
    //value_type operator()( vector_type const& T, parameter_type const& mu ) = 0;

    value_type operator()( model_solution_type const& T, node_type const& sig, parameter_type const& mu )
        {
            VLOG(2) << "calling GEIMFunctionBase::operator()( sig=" << sig << ", mu=" << mu << ")\n";
    ...
    virtual double diffL2Norm( model_solution_type const& T , parameter_type const& mu , element_type const& geim_expansion ) const = 0;
    virtual double projExpressionL2Norm( model_solution_type const& T , parameter_type const& mu ) const = 0;
    virtual double projDiffL2Norm( model_solution_type const& T , parameter_type const& mu , element_type const& geim_expansion ) const = 0;
    virtual double projDiffLinfNorm( model_solution_type const& T , parameter_type const& mu , element_type const& geim_expansion ) const = 0;
...
template<typename GEimSpaceType,typename ModelType>
class GEIMFunctionFeSpaceDb
{
public :
    typedef GEimSpaceType geim_functionspace_type;
    typedef boost::shared_ptr<geim_functionspace_type> geim_functionspace_ptrtype;
    typedef typename geim_functionspace_type::element_type geim_element_type;
    ...
    geim_functionspace_ptrtype const& geimFunctionSpace() { return M_geimFunctionSpace; }
    ...
    std::vector<geim_element_type> const& q() const { return M_q_vector; }
    geim_element_type const& q( int k ) const { return M_q_vector[k]; }
    std::vector<geim_element_type> const& g() const { return M_g_vector; }
    geim_element_type const& g( int k ) const { return M_g_vector[k]; }
    std::vector<geim_element_type> const& z() const { return M_z_vector; }
    geim_element_type const& z( int k ) const { return M_z_vector[k]; }
    ...
    void setEimFunctionSpace( geim_functionspace_ptrtype const& space ) { M_geimFunctionSpace = space; }
    ...
    void addBasis( geim_element_type const &q )
    {
        M_q_vector.push_back( q );
    }
    void addExpressionEvaluation( geim_element_type const &g )
    {
        M_g_vector.push_back( g );
    }
    void addZ( geim_element_type const &z )
    {
    ...
    {
        //max of geim basis function projected on function space
        ...
        {
            if( M_q_vector.size() == 0 && this->geimFunctionSpace() )
            {
                for(int i = 0; i < M_max_q; i++ )
                    M_q_vector.push_back( this->geimFunctionSpace()->element() );
                for( int i = 0; i < M_max_q; ++ i )
                    __ar & BOOST_SERIALIZATION_NVP( M_q_vector[i] );
            }
            DVLOG(2) << "q saved/loaded ( "<<M_max_g<<" elements ) ";

            if( M_g_vector.size() == 0 && this->geimFunctionSpace() )
            {
                for(int i = 0; i < M_max_g; i++ )
                    M_g_vector.push_back( this->geimFunctionSpace()->element() );
                for( int i = 0; i < M_max_g; ++ i )
                    __ar & BOOST_SERIALIZATION_NVP( M_g_vector[i] );
            }
            DVLOG(2) << "g saved/loaded ( "<<M_max_g<<" elements ) ";

            if( M_z_vector.size() == 0 && this->geimFunctionSpace() )
            {
                for(int i = 0; i < M_max_z; i++ )
                    M_z_vector.push_back( this->geimFunctionSpace()->element() );
                for( int i = 0; i < M_max_z; ++ i )
...
private :
    geim_functionspace_ptrtype M_geimFunctionSpace;
    ...
    std::vector< geim_element_type > M_q_vector ;
    std::vector< geim_element_type > M_g_vector ;
    std::vector< geim_element_type > M_z_vector ; // we need to store this to be able to compute expression of geim basis functions and the user can use them in integrals
    std::vector< model_element_type > M_solution_vector ;
};


template<typename ModelType, typename DataType, typename SpaceType, typename ExprType, int SubSpaceId = -1, int SubSpaceId2 = -1>
class GEIMFunction
    : public GEIMFunctionBase<SpaceType, typename ModelType::functionspace_type, typename DataType::element_type, typename ModelType::parameterspace_type>
{
    typedef GEIMFunctionBase<SpaceType, typename ModelType::functionspace_type, typename DataType::element_type, typename ModelType::parameterspace_type> super;
public:
    typedef ModelType model_type;
    ...
    static const bool model_use_nosolve = boost::is_base_of<GEimFunctionNoSolveBase,model_type>::type::value;
    ...
    typedef typename super::geim_type geim_type;
    typedef typename super::geim_ptrtype geim_ptrtype;
    ...
    GEIMFunction( model_ptrtype model,
                 data_ptrtype data,
        ...
        M_write_nl_directory( soption( "eim.elements.directory") )
        {
           ...
           if ( this->functionSpace() )
           {
            ...
            if ( !hasLoadedDb )
                LOG(INFO) << "No GEIMFunction database ";
            else
                LOG( INFO ) << "GEIMFunction loaded";

            // build geim basis
            if ( this->functionSpace() )
                M_geim.reset( new geim_type( this, sampling , 1e-8, hasLoadedDb ) );
    ...
    /**
     * storing of shared_ptr model can be necessary (as GeimNoSolve)
     */
    ...
    void saveDB() override
    {
        ...
        M_geimFeSpaceDb.save( this->name() + "_fespace", this->dbDirectory(), this->worldComm() );
    }

    /**
     * load the GEIM database
     */
    bool loadDB() override
    {
        if( M_geimFeSpaceDb.q().size() > 0 && M_sig.size() > 0 )
        {
            return true;
        }
        ...
        if ( ifs )
        {
            ...
            if( M_ctxFeBasisGeim && M_ctxFeBasisGeim->nPoints()==0 && M_ctxFeModelSolution )
            {

                for(int i=0 ; i<M_sig.size(); i++)
                {
                    M_ctxFeBasisGeim->add( M_sig[i]);
                    M_ctxFeModelSolution->add( M_sig[i] );
                }
            }

            // load geim fe space
            bool hasLoadedFeSpaceDb = M_geimFeSpaceDb.load( this->name() + "_fespace", this->dbDirectory(), this->worldComm() );
            if ( this->functionSpace() )
    ...
    void initializeDataStructures() override
    {
        M_mu_sampling->clear();
        if ( M_ctxGeoGeim )
            M_ctxGeoGeim->removeCtx();
        M_ctxFeBasisGeim->removeCtx();
        M_ctxFeModelSolution->removeCtx();
        M_t.clear();
        M_M_max=0;
        M_B.resize(0,0);
        M_geimFeSpaceDb.clear();
    }
    ...
    beta( parameter_type const& mu, size_type __M ) override
    {
        ...
        if ( model_use_nosolve && M_ctxGeoGeim )
            __beta = this->operator()( *M_ctxGeoGeim, mu , __M );
        else
        {
    ...
    void addInterpolationPoint( node_type const& sig ) override
    {
        M_sig.push_back( sig );

        // add in precompute object the last magic point
        if ( M_ctxGeoGeim )
            M_ctxGeoGeim->add( sig );
        M_ctxFeBasisGeim->add( sig );
        M_ctxFeModelSolution->add( sig );
        std::for_each( M_sig.begin(), M_sig.end(), []( node_type const& sig ) { DVLOG(2) << "sig=" << sig << "\n"; } );
    }
    ...
    void updateRbSpaceContext( boost::any const& rbspacebase, mpl::false_ )
    {
        ...
        {
            std::cout << "[GEIMFunction::updateRbSpaceContext] cast fails with rbfunctionspace_ptrtype\n";
            return;
        }
        ...
        for (auto const& pt : M_sig )
        {
            M_ctxRbModelSolution->add( pt );
        }
    ...
    void updateRbSpaceContext( boost::any const& rbspacebase, mpl::true_ )
    {
        ...
        {);
            ...
            std::cout << "[GEIMFunction::updateRbSpaceContext (composite)] cast fails with rbfunctionspace_ptrtype\n";
            return;
        }
    ...
    void setRbSpaceContext( boost::any const& rbCtxBase ) override
    {
        ...
        {
            std::cout << "[GEIMFunction::setRbSpaceContext] cast fails with rbfunctionspace_context_ptrtype\n";
            return;
        }
    ...
    void setRbSpaceContext2( boost::any const& rbCtxBase ) override
    {
        ...
        {
            std::cout << "[GEIMFunction::setRbSpaceContext2] cast fails with rbfunctionspace_context2_ptrtype\n";
            return;
        }
    ...
    {
        int size = M_sig.size();
        DCHECK( position < size ) << "Invalid point position: " << position << " M_sig.size() =" << M_sig.size() << "\n";
        return M_sig[position];
    }

    void addBasis( element_type const &q ) override
    {
        M_geimFeSpaceDb.addBasis( q );
    }
    void addExpressionEvaluation( element_type const &g ) override
    {
        M_geimFeSpaceDb.addExpressionEvaluation( g );
    }
    ...
    {
        M_geimFeSpaceDb.addSolution( solution );
    }
    ...
    model_solution_type solve( parameter_type const&  mu ) override
    {
        ...
        if ( M_write_nl_solutions )
        {
            ...
            if ( need_solve )
                LOG(INFO) << "GEIM : Unable to load nl solution in direcotry "
                          << M_write_nl_directory << ", for parameter : " << mu.toString()
                          <<" / " << mu.key()<< ". Solve function will be called.";
            else
                LOG(INFO) << "GEIM : NL solution loaded in direcotry "
                          << M_write_nl_directory << ", for parameter : " << mu.toString()
                          <<" / " << mu.key();
        }
        ...
        if ( need_solve )
        {
            LOG(INFO) << "GEIM : calling solve function for parameter " << mu.toString()
                      <<" / " << mu.key();
            u = this->model()->solve(mu);

            if ( M_write_nl_solutions )
            {
                LOG(INFO) << "GEIM : Wrting solution on disk in directory "
                          << M_write_nl_directory << ", for parameter : " << mu.toString()
                          <<" / " << mu.key();
    ...
    element_type operator()( parameter_type const&  mu ) override
        {
            ...
            auto geimexpr = this->expr( mu, sol );
            //LOG(INFO) << "operator() mu=" << mu << "\n" << "sol=" << M_u << "\n";
            return vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
        }
    element_type operator()( model_solution_type const& T, parameter_type const&  mu ) override
        {
            auto geimexpr = this->expr( mu, T );
            return vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
        }

    vector_type operator()( geometricspace_context_type const& ctx, parameter_type const& mu , int M )
        {
            auto geimexpr = this->expr( mu );
            bool applyProjection = false;//true;
            bool doMpiComm = false;//true;//false;
            return evaluateFromContext( _context=ctx, _expr=geimexpr, _max_points_used=M,
    ...
    vector_type operator()( vectorN_type const& urb, rbfunctionspace_context_type const& ctx, parameter_type const& mu , int M )
        {
            ...
            for ( int k=0; k<dimRb; ++k )
                urbelt(k) = urb(k);
            auto geimexpr = this->expr( mu,urbelt );
            return evaluateFromContext( _context=ctx, _expr=geimexpr , _max_points_used=M,
    ...
    vector_type operator()( vectorN_type const& urb, rbfunctionspace_context_type const& ctx, rbfunctionspace_context2_type const& ctx2, parameter_type const& mu , int M )
        {
            ...
            for ( int k=0; k<dimRb2; ++k )
                urbelt2(k) = urb(k);
            auto geimexpr2 = this->expr( mu,urbelt,urbelt2 );
            return evaluateFromContext( _context=ctx, _context2=ctx2, _expr=geimexpr2 , _max_points_used=M,
                                        _mpi_communications=false, _projection=false );
        }
    ...
    vector_type computeExpansionCoefficients( parameter_type const& mu, model_solution_type const& solution,  int M) override
    {
        ...
        auto geimexpr = this->expr( mu, solution );

        if( 0 )// boption(_name="eim.compute-expansion-of-expression") )
        {
            rhs = evaluateFromContext( _context=*M_ctxFeModelSolution, _expr=geimexpr );
        }
        else
        {
            //auto proj_g = vf::project( _space=this->functionSpace(), _expr=M_expr );
            //rhs = proj_g.evaluate( M_ctx );
            rhs = evaluateFromContext( _context=*M_ctxFeModelSolution, _expr=geimexpr , _max_points_used=M, _projection=true );
        }
    ...
    {
        auto geimexpr = this->expr( mu, solution );
        return evaluateFromContext( _context=*M_ctxFeModelSolution, _expr=geimexpr , _max_points_used=M, _projection=true );
    }
    ...
    boost::tuple<double,node_type> computeMaximumOfResidual( parameter_type const& mu, model_solution_type const& solution, element_type const& z) override
    {
        ...
        auto geimexpr = this->expr( mu, solution );
        auto residual_expr = geimexpr - idv(z);
    ...
    boost::tuple<double,node_type> computeMaximumOfExpression( parameter_type const& mu, model_solution_type const& solution ) override
    {
...
#if 1
        if ( !this->M_computeExpansionOfExpression || ( this->M_normUsedForResidual == super::ResidualNormType::LinftyVec ) )
            this->M_internalModelFeFunc->on(_range=this->functionSpace()->template rangeElements<0>(),_expr=geimexpr );
        ...
        {
            if( this->M_computeExpansionOfExpression )
                boost::tie( max, node ) = normLinf( _range=this->functionSpace()->template rangeElements<0>(), _pset=_Q<0>(), _expr= geimexpr );
            else
        ...
        {
            if( this->M_computeExpansionOfExpression )
                max = normL2( _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
            else
        ...
        if( norm_used == "Linfty" )
        {
            ...
            {
                auto exprmax = normLinf( _range=this->functionSpace()->template rangeElements<0>(), _pset=_Q<0>(), _expr= geimexpr );
    ...
    void fillInterpolationMatrixFirstTime(  ) override
    {
        ...
        CHECK( M_ctxFeBasisGeim->nPoints() == 1 );

        if( this->M_computeExpansionOfExpression )
        {
            M_mu = M_mu_sampling->at(0);
            auto geimexpr = this->expr( M_mu, M_geimFeSpaceDb.solution(0) );

            auto expression_evaluated = evaluateFromContext( _context=*M_ctxFeModelSolution, _expr=geimexpr );
            double eval = expression_evaluated(0);
            auto expression_q = geimexpr / eval ; // normalization
            auto q_evaluated = evaluateFromContext( _context=*M_ctxFeModelSolution, _expr=expression_q );
            eval = q_evaluated(0);
            CHECK( math::abs(eval-1) < 1e-10 )
                << "q[0](t[0])) != 1 " << "q[0] = " << eval << " t[0] = \n "<<M_ctxFeBasisGeim->node(0)<<" \n" ;
        }
        else
        {
            auto geimexpr = this->expr( M_mu );//, *M_u );
            auto projected_g = vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(),_expr=geimexpr );

            auto projected_g_evaluated = evaluateFromContext( _context=*M_ctxFeBasisGeim, _expr=idv( projected_g ) );
            double eval = projected_g_evaluated(0);
            projected_g.scale( 1./eval );
            auto q_evaluated = evaluateFromContext( _context=*M_ctxFeBasisGeim, _expr=idv( projected_g ) );
            eval = q_evaluated(0);
            CHECK( math::abs(eval-1) < 1e-10 )
                << "q[0](t[0])) != 1 " << "q[0] = " << eval << " t[0] = \n "<<M_ctxFeBasisGeim->node(0)<<" \n" ;
        }
    ...
    //build geim m^th geim basis function
    //m : index of the geim basis
    //q : vector of projected on functionspace geim basis functions
    boost::any buildBasisFunction( int m ) override
    {
        //rank of the current processor
        int proc_number = this->worldComm().globalRank();

        int npoints = M_ctxFeBasisGeim->nPoints();
        ...
        if( this->M_computeExpansionOfExpression )
        {
            ...
            auto geimexpr = this->expr( M_mu, M_geimFeSpaceDb.solution(m) );
            auto residual = geimexpr - idv( M_geimFeSpaceDb.z(m) );
        ...
        {
            //in that case we have already the geim basis functions
            return M_geimFeSpaceDb.q(m);
        }
    ...
    element_type Residual( int m, element_type const& z) override
    {
        if( this->M_computeExpansionOfExpression )
        {
            ...
            //the interpolation point
            interpolation_point.clear();
            proc_having_the_point = M_ctxFeBasisGeim->processorHavingPoint( m );
            if( proc_number == proc_having_the_point )
            {
                auto basis = M_ctxFeBasisGeim->at( m );
                interpolation_point.addCtx( basis , proc_number );
            }
            vector_type expression_evaluated;
            element_type res;
            if( m == 0 )
            {
                expression_evaluated = evaluateFromContext( _context=interpolation_point , _expr=geimexpr );
                double eval = expression_evaluated( 0 );
                res = vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr/eval );
            }
            else
            {
                expression_evaluated = evaluateFromContext( _context=interpolation_point , _expr=residual );
        ...
        {
            auto residual = vf::project(_space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=idv(M_geimFeSpaceDb.g(m)) - idv( z ) );
            auto sig = M_sig[m];
    ...
    void fillInterpolationMatrix() override
    {
        ...
        int size = M_ctxFeBasisGeim->nPoints();
        M_B.conservativeResize( size , size );

        DVLOG( 2 ) << "solution.size() : "<<M_geimFeSpaceDb.solutions().size();
        DVLOG( 2 ) << "q.size() : "<< M_geimFeSpaceDb.q().size();

        auto geimexpr = this->expr( M_mu );//, *M_u );

        auto element_zero = project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=cst(0) );
        auto example_of_q = ( geimexpr - idv(element_zero) )/1.0;
        ...
        for( int __j = 0; __j < size; ++__j )
        {
            ...
            if( this->M_computeExpansionOfExpression )
            {
                ...
                if (!boost::any_cast<expression_type>(&any_q))
                    throw std::logic_error( "[GEIM::fillInterpolationMatrix] not cast possible for geim basis function expression" );
                auto q = boost::any_cast<expression_type>( any_q );
                M_B.col( __j ) = evaluateFromContext( _context=*M_ctxFeBasisGeim, _expr=q );
            }
            else
            {
                if (!boost::any_cast<element_type>(&any_q))
                    throw std::logic_error( "[GEIM::fillInterpolationMatrix] not cast possible for geim basis function projected in function space" );
                auto q = boost::any_cast<element_type>( any_q );
                M_B.col( __j ) = q.evaluate( *M_ctxFeBasisGeim );
            }
        }
    ...
    sampling_ptrtype createSubTrainset( sampling_ptrtype const& trainset, int method ) override
    {
        ...
        if( this->worldComm().isMasterRank() )
        {
            std::cout << "[geim::createSubTrainset] error indicator min = " << *min << " reached at mu = " << trainset->at(min_idx) << std::endl;
            std::cout << "[geim::createSubTrainset] error indicator max = " << *max << " reached at mu = " << trainset->at(max_idx) << std::endl;
            std::cout << "[geim::createSubTrainset] error indicator tol(mean) = " << tol << std::endl;
    ...
    //Let g the expression that we want to have an geim expasion
    ...
    double expressionL2Norm( model_solution_type const& T , parameter_type const& mu ) const override
    {
        ...
        return normL2( _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
    }

    //Let geim the geim expansion of g
    //here is computed || g - geim ||_L2
    double diffL2Norm(  model_solution_type const& T , parameter_type const& mu , element_type const & geim_expansion ) const override
    {
        auto geimexpr = this->expr( mu, T );
        auto mesh = this->modelFunctionSpace()->mesh();
        auto difference = geimexpr - idv(geim_expansion);
    ...
    double projExpressionL2Norm( model_solution_type const& T , parameter_type const& mu ) const override
    {
        auto geimexpr = this->expr( mu, T );
        auto mesh = this->functionSpace()->mesh();
        auto pi_g = vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
        return normL2( _range=this->functionSpace()->template rangeElements<0>(), _expr=idv(pi_g) );
    }

    //here is computed || \pi_g - geim ||_L2
    double projDiffL2Norm( model_solution_type const& T , parameter_type const& mu , element_type const& geim_expansion ) const override
    {
        auto geimexpr = this->expr( mu, T );
        auto mesh = this->functionSpace()->mesh();
        auto pi_g = vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
        auto diff = pi_g - geim_expansion;
        return normL2( _range=this->functionSpace()->template rangeElements<0>(), _expr=idv(diff) );
    }

    //here is computed || \pi_g - geim ||_Linf
    double projDiffLinfNorm( model_solution_type const& T , parameter_type const& mu , element_type const& geim_expansion ) const override
    {
        auto geimexpr = this->expr( mu, T );
        auto mesh = this->modelFunctionSpace()->mesh();
        auto pi_g = vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
    ...
    double interpolationError(model_solution_type const& T , parameter_type const& mu ) const override
    {
        ...
        auto mesh = this->modelFunctionSpace()->mesh();
        auto pi_g = vf::project( _space=this->functionSpace(), _range=this->functionSpace()->template rangeElements<0>(), _expr=geimexpr );
        auto difference = geimexpr - idv(pi_g);
        return normL2( _range=this->functionSpace()->template rangeElements<0>(), _expr=difference );
    }
    ...
    bool offlineStep() const override { return M_geim->offlineStep(); }
    bool adaptationSER() const override { return M_geim->adaptationSER(); }
    void setAdaptationSER(bool b) override {M_geim->setAdaptationSER(b);}
    bool rbCorrection() const override { return M_geim->rbCorrection(); }
    void setRbCorrection(bool b) override {M_geim->setRbCorrection(b);}

    void offline() override { M_geim->offline(); }
    void setRestart(bool b) override { M_geim->setRestart(b);}
    ...
    void setTrainSet( sampling_ptrtype tset ) override { M_geim->setTrainSet( tset ); }
    element_type interpolant( parameter_type const& mu ) override
    {
        auto beta = this->beta( mu );
        return expansion( M_geimFeSpaceDb.q(), this->beta( mu ) , M_M_max);
    }
    element_type interpolant( parameter_type const& mu , model_solution_type const & solution , int M) override
    {
        return expansion( M_geimFeSpaceDb.q(), this->beta( mu , solution , M) , M );
    }
    ...
    void printRbIterationsSER( int M ) const override
    {
        ...
        {
            std::ofstream file;
            std::string filename = "GEIM-" + super::name() + "-rb_online_greedy_summary.dat";
    ...
    std::vector<element_type> const& q() const override { return M_geimFeSpaceDb.q(); }


    void studyConvergence( parameter_type const & mu , model_solution_type & solution, std::vector< std::string > all_file_name ) const override { return M_geim->studyConvergence( mu , solution , all_file_name ) ; }
    boost::tuple<double,element_type> interpolationErrorEstimation( parameter_type const & mu , model_solution_type const& solution, int M ) const override { return M_geim->interpolationErrorEstimation(mu , solution, M) ; }
    double errorEstimationLinf( parameter_type const & mu, model_solution_type const& solution , int M ) const override { return M_geim->errorEstimationLinf(mu, solution, M) ; }
    ...
    size_type mMax( bool & error) const override
    {
        ...
        //if the geim expansion contains less terms that expected by the user then there is no error.
        //But if there is already enough basis functions then we return M_M_max-1
        //to deal with error estimation
        if( (user_max+1) > M_geimFeSpaceDb.maxQ() )
        {
            max = M_M_max;
            //in that case if the geim expansion is finished there is no error associated
    ...
    {
        M_M_max = m;
        M_geimFeSpaceDb.setMax( max_q, max_g, max_z, max_solution );
    }

    int maxQ() const override
    {
        return M_geimFeSpaceDb.maxQ();
    }
    int maxG() const override
    {
        return M_geimFeSpaceDb.maxG();
    }
    int maxZ() const override
    {
        return M_geimFeSpaceDb.maxZ();
    }
    int maxSolution() const override
    {
        return M_geimFeSpaceDb.maxSolution();
    }
    ...
    void serialize(Archive & __ar, const unsigned int __version )
    {
        ...
        // save t
        __ar & BOOST_SERIALIZATION_NVP( M_size );
        ...
        if ( Archive::is_loading::value )
        {
            bool hasCtxGeoGeim = false;
            __ar & BOOST_SERIALIZATION_NVP( hasCtxGeoGeim );
            // std::cout << "GEIM load geoctx " << hasCtxGeoGeim <<"\n";
            if ( hasCtxGeoEim )
            {
                ...
                M_ctxGeoEim.reset( new geometricspace_context_type( geospace ) );
                __ar & BOOST_SERIALIZATION_NVP( *M_ctxGeoGeim );
            }
        }
        else
        {
            bool hasCtxGeoGeim = ( M_ctxGeoGeim )? true : false;
...
private:
    GEIMFunctionFeSpaceDb<functionspace_type,model_type> M_geimFeSpaceDb;
    ...
    std::vector< double > M_evaluation_vector; // we need to store this to be able to compute expression of geim basis functions and the user can use them in integrals
    int M_M_max;
    std::vector<node_type> M_t;
    geometricspace_context_ptrtype M_ctxGeoGeim;
    boost::shared_ptr<context_type> M_ctxFeBasisGeim;
...
struct compute_geim_return
{
    ...
    typedef GEIMFunction<model_type, space_type, expr_type, subspaceid, subspaceid2 > type;
    typedef boost::shared_ptr<type> ptrtype;
};
}

BOOST_PARAMETER_FUNCTION(
    ( typename Feel::detail::compute_geim_return<Args>::ptrtype ), // 1. return type
    geim,                        // 2. name of the function template
...
#endif
    typedef typename Feel::detail::compute_geim_return<Args>::type geim_type;
    typedef typename Feel::detail::compute_geim_return<Args>::ptrtype geim_ptrtype;
    auto geimFunc = boost::make_shared<geim_type>( model, space, element, element2, parameter, expr, sampling, name, filename, directory );
    if (geim_type::model_use_nosolve )
        geimFunc->attachModel( model );
    return geimFunc;
} // geim


template<typename ModelType>
struct GEimFunctionNoSolve : public GEimFunctionNoSolveBase
{
    typedef typename ModelType::functionspace_type functionspace_type;
    ...
    GEimFunctionNoSolve( model_ptrtype const& model )
        :
        M_model( model )
        {
...
template<typename ModelType>
boost::shared_ptr<GEimFunctionNoSolve<ModelType>>
eim_no_solve( boost::shared_ptr<ModelType> model )
{
    return boost::shared_ptr<GEimFunctionNoSolve<ModelType>>( new GEimFunctionNoSolve<ModelType>( model ) );
}

template< typename ExprType , typename GeimType >
Expr< vf_div< Expr< vf_sub< ExprType, Expr< OpId< typename GeimType::element_type , __VALUE> > > > ,  Cst<double>  > >
geimBasisExpression(int m, ExprType const& expr, GeimType const& geim)
{
    auto any_type = geim->buildBasisFunction( m );
    typedef typename GeimType::element_type element_type;
    ...
    {
        throw std::logic_error( "[qExpression] not cast possible for geim basis function expression" );
    }
