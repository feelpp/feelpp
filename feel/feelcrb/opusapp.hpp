  ...
  public:
    ...
    void init()
    {
        try
        {
            ...
            this->changeRepository( boost::format( "%1%/%2%/" ) % this->about().appName() % results_repo_name );
    ...
    void run()
    {
        ...
        for ( auto mu : *Sampling )
        {
            ...
            for ( int i = 0; i < sizemax - 1; i++ )
                mu_str << std::scientific << std::setprecision( 5 ) << mu[i] << ",";
        ...
        if ( this->vm().count( "crb.minimization-func" ) && proc_number == Environment::worldComm().masterRank() )
        {
            if ( M_mode == CRBModelMode::PFEM || compute_fem )
            {
                ...
                for ( int i = 0; i < size - 1; i++ )
                    file_summary_of_simulations << ( min_mu["fem"] )[i] << " , ";
            ...
            for ( int i = 0; i < size - 1; i++ )
                file_summary_of_simulations << ( min_mu["rb"] )[i] << " , ";
