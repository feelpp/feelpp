...
public:
    ...
    /**
     * resize Online data prior to online computations if many are done
     */
     void resizeOnline( size_type N,
...
private:
    ...
    // Online data
    mutable matrixN_type J;
    mutable vectorN_type R;
    mutable matrixN_type A, Adu;
    mutable matrixN_type M, Mdu;
    mutable vectorN_type F, Fdu;
    mutable vectorN_type L, Ldu;
    mutable matrixN_type Aprdu;
    mutable matrixN_type Mprdu;

    mutable vectorN_type previous_uN;
    mutable std::vector<vectorN_type> uN;
    mutable std::vector<vectorN_type> uNdu;
    mutable std::vector<vectorN_type> uNold;
    mutable std::vector<vectorN_type> uNduold;

    //std::vector<vectorN_type> uN, uNdu, uNold, uNduold;

    mutable beta_vector_type betaJqm;
    mutable std::vector< beta_vector_type > betaRqm;
    mutable std::vector<beta_vector_type> betaFqm,betaLqm;
    mutable beta_vector_type betaAqm;
    mutable beta_vector_type betaMqm;
    mutable beta_vector_type betaMFqm;
};
...
CRB<TruthModelType>::offline()
{
    ...
    vector_ptrtype u2( M_backend->newVector( M_model->functionSpace() ) );
    u1->setConstant(100);
    u2->setConstant(100);
    u1->close();
    u2->close();
    element_ptrtype eu1( new element_type( M_model->functionSpace() ) );
    element_ptrtype eu2( new element_type( M_model->functionSpace() ) );
    *eu1= *u1;
    *eu2 = *u2;
    //eu1->setConstant(100);
    //eu2->setConstant(100);
    ...
    {
        mu = M_Xi->at( no_residual_index );
        int size = mu.size();
        if( proc_number == 0 )
        {
            std::cout << "  -- restart with mu = [ ";
            for ( int i=0; i<size-1; i++ ) std::cout<<mu( i )<<" ";
            std::cout<<mu( size-1 )<<" ]"<<std::endl;
        }
    }
        ...
        if ( M_model->isSteady() && ! M_use_newton )
        {
            ...
            if( solve_dual_problem )
            {
                ...
                *Rhs = *F[M_output_index];
                ...
                if( proc_number == 0 ) std::cout<<"dual solved "<<std::endl;
            }
...
CRB<TruthModelType>::compareResidualsForTransientProblems( int N, parameter_type const& mu, std::vector<element_ptrtype> const & Un, std::vector<element_ptrtype> const & Unold, std::vector<element_ptrtype> const& Undu, std::vector<element_ptrtype> const & Unduold, std::vector< std::vector<double> > const& primal_residual_coeffs,  std::vector < std::vector<double> > const& dual_residual_coeffs ) const
{
    ...
    for ( bdf_primal->start() ; !bdf_primal->isFinished(); bdf_primal->next() )
    {
        ...
        boost::tie( M, A, F, boost::tuples::ignore ) = M_model->update( mu , bdf_primal->time() );
...
CRB<TruthModelType>::check( size_type N ) const
{
    ...
    parameter_type mu = M_WNmu->at( 0 );
    this->resizeOnline( N, uN, uNdu , uNold, uNduold );
...
CRB<TruthModelType>::newton(  size_type N, parameter_type const& mu , vectorN_type & uN  , double& condition_number, double& output) const
{
    ...
    //compute output
    boost::tie( boost::tuples::ignore, boost::tuples::ignore, betaFqm, boost::tuples::ignore ) =
        M_model->computeBetaQm( this->expansion( uN , N , M_WN  ), mu , 0 );
    L.setZero( N );
    for ( size_type q = 0; q < M_model->Ql( M_output_index ); ++q )
    {
        int M = M_model->mMaxF(M_output_index,q);
        for(int m=0; m < M; m++)
...
template<typename TruthModelType>
void
CRB<TruthModelType>::resizeOnline( size_type N,
                                   std::vector< vectorN_type > & uN,
                                   std::vector< vectorN_type > & uNdu,
                                   std::vector<vectorN_type> & uNold,
                                   std::vector<vectorN_type> & uNduold,
                                   int K ) const
{
    ...
    previous_uN.resize( N );
    if( M_use_newton )
    {
        J.resize( ( int )N, ( int )N ) ;
        R.resize ( ( int )N );
        L.resize ( ( int )N );
    }
    else
    {
        ...
        if( solve_dual_problem )
        {
            Adu.resize( ( int )N, ( int )N ) ;
            Mdu.resize( ( int )N, ( int )N ) ;
            Aprdu.resize( ( int )N, ( int )N ) ;
            Mprdu.resize( ( int )N, ( int )N ) ;
            Fdu.resize ( ( int )N );
            Ldu.resize ( ( int )N );
        }
    }
...
CRB<TruthModelType>::lb( size_type N, parameter_type const& mu, std::vector< vectorN_type > & uN,
                         std::vector< vectorN_type > & uNdu,  std::vector<vectorN_type> & uNold,
                         std::vector<vectorN_type> & uNduold,int K  ) const
{
        ...
        for ( int __m1=0 ; __m1< M_model->mMaxM(__q1); ++__m1 )
        {
            ...
            for ( int __q2=0 ; __q2<__QOutput; ++__q2 )
            {
                for ( int __m2=0 ; __m2< M_model->mMaxF(M_output_index,__q2); ++__m2 )
                {
                    ...
                    __Cmf_du +=   m_q1 * f_q2 * M_Cmf_du[__q1][__m1][__q2][__m2].head( __N ).dot( Unduini );
                }//m2
            }//q2
...
CRB<TruthModelType>::steadyPrimalResidual( int Ncur,parameter_type const& mu, vectorN_type const& Un, double time ) const
{
    ...
    beta_vector_type betaAqm;
    std::vector<beta_vector_type> betaFqm, betaLqm;
    boost::tie( boost::tuples::ignore, betaAqm, betaFqm, boost::tuples::ignore ) = M_model->computeBetaQm( mu , time );
...
CRB<TruthModelType>::steadyDualResidual( int Ncur,parameter_type const& mu, vectorN_type const& Undu, double time ) const
{
    ...
    beta_vector_type betaMFqm;
    std::vector<beta_vector_type> betaFqm, betaLqm;
    boost::tie( boost::tuples::ignore, betaAqm, betaFqm, betaMFqm ) = M_model->computeBetaQm( mu , time );
...
CRB<TruthModelType>::expansion( parameter_type const& mu , int N  )
{
    ...
    this->resizeOnline( Nwn, uN, uNdu , uNold, uNduold );
    auto o = lb( Nwn, mu, uN, uNdu , uNold, uNduold );
    int size = uN.size();
...
CRB<TruthModelType>::run( const double * X, unsigned long N, double * Y, unsigned long P, mpl::bool_<false> )
{
    ...
    DCHECK( P == 2 ) << "invalid number of outputs " << P << " (should be 2)" ;
    auto o = lb( Nwn, mu, uN, uNdu , uNold, uNduold );
    auto e = delta( Nwn, mu, uN, uNdu , uNold, uNduold );
    Y[0]  = o.template get<0>();
    Y[1]  = e.template get<0>();
