...
CRB<TruthModelType>::offlineWithErrorEstimation(mpl::bool_<true>)
{
    ...
    while ( maxerror > M_tolerance && M_N <= M_iter_max )
    {
        ...
        */
        ...
        }//end of loop over the number of mode per mu and end of pod
...
CRB<TruthModelType>::lb( size_type N, parameter_type const& mu, vectorN_type & uN, vectorN_type & uNdu, mpl::bool_<true>   ) const
{
    ...
    boost::tie( theta_mq, theta_aq, theta_fq ) = M_model->computeThetaq( mu );
    ...
    vectorN_type F1 ( (int)N );
    F1.setZero(N,N);
    for(int q = 0;q < M_model->Ql(0); ++q)
    {
        F1 += theta_fq[0][q]*M_Fq_pr[q].block(0,0,N,N);
    }
