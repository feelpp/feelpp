    ...
    public OperatorLinear<SpaceType, typename detailOpLagP1::SpaceToLagrangeP1Space<SpaceType>::image_space_type>
{
  ...
  public:
              ...
              mpl::bool_<false> ) const
    {
        ...
        {
            size_type searchdofid = this->domainSpace()->dof()->localToGlobal( it->id(), i, 0 ).index();
...
template <typename space_type>
void OperatorLagrangeP1<space_type>::buildReferenceMesh( bool rebuild, std::string pathMeshLagP1, std::string prefix )
{
...
template <typename space_type>
void OperatorLagrangeP1<space_type>::buildLagrangeP1Mesh( bool parallelBuild )
{
...
#if !defined( NDEBUG )
    if ( it != en && !M_gmc )
        M_gmc = gmc_ptrtype( new gmc_type( this->domainSpace()->mesh()->gm(), *it, M_gmpc ) );
#endif
        ...
        for ( int cptEltp2m = 0; itl != enl; ++itl, ++elid, ++cptEltp2m )
        {
            ...
            for ( int p = 0; p < image_mesh_type::element_type::numVertices; ++p )
            {
                ...
                size_type ptid = boost::get<0>( this->domainSpace()->dof()->localToGlobal( it->id(),
                                                                                           localptid_dof, 0 ) );
                FEELPP_ASSERT( ptid < this->domainSpace()->nLocalDof() / domain_space_type::nComponents )
                ( ptid )( this->domainSpace()->nLocalDof() / domain_space_type::nComponents ).warn( "invalid domain dof index" );
                ...
                ( boost::get<0>( this->domainSpace()->dof()->dofPoint( ptid ) ) )( p )( elt.point( p ).node() )( elid )( itl->id() )( localptid )( localptid_dof )( ptid ).warn( "[after] inconsistent point coordinates" );

                FEELPP_ASSERT( ublas::norm_2( elt.point( p ).node() - ublas::column( elt.G(), p ) ) < 1e-10 )
                ( p )( elt.point( p ).node() )( elid )( elt.G() ).warn( "[after] inconsistent point coordinates" );
#endif
            }
...
//FEELPP_ASSERT( ublas::norm_inf( elt.G()- itl->G() ) < 1e-10 )( itl->G() )( elt.G() ).warn( "local: not same element" );
    ...
    if ( doParallelBuild )
    {
        ...
        std::map<int, std::vector<boost::tuple<std::vector<boost::tuple<std::vector<uint16_type>, size_type>>,
                                               std::vector<boost::tuple<size_type, ublas::vector<double>>>>>>
            MAPresultRecvData;
...
template <typename space_type>
void OperatorLagrangeP1<space_type>::buildOperator()
{
...
template <typename space_type>
void OperatorLagrangeP1<space_type>::check() const
{
#if !defined( NDEBUG )
    for ( size_type i = 0; i < this->domainSpace()->nLocalDof() / domain_space_type::nComponents; ++i )
    {
                                          ...
                                          mpl::bool_<true> ) const
{
         ...
         it->edgePermutation( M_pset.pointToEntity( localptid ).second ) == edge_permutation_type::REVERSE_PERMUTATION )
    {
        ...
        ( l )( domain_fe_type::nDofPerEdge )( localptid )( M_pset.pointToEntity( localptid ).second )( domain_mesh_type::element_type::numVertices * domain_fe_type::nDofPerVertex +
                                                                                                       edge_id * domain_fe_type::nDofPerEdge )
            .error( "invalid value" );
        ...
        ( l )( localptid )( localptid_dof )( domain_fe_type::nLocalDof )( M_pset.pointToEntity( localptid ).second )( domain_mesh_type::element_type::numVertices * domain_fe_type::nDofPerVertex +
                                                                                                                      edge_id * domain_fe_type::nDofPerEdge )
            .error( "invalid value" );
    }
...
BOOST_PARAMETER_FUNCTION(
    ...
    ( optional( backend, *, Backend<typename compute_opLagrangeP1_return<Args>::space_type::value_type>::build( soption( _name = "backend" ) ) )( path, *(boost::is_convertible<mpl::_, std::string>), std::string( "." ) )( prefix, *(boost::is_convertible<mpl::_, std::string>), std::string( "" ) )( rebuild, *(boost::is_integral<mpl::_>), 1 )( parallel, *(boost::is_integral<mpl::_>), 1 ) ) // optionnal
    )
{
