                                        ...
                                        const unsigned int n_sbdmns )
{
    ...
    M_vtxdist.resize ( n_procs+1 );
    std::fill ( M_vtxdist.begin(), M_vtxdist.end(), 0 );
    M_tpwgts.resize  ( M_nparts );
    std::fill ( M_tpwgts.begin(),  M_tpwgts.end(),  1./M_nparts );
    M_ubvec.resize   ( M_ncon );
    std::fill ( M_ubvec.begin(),   M_ubvec.end(),   1. );
    M_part.resize    ( n_active_elem );
    std::fill ( M_part.begin(),    M_part.end(),    0 );
    M_options.resize ( 5 );
    ...
    // contiguous range.
    M_forward_map.resize ( n_elem );
    std::fill ( M_forward_map.begin(),
                M_forward_map.end(),
