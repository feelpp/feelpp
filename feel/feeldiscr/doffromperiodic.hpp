        ...
        mpl::bool_<true> )
{
    ...
    for ( uint16_type iVeFa = 0; iVeFa < face_type::numVertices; ++iVeFa )
    {
        ...
        for ( uint16_type l = 0; l < fe_type::nDofPerVertex; ++l )
        {
            ...
            bool inserted = M_doftable->insertDof( __elt.id(), lid, iVeEl, std::make_tuple( 0, 0, gDof ), 0, next_free_dof, 1, true, 0, __elt.point( iVeEl ).marker()  );
        ...
        mpl::int_<2> )
{
    ...
    for ( uint16_type l = 0; l < fe_type::nDofPerEdge; ++l )
    {
        ...
        size_type dof_id = next_free_dof;
        // next_free_dof might be incremented if a new dof is created
        bool inserted = M_doftable->insertDof( __elt.id(), lid, iFaEl, std::make_tuple( 1, 0, gDof ), 0, next_free_dof, 1, true, 0, __elt.edge( iFaEl ).marker() );
        ...
        mpl::int_<3> )
{
    ...
    for ( uint16_type iEdFa = 0; iEdFa < face_type::numEdges; ++iEdFa )
    {
        ...
        for ( uint16_type l = 0; l < fe_type::nDofPerEdge; ++l )
        {
            ...
            size_type dof_id = next_free_dof;
            // next_free_dof might be incremented if a new dof is created
            bool inserted = M_doftable->insertDof( __elt.id(), lid, l, std::make_tuple( 1, 0, gDof ), 0, next_free_dof, 1, true, 0, __elt.edge( l ).marker() );
