                         ...
                         fusion::pair<typename ThePair2Type::first_type, typename ThePair2Type::second_type>>
{
    ...
    typedef fusion::map<fusion::pair<key1_type, typename ThePair1Type::second_type>,
                        fusion::pair<key2_type, typename ThePair2Type::second_type>>
        super_type;
...
struct stencilQuadSet : public stencilQuadSetBase,
  ...
  public:
    ...
    struct rangeiteratorType
    {
        ...
        typedef typename mpl::if_<hasnotfindrange_type,
                                  ...
                                  mpl::identity<resultfindrange_type>>::type::type type;
    };
...
class StencilManagerImpl : public std::map<boost::tuple<boost::weak_ptr<FunctionSpaceBase>,
                                                        boost::weak_ptr<FunctionSpaceBase>,
                                                        size_type,
                                                        std::vector<size_type>,
                                                        bool>,
                                           boost::shared_ptr<GraphCSR>>,
...
BOOST_PARAMETER_FUNCTION(
    ...
    ( required                                                                                                                                                                    // 4. one required parameter, and
      ( test, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>))( trial, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>)) )( optional //    four optional parameters, with defaults
                                                                                                                                                                         ( pattern, *(boost::is_integral<mpl::_>), Pattern::COUPLED )( pattern_block, *, default_block_pattern )( diag_is_nonzero, *(boost::is_integral<mpl::_>), false )( collect_garbage, *(boost::is_integral<mpl::_>), true )( close, *(boost::is_integral<mpl::_>), true )( range, *(boost::is_convertible<mpl::_, stencilRangeMapTypeBase>), stencilRangeMap0Type() )( range_extended, *(boost::is_convertible<mpl::_, stencilRangeMapTypeBase>), stencilRangeMap0Type() )( quad, *(boost::is_convertible<mpl::_, stencilQuadSetBase>), stencilQuadSet<>() ) ) )
{
    if ( collect_garbage )
    {
...
void Stencil<X1, X2, RangeItTestType, RangeExtendedItType, QuadSetType>::mergeGraph( int row, int col, graph_ptrtype g )
{
    boost::timer tim;
...
template <typename X1, typename X2, typename RangeItTestType, typename RangeExtendedItType, typename QuadSetType>
void Stencil<X1, X2, RangeItTestType, RangeExtendedItType, QuadSetType>::mergeGraphMPI( size_type test_index, size_type trial_index,
                                                                                        DataMap const& mapOnTest, DataMap const& mapOnTrial,
                                                                                        graph_ptrtype g )
{
    ...
    for ( ; it != en; ++it )
    {
        ...
        if ( it->second.get<0>() != g->worldComm().globalRank() )
        {
            ...
            const size_type realrowStart = this->testSpace()->dof()->firstDofGlobalCluster( proc ) + this->testSpace()->nLocalDofWithoutGhostOnProcStart( proc, test_index );
            theglobalrow = realrowStart + ( it->first - mapOnTest.firstDofGlobalCluster( proc ) );
            thelocalrow = this->testSpace()->nLocalDofWithoutGhost() + ( this->testSpace()->nLocalDofWithGhostOnProcStart( myrank, test_index ) - locdofStart ) + ( it->second.get<1>() - mapOnTest.nLocalDofWithoutGhost() );
        }
        ...
        if ( !row2_entries.empty() )
        {
            for ( auto itcol = row2_entries.begin(), encol = row2_entries.end(); itcol != encol; ++itcol )
            {
                ...
                else
                {
                    ...
                    const size_type realcolStart = this->trialSpace()->dof()->firstDofGlobalCluster( realproc ) + this->trialSpace()->nLocalDofWithoutGhostOnProcStart( realproc, trial_index );
...
Stencil<X1, X2, RangeItTestType, RangeExtendedItType, QuadSetType>::computeGraphInCaseOfInterpolate( size_type hints, mpl::bool_<true> )
{
    ...
    typedef typename mpl::if_<mpl::bool_<test_mesh_type::element_type::is_simplex>,
                              ...
                              mpl::identity<typename _Q<order_used_type::value>::template applyIMGeneral<test_mesh_type::element_type::nDim, typename test_mesh_type::value_type, Hypercube>::type>>::type::type theim_type;
