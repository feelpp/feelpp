...
enum class interpolation_operand_type
{
    ID = 0,
    GRADIENT,
    CURL,
...
OperatorInterpolation<DomainSpaceType, ImageSpaceType, IteratorRange, InterpType>::
    ...
    : super( domainspace, imagespace, backend, false ),
      M_listRange(),
      M_WorldCommFusion( ( ddmethod || ( this->domainSpace()->worldComm() == this->dualImageSpace()->worldComm() ) ) ? this->domainSpace()->worldComm() : this->domainSpace()->worldComm() + this->dualImageSpace()->worldComm() ),
...
template <typename DomainSpaceType, typename ImageSpaceType, typename IteratorRange, typename InterpType>
void OperatorInterpolation<DomainSpaceType, ImageSpaceType, IteratorRange, InterpType>::update()
{
...
struct PrecomputeDomainBasisFunction
{
    ...
    static const size_type context2 = ( mpl::or_<is_hdiv_conforming<fe_type>, is_hcurl_conforming<fe_type>>::type::value ) ? expression_type::context | vm::JACOBIAN | vm::KB | vm::TANGENT | vm::NORMAL : expression_type::context;
...
template <typename DomainSpaceType, typename ImageSpaceType, typename IteratorRange, typename InterpType>
void OperatorInterpolation<DomainSpaceType, ImageSpaceType, IteratorRange, InterpType>::updateSameMesh()
{
                                ...
                                for ( uint16_type jloc = 0; jloc < domain_basis_type::nLocalDof; ++jloc )
                                {
                                    ...
                                    for ( int cdomain = 0; cdomain < nCompDomain; ++cdomain )
                                    {
                                        if ( domain_basis_type::is_product &&
                                             ...
                                             ( M_interptype.interpolationOperand() == interpolation_operand_type::ID ) && cdomain != comp ) continue;
...
template <typename DomainSpaceType, typename ImageSpaceType, typename IteratorRange, typename InterpType>
void OperatorInterpolation<DomainSpaceType, ImageSpaceType, IteratorRange, InterpType>::updateNoRelationMesh()
{
                    ...
                    if ( !dof_done[gdof] )
                    {
                        ...
                        for ( ; itanal != itanal_end; ++itanal )
                        {
                            ...
                            for ( uint16_type jloc = 0; jloc < domain_basis_type::nLocalDof; ++jloc )
                            {
                                ...
                                value_type v = MlocEval( domain_basis_type::nComponents1 * jloc + comp * domain_basis_type::nComponents1 * domain_basis_type::nLocalDof + comp,
                                                         0 );
                                row.template get<2>().insert( domaindof->mapGlobalProcessToGlobalCluster()[j] );
...
template <typename DomainSpaceType, typename ImageSpaceType, typename IteratorRange, typename InterpType>
void OperatorInterpolation<DomainSpaceType, ImageSpaceType, IteratorRange, InterpType>::updateNoRelationMeshMPI()
{
...
template <typename DomainSpaceType, typename ImageSpaceType, typename IteratorRange, typename InterpType>
void OperatorInterpolation<DomainSpaceType, ImageSpaceType, IteratorRange, InterpType>::updateNoRelationMeshMPI_run( bool buildNonZeroMatrix )
{
    ...
    for ( size_type k = 0; k < memory_valueInMatrix.size(); ++k )
        memory_valueInMatrix[k].clear();
    ...
    if ( InterpType::isConforming() )
        nMPIsearch = this->domainSpace()->mesh()->worldComm().localSize();
    else if ( this->domainSpace()->mesh()->worldComm().localSize() < nMPIsearch )
        nMPIsearch = this->domainSpace()->mesh()->worldComm().localSize();
    ...
    while ( !FinishMPIsearch )
    {
        ...
        if ( this->interpolationType().searchWithCommunication() )
        {
            ...
            nbLocalisationFail = memory_localisationFail.size();
        ...
        if ( counterMPIsearch < nMPIsearch && nbLocalisationFail > 0 )
            ++counterMPIsearch;
        else
            FinishMPIsearch = true;
    }
    ...
    if ( doExtrapolationAtStart && nbLocalisationFail > 0 )
    {
        ...
        while ( !FinishMPIsearchExtrap )
        {
            ...
            if ( counterMPIsearchExtrap < nMPIsearchExtrap )
                ++counterMPIsearchExtrap;
            else
                FinishMPIsearchExtrap = true;
        } // while(!FinishMPIsearchExtrap)
        ...
        for ( size_type cpt_gdof = 0; it_extrap != en_extrap; ++it_extrap, ++cpt_gdof )
        {
            ...
            for ( ; it_proc != en_proc; ++it_proc )
            {
                ...
                // COMPUTE DISTANCE
                double normDist = 0;
                for ( int q = 0; q < image_mesh_type::nRealDim; ++q )
                    normDist += std::pow( bary( q ) - pointExtrap( q ), 2 );
                //std::cout << std::sqrt(normDist) << " "<< bary.size() << " " << pointExtrap.size() << std::endl;
                if ( std::sqrt( normDist ) < distMin )
                {
                    distMin = std::sqrt( normDist );
                    procExtrapoled = procCurrent;
                }
            }
        ...
        //------------------------------------------------------------
        {
            ...
            else // point found
            {
                ...
                if ( extrapolation_mode )
                {
                    ...
                    for ( int qi = 0; qi < verticesExtrapoled.size1(); ++qi )
                        bary( qi ) = 0; //important
        ...
        else
        {
                  ...
                  it_rankLocalization != en_rankLocalization; ++it_rankLocalization )
            {
                ...
                if ( proc_id_domain == rankLocalization && domainProcIsActive_fusion[this->worldCommFusion().globalRank()] ) // get request of proc
                {
                    ...
                    pointsRefIsFinded.resize( nDataRecv );
                    std::fill( pointsRefIsFinded.begin(), pointsRefIsFinded.end(), false );
                                                                                             ...
                                                                                             extrapolation_memory_type& dof_extrapolationData )
{
    ...
    std::map<rank_type, boost::tuple<std::vector<typename image_mesh_type::node_type>,
                                     ...
                                     std::vector<std::vector<int>>,
                                     std::vector<typename image_mesh_type::node_type>>>
        dataReRecv, dataReSend;
    ...
    for ( auto const& dataToTreatBase : dataToRecv )
    {
        ...
        pointsDofsGlobalClusterColFinded.resize( nDataRecv, std::vector<int>( 1, 0 ) );
        if ( extrapolation_mode )
            pointsBaryFinded.resize( nDataRecv );
        else
            pointsBaryFinded.clear();
    ...
    typedef typename boost::remove_const<
        typename boost::remove_reference<
            typename parameter::binding<Args,
                                        tag::range,
                                        typename OperatorInterpolation<domain_space_type, image_space_type>::range_iterator>::type>::type>::type iterator_base_range_type;
...
BOOST_PARAMETER_FUNCTION(
    ...
    ( required( domainSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>))( imageSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>)) )                                                                                  // required
    ( optional( range, *, elements( imageSpace->mesh() ) )( backend, *, Backend<typename compute_opInterpolation_return<Args>::domain_space_type::value_type>::build( soption( _name = "backend" ) ) )( type, *, InterpolationNonConforming() )( ddmethod, (bool), false ) ) // optional
    )
{
...
BOOST_PARAMETER_FUNCTION(
    ...
    tag,                                                                                                                                                                                                                                                                                                        // 3. namespace of tag types
    ( required( domainSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>))( imageSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>)) )                                                                                                                     // required
    ( optional( range, *, elements( imageSpace->mesh() ) )( backend, *, Backend<typename compute_opInterpolation_return<Args>::domain_space_type::value_type>::build( soption( _name = "backend" ) ) )( type, *, makeGradientInterpolation<nonconforming_t>( nonconforming_t() ) )( ddmethod, (bool), false ) ) // optional
    )
{
#if BOOST_VERSION < 105900
    Feel::detail::ignore_unused_variable_warning( args );
...
BOOST_PARAMETER_FUNCTION(
    ...
    tag,                                                                                                                                                                                                                                                                                                    // 3. namespace of tag types
    ( required( domainSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>))( imageSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>)) )                                                                                                                 // required
    ( optional( range, *, elements( imageSpace->mesh() ) )( backend, *, Backend<typename compute_opInterpolation_return<Args>::domain_space_type::value_type>::build( soption( _name = "backend" ) ) )( type, *, makeCurlInterpolation<nonconforming_t>( nonconforming_t() ) )( ddmethod, (bool), false ) ) // optional
    )
{
#if BOOST_VERSION < 105900
    Feel::detail::ignore_unused_variable_warning( args );
...
BOOST_PARAMETER_FUNCTION(
    ...
    tag,                                                                                                                                                                                                                                                                                                   // 3. namespace of tag types
    ( required( domainSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>))( imageSpace, *(boost::is_convertible<mpl::_, boost::shared_ptr<FunctionSpaceBase>>)) )                                                                                                                // required
    ( optional( range, *, elements( imageSpace->mesh() ) )( backend, *, Backend<typename compute_opInterpolation_return<Args>::domain_space_type::value_type>::build( soption( _name = "backend" ) ) )( type, *, makeDivInterpolation<nonconforming_t>( nonconforming_t() ) )( ddmethod, (bool), false ) ) // optional
    )
{
#if BOOST_VERSION < 105900
    Feel::detail::ignore_unused_variable_warning( args );
