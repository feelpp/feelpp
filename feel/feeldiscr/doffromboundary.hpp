...
class DofFromBoundary
{
  ...
  private:
    ...
    template <typename FaceIterator>
    void addVertexBoundaryDof( FaceIterator /*face_it*/, bool /*useConnection0*/, uint16_type& /*lc*/, mpl::bool_<false>, mpl::int_<1> ) {}
    template <typename FaceIterator>
    void addVertexBoundaryDof( FaceIterator /*face_it*/, bool /*useConnection0*/, uint16_type& /*lc*/, mpl::bool_<false>, mpl::int_<2> ) {}
    template <typename FaceIterator>
    void addVertexBoundaryDof( FaceIterator /*face_it*/, bool /*useConnection0*/, uint16_type& /*lc*/, mpl::bool_<false>, mpl::int_<3> ) {}
    template <typename FaceIterator>
    ...
    void addEdgeBoundaryDof( FaceIterator face_it, bool useConnection0, uint16_type& lc, mpl::bool_<true>, mpl::int_<3> )
    {
        ...
        for ( int c = 0; c < ncdof; ++c )
        {
            ...
            for ( uint16_type iEdFa = 0; iEdFa < face_type::numEdges; ++iEdFa )
            {
                ...
                FEELPP_ASSERT( iEdEl != invalid_uint16_type_value )
                    .error( "invalid local dof" );
