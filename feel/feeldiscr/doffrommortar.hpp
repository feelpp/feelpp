    ...
    void addEdgeDof( element_type const& elt, uint16_type processor, size_type& next_free_dof,
                     ref_shift_type& shifts, mpl::int_<1>, mpl::bool_<true> )
        {
            ...
            for ( uint16_type l = 0; l < mortar_fe_type::nDofPerEdge; ++l, ++lc )
            {
                ...
                {
                    M_doftable->insertDof( ie, lc, l, boost::make_tuple( 1, 0, gDof ), processor, next_free_dof, 1, false, global_shift, elt.marker() );
                }
            }
                     ...
                     ref_shift_type& shifts, mpl::int_<2>, mpl::bool_<true> )
        {
            ...
            for ( uint16_type i = 0; i < element_type::numEdges; ++i )
            {
                for ( uint16_type l = 0; l < mortar_fe_type::nDofPerEdge; ++l, ++lc )
                {
                    ...
                    M_doftable->insertDof( ie, lc, i, boost::make_tuple( 1, 0, gDof ), processor, next_free_dof, sign, false, global_shift, elt.edge( i ).marker() );
                }
            }
    ...
    void addFaceDof( element_type const& elt, uint16_type processor, size_type& next_free_dof,
                     ref_shift_type& shifts, mpl::int_<2>, mpl::bool_<true> )
        {
            ...
            for ( uint16_type l = 0; l < mortar_fe_type::nDofPerFace; ++l, ++lc )
            {
                ...
                M_doftable->insertDof( ie, lc, l, boost::make_tuple( 2, 0, gDof ), processor, next_free_dof, 1, false, global_shift, elt.marker() );
            }
                     ...
                     ref_shift_type& shifts, mpl::int_<3>, mpl::bool_<true> )
        {
            ...
            for ( uint16_type i = 0; i < element_type::numFaces; ++i )
            {
                ...
                for ( uint16_type l = 0; l < mortar_fe_type::nDofPerFace; ++l, ++lc )
                {
                    ...
                    M_doftable->insertDof( ie, lc, i, boost::make_tuple( 2, 0, gDof ), processor, next_free_dof, sign, false, global_shift,elt.face( i ).marker() );
    ...
    void addVolumeDof( element_type const& elt, uint16_type processor, size_type& next_free_dof,
                       ref_shift_type& shifts, mpl::bool_<true> )
        {
            ...
            for ( uint16_type l = 0; l < mortar_fe_type::nDofPerVolume; ++l, ++lc )
            {
                ...
                M_doftable->insertDof( ie, lc, l, boost::make_tuple( 3, 0, gDof ), processor, next_free_dof, 1, false, global_shift, elt.marker() );
            }
