      ...
      public boost::enable_shared_from_this<Mesh<GeoShape, T, Tag>>
{
  ...
  public:
    ...
    void updateNumGlobalElements( typename std::enable_if<is_3d<MT>::value>::type* = nullptr )
    {
        ...
        auto opBinaryPlusTuple3 = []( std::tuple<size_type, size_type, size_type> const& cur, std::tuple<size_type, size_type, size_type> const& res ) { return std::make_tuple( std::get<0>( cur ) + std::get<0>( res ), std::get<1>( cur ) + std::get<1>( res ), std::get<2>( cur ) + std::get<2>( res ) ); };
...
template <typename Shape, typename T, int Tag>
template <typename Iterator>
void Mesh<Shape, T, Tag>::createSubmesh( self_type& new_mesh,
                                         Iterator const& begin_elt,
                                         Iterator const& end_elt,
                                         size_type extraction_policies ) const
{
    ...
    for ( Iterator it = begin_elt; it != end_elt; ++it )
    {
        ...
        {
            FEELPP_ASSERT( old_elem.point( n ).id() < new_node_numbers.size() )
            ...
            if ( new_node_numbers[old_elem.point( n ).id()] == invalid_size_type_value )
            {
                ...
                {
                    FEELPP_ASSERT( new_vertex[old_elem.point( n ).id()] == 0 )
                        .error( "already seen this point?" );
                    new_vertex[old_elem.point( n ).id()] = 1;
                }
            }

            // Define this element's connectivity on the new mesh
            FEELPP_ASSERT( new_node_numbers[old_elem.point( n ).id()] < new_mesh.numPoints() )
    ...
    for ( ; it != en; ++it )
    {
        ...
        for ( uint16_type n = 0; n < element_type::numVertices; n++ )
        {
            ...
            if ( new_node_numbers.find( old_point.id() ) == new_node_numbers.end() )
            {
                ...
                // Increment the new node counter
                n_new_nodes++;
                FEELPP_ASSERT( !new_vertex[old_point.id()] )
                    .error( "already seen this point?" );
