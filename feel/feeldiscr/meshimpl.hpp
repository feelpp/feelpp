...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::partition( const uint16_type n_parts )
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::updateForUse()
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::updateMeasures()
{
    if ( this->components().test( MESH_NO_UPDATE_MEASURES ) )
        return;
    ...
    Mesh<Shape, T, Tag>::renumber( mpl::bool_<true> )
{
    ...
    FEELPP_ASSERT( check_id.size() == node_map.size() )
    ( node_map.size() )( check_id.size() ).error( "all ids must be unique" );
    ...
    {
        FEELPP_ASSERT( std::find( node_map.begin(), node_map.end(), invalid_size_type_value ) == node_map.end() )
            .warn( "invalid size_type value found as id " );
    }
    ...
    for ( ; ptmapit != ptmapen; ++ptmapit )
    {
        ...
        FEELPP_ASSERT( __rep1 )
        ( __rep1 )( ptit->id() )( ptmapit->second.id() ).warn( "invalid point replacement" );
    }
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::renumber( std::vector<size_type> const& node_map, mpl::int_<1> )
{
}
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::renumber( std::vector<size_type> const& node_map, mpl::int_<2> )
{
}

template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::renumber( std::vector<size_type> const& node_map, mpl::int_<3> )
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::localrenumber()
{
    for ( element_const_iterator elt = this->beginElement();
          elt != this->endElement(); ++elt )
    {
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::updateEntitiesCoDimensionOne()
{
    ...
    if ( ! this->faces().empty() )
    {
        ...
        for ( ; __it!=__en; )
        {
            ...
            else
            {
                ...
                size_type newFaceId = _faceit->second;
                this->faces().modify( __it, [&newFaceId]( face_type& f ) { f.setId( newFaceId ); } );
    ...
    for ( ; iv != en; ++iv )
    {
        ...
        for ( uint16_type/*size_type*/ j = 0; j < this->numLocalFaces(); j++ )
        {
            ...
            if ( faceinserted )
            {
                ...
                face.setConnection0( boost::make_tuple( eltPtr, __element_id, j, eltPid ) );
                ...
                __fit  = this->faces().insert( this->endFace(), face );
                //continue;
            ...
            else
            {
                ...
                face_iterator __fit = this->faceIterator( _faceit->second );
                //FEELPP_ASSERT( __fit != this->endFace() )( _faceit->second ).error( "face is not in face container" );
                face_type const& cface = *__fit;

                if ( this->components().test( MESH_ADD_ELEMENTS_INFO ) )
                    this->faces().modify( __fit, [&__element_id]( face_type& f ) { f.addElement( __element_id ); });
                ...
                if ( cface.isConnectedTo0() && cface.connection0().template get<0>() == 0 && ( __element_id == cface.ad_first() ) )
                {
                    ...
                    auto connect0 = cface.connection0();
                    connect0.template get<0>() = boost::addressof( __element );
                    this->faces().modify( __fit, [&connect0]( face_type& f ) { f.setConnection0( connect0 ); });
                    // need to reconstruct the neighbors
                    this->elements().modify( iv, Feel::detail::UpdateFace<face_type>( boost::cref( *__fit ) ) );
                }
                if ( cface.isConnectedTo1() && cface.connection1().template get<0>() == 0 && ( __element_id == __fit->ad_second() ) )
                {
                    ...
                    this->faces().modify( __fit, [&connect1]( face_type& f ) { f.setConnection1( connect1 ); });
                    // need to reconstruct the neighbors
                    element_iterator elt1 = this->elementIterator( cface.ad_first(), cface.proc_first() );
                ...
                if ( cface.isConnectedTo0() && cface.isConnectedTo1() )
                {
                    ...
                    if ( cface.processId()!=this->worldComm().localRank() )
                    {
                        if ( ( cface.element0().processId()==this->worldComm().localRank() ) ||
                            ...
                            this->faces().modify( __fit, [&currentPid]( face_type& f ) { f.setProcessId( currentPid ); });
                    }
                }
...
#endif
                    Feel::detail::UpdateFaceConnection0<typename face_type::element_connectivity_type> update0( boost::make_tuple( eltPtr, __element_id, j, eltPid ) );
                    this->faces().modify( __fit, update0 );
                    this->elements().modify( iv, Feel::detail::UpdateFace<face_type>( cface ) );
                ...
                {
                    Feel::detail::UpdateFaceConnection1<typename face_type::element_connectivity_type> update1( boost::make_tuple( eltPtr, __element_id, j, eltPid ), currentPid );
                    this->faces().modify( __fit, update1 );
                    ...
                    this->elements().modify( elt1, Feel::detail::UpdateFace<face_type>( cface ) );
                    this->elements().modify( iv, Feel::detail::UpdateFace<face_type>( cface ) );
...
{
    boost::timer ti;
    const bool updateComponentFacesMinimal = !this->components().test( MESH_UPDATE_FACES ) && this->components().test( MESH_UPDATE_FACES_MINIMAL );
    ...
    if ( !this->faces().empty() )
    {
        ...
        for ( ; __it != __en; )
        {
            ...
            else
            {
                ...
                if ( updateComponentFacesMinimal )
                {
                    f2e.push_back( std::make_tuple( invalid_size_type_value, invalid_rank_type_value, 0 ) );
                }
        ...
        // clean faces container, rebuild properly after
        this->faces().clear();
    }
    ...
    for ( ; iv != en; ++iv )
    {
        ...
        for ( uint16_type /*size_type*/ j = 0; j < this->numLocalFaces(); j++ )
        {
            ...
            if ( faceinserted )
            {
                ...
                face_type* facePtr = _facesOrderedWithId.back();
                ...
                if ( updateComponentFacesMinimal )
                {
                    f2e.push_back( std::make_tuple( __element_id, eltPid, j ) );
                }
            ...
            else
            {
                ...
                if ( cface.isConnectedTo0() && cface.isConnectedTo1() )
                {
                    ...
                    ( j )( iv->id() ).warn( "invalid element face error" );
                    FEELPP_ASSERT( cface.isConnectedTo0() && cface.isConnectedTo1() )
                    ( cface.isConnectedTo0() )( cface.isConnectedTo1() ).error( "inconsistent data structure" );
                    if ( cface.processId() != this->worldComm().localRank() )
                    {
                ...
                if ( !facePtr->isConnectedTo0() )
                {
                    ...
                    if ( updateComponentFacesMinimal )
                    {
                        f2e[facePtr->id()] = std::make_tuple( __element_id, eltPid, j );
                    }
                ...
                // we found an internal face
                else if ( !facePtr->isConnectedTo1() )
                {
                    if ( updateComponentFacesMinimal )
                    {
                        ...
                        if ( removeThisFace && numPartition > 1 )
                        {
                            ...
                            {
                                if ( ptsTouchInterProcess.find( cface.point( vLocId ).id() ) != ptsTouchInterProcess.end() )
                                {
                                    removeThisFace = false;
                                    break;
                                }
                            }
                        }
                        if ( removeThisFace )
                        {
                            size_type faceId = cface.id();
                            delete facePtr;
                            _facesOrderedWithId[faceId] = nullptr;
                            _faceit->second = nullptr;
                            // go to next face
                            continue;
                        }
                    }
...
#else
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::updateAdjacencyElements()
{
    VLOG( 2 ) << "Compute adjacency graph\n";
    //boost::unordered_map<std::set<int>, size_type > _faces;
    //typename boost::unordered_map<std::set<int>, size_type >::iterator _faceit;
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::fixPointDuplicationInHOMesh( element_iterator iv, face_iterator __fit, mpl::true_ )
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::fixPointDuplicationInHOMesh( element_iterator iv, face_iterator __fit, mpl::false_ )
{
}

template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::modifyEdgesOnBoundary( face_iterator& it, mpl::bool_<true> )
{
    ...
    for ( int f = 0; f < face_type::numEdges; ++f )
    {
        if ( it->edge( f ).isOnBoundary() == false )
        {
            ...
            this->edges().modify( eit,
                                  []( edge_type& e ) { e.setOnBoundary( true, 1 ); } );
        }
    }
    DVLOG( 3 ) << "We have " << nelements( boundaryedges( this ) ) << " boundary edges";
}
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::modifyEdgesOnBoundary( face_iterator& f, mpl::bool_<false> )
{
}

template <typename Shape, typename T, int Tag>
bool Mesh<Shape, T, Tag>::modifyElementOnBoundaryFromEdge( element_iterator& e, mpl::bool_<false> )
{
    return false;
}
template <typename Shape, typename T, int Tag>
bool Mesh<Shape, T, Tag>::modifyElementOnBoundaryFromEdge( element_iterator& iv, mpl::bool_<true> )
{
    // in 3D check if the edges of the element touch the boundary
    bool isOnBoundary = false;
    for ( size_type j = 0; j < iv->nEdges(); j++ )
    {
        auto edgePtr = iv->edgePtr( j );
        if ( edgePtr )
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::updateOnBoundary()
{
            ...
            for ( int f = 0; f < face_type::numPoints; ++f )
            {
                if ( it->point( f ).isOnBoundary() == false )
                {
                    ...
                    this->points().modify( pit,
                                           []( point_type& p ) { p.setOnBoundary( true, 0 ); } );
                }
            }
        }
    }
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::removeFacesFromBoundary( std::initializer_list<uint16_type> markers )
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::updateEntitiesCoDimensionGhostCellByUsingBlockingComm()
{
...
void updateEntitiesCoDimensionTwoGhostCell_step1( ElementType const& theelt, resultghost_edge_type& idEdgesWithBary, mpl::false_ /**/ )
{
}
template <typename ElementType>
void updateEntitiesCoDimensionTwoGhostCell_step1( ElementType const& theelt, resultghost_edge_type& idEdgesWithBary, mpl::true_ /**/ )
{
...
void updateEntitiesCoDimensionTwoGhostCell_step2( MeshType& mesh, typename MeshType::element_type const& theelt, resultghost_edge_type const& idEdgesWithBaryRecv, mpl::false_ /**/ )
{
}
template <typename MeshType>
void updateEntitiesCoDimensionTwoGhostCell_step2( MeshType& mesh, typename MeshType::element_type const& theelt, resultghost_edge_type const& idEdgesWithBaryRecv, mpl::true_ /**/ )
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::updateEntitiesCoDimensionGhostCellByUsingNonBlockingComm()
{
    typedef std::vector<boost::tuple<size_type, std::vector<double>>> resultghost_point_type;
    typedef std::vector<boost::tuple<size_type, bool, std::vector<double>>> resultghost_edge_type;
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::check() const
{
    if ( nDim != nRealDim )
        return;
#if !defined( NDEBUG )
    ...
    for ( ; iv != en; ++iv )
    {
        ...
        if ( this->components().test( MESH_UPDATE_FACES ) )
        {
            ...
            {
                FEELPP_ASSERT( iv->facePtr( j ) )
                ( j )( iv->id() ).error( "invalid element face check" );
                VLOG( 2 ) << "------------------------------------------------------------\n";
             ...
             this->components().test( MESH_UPDATE_ELEMENTS_ADJACENCY ) )
        {
            ...
            {
                FEELPP_ASSERT( counter >= 1 || nEltInMesh == 1 )
                ( __element.id() )( __element.nNeighbors() )( counter ).warn( "invalid neighboring data" );
            }
        }
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::checkAndFixPermutation()
{
...
// --verbose
        ...
        if ( !is_anticlockwise )
        {
            ...
            {
                FEELPP_ASSERT( 0 )
                ( __element.id() )( __element.G() ).error( "invalid element type" );
                throw std::logic_error( "invalid element type" );
            }

            is_anticlockwise = __element.isAnticlockwiseOriented();
            FEELPP_ASSERT( is_anticlockwise == true )
            ( is_anticlockwise )( __element.id() )( __element.G() ).error( "invalid element permutation" );
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::send( int p, int tag )
{
    encode();
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::recv( int p, int tag )
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::Inverse::distribute( bool extrapolation )
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::Localization::init()
{
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::Localization::initBoundaryFaces()
{
    auto mesh = M_mesh.lock();
    if ( !mesh ) return;
...
Mesh<Shape, T, Tag>::Localization::searchElements( const node_type& p )
{
    ...
    FEELPP_ASSERT( std::distance( itLT, itLT_end ) > 0 )
        .error( " problem in list localization : is empty" );
#endif
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::Localization::searchInKdTree( const node_type& p,
                                                        std::list<std::pair<size_type, uint>>& ListTri )
{
...
#if !defined( NDEBUG )
    FEELPP_ASSERT( std::distance( itNN, itNN_end ) > 0 )
        .error( "none Near Neighbor Points are find" );
#endif
    ...
    for ( ; itNN != itNN_end; ++itNN )
    {
        ...
        for ( ; itL != itL_end; ++itL )
        {
            ...
            if ( find )
            {
                ...
                ListTri.push_back( std::make_pair( *itL, 1 ) );
        }
    }
}
...
template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::Localization::computeBarycenter()
{
    ...
    Mesh<Shape, T, Tag>::Localization::computeBarycenter( mpl::int_<3> /**/ ) const
{
    ...
    res( 0 ) /= this->kdtree()->nPoints();
    res( 1 ) /= this->kdtree()->nPoints();
    res( 2 ) /= this->kdtree()->nPoints();
    return res;
}

template <typename Shape, typename T, int Tag>
void Mesh<Shape, T, Tag>::Localization::computeBarycentersWorld()
{
