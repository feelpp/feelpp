...
struct KDTree::Leaf : public KDTree::Element
{
    ...
    {
        FEELPP_ASSERT( begin != end )
        ( std::distance( begin, end ) ).error( "invalid iterator" );
        it = begin;
    }
};
            ...
            unsigned dir )
{
    ...
    if ( npts > KDTree::Element::POINTS_PER_LEAF )
    {
        ...
        else
        {
            ...
            ( dir )( boost::get<0>( *itmedian )[dir] )( boost::get<0>( *( itmedian - 1 ) )[dir] )( median ).error( "invalid median iterator" );
...
void projection2d( KDTree::node_type& res,
...
void projection3d( KDTree::node_type& res,
                   const KDTree::node_type& pMin,
                   const KDTree::node_type& pMax,
                   const KDTree::node_type& node_search )
{
 ...
 */
void writeDecompositionData( KDTree::Element* tree, boost::shared_ptr<std::ostringstream> __ostr )
{
    if ( !tree->isleaf() )
    {
...
void KDTree::pointsInBox( points_type& inpts,
                          const node_type& min,
                          const node_type& max )
{
    inpts.resize( 0 );
...
void KDTree::search( const node_type& node_ )
{
...
void KDTree::run_search( KDTree::Element* tree, uint16_type iter )
{
...
void KDTree::update_Pts_search( const index_node_type& p )
{
...
void KDTree::writeLatexData( std::string __nameFile )
{
