...
class MeshAdaptation
{
  public:
    ...
    BOOST_PARAMETER_MEMBER_FUNCTION(
        ...
        adaptMesh,
        tag,
        ( required( initMesh, ( mesh_ptrtype ) ) // initial mesh
          ( geofile, ( std::string ) )           // geometry
          ( adaptType, ( std::string ) ) )       //type of adaptation : isotropic | anisotropic
        ( optional( var, (std::list<std::pair<element_type, std::string>>), M_defaultVar )( metric, (std::list<std::pair<std::vector<p1_element_type>, std::string>>), M_defaultMetric )( hMin, (double), 1.0e-3 )( hMax, (double), 100.0 )( tol, (double), 1.0 ) ) )
    {
                                         ...
                                         double hMin, double hMax, double tol )
{
    ...
    {
        boost::for_each( var, [&initMesh, &hMin, &hMax, &geofile, &tol, &adaptType, &posfiles, &finalName, this]( std::pair<element_type, std::string> _var ) {
    ...
    {
        boost::for_each( metric, [&initMesh, &adaptType, &posfiles, &finalName, this]( std::pair<std::vector<p1_element_type>, std::string> _metric ) {
