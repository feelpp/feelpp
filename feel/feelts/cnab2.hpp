  ...
  public:
    ...
    double l2Error() const
    {
        ...
        if ( norm_u < 1e-12 )
            ...
            return normL2( _range = elements( mesh ), _expr = idv( u ) - idv( up ) ) / norm_u;
    }
...
template <typename FieldType>
template <typename FT, typename BCType>
void CNAB2<FieldType>::start( FT& f_d, BCType& bc )
{
    next( f_d, bc );
}
...
template <typename FieldType>
template <typename BCType>
void CNAB2<FieldType>::averaging( BCType& bc, double k1 )
{
...
template <typename FieldType>
template <typename FT, typename BCType>
bool CNAB2<FieldType>::next( FT& fd, BCType& bc, bool is_converged )
{
    if ( is_converged )
    {
        if ( Environment::isMasterRank() )
