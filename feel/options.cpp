...
gmsh_options( std::string const& prefix )
{
    ...
    // solver options
        ...
        ( prefixvm( prefix,"gmsh.use-json" ).c_str(), Feel::po::value<bool>()->default_value( true ), "use json/hdf5 file if it exists, instead of the Gmsh files (geo or msh)" )
...
solvereigen_options( std::string const& prefix )
{
    ...
    // solver options
        ...
        ( ( _prefix+"solvereigen.ncv" ).c_str(), Feel::po::value<int>()->default_value( PETSC_DEFAULT ), "dimension of the subspace" )
        ( ( _prefix+"solvereigen.mpd" ).c_str(), Feel::po::value<int>()->default_value( PETSC_DEFAULT ), "maximum projected dimension" )
