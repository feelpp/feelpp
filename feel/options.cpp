...
exporter_options( std::string const& prefix )
{
    ...
    _options.add_options()
        ...
        ( prefixvm( prefix,"exporter.merge" ).c_str(), Feel::po::value<bool>()->default_value( false ), "(HDF5 Only) Merge exported data from different into a single file (reduces the number of output files )" )
