...
ExporterExodus<MeshType,N>::save() const
{
    ...
    DVLOG(2) << "[ExporterExodus::save] checking if frequency is ok\n";
    ...
    boost::timer ti;
    DVLOG(2) << "[ExporterExodus::save] export in ensight format\n";

    DVLOG(2) << "[ExporterExodus::save] export sos\n";
    _F_writeSoSFile();
    DVLOG(2) << "[ExporterExodus::save] export sos ok, time " << ti.elapsed() << "\n";

    ti.restart();
    DVLOG(2) << "[ExporterExodus::save] export case file\n";
    _F_writeCaseFile();
    DVLOG(2) << "[ExporterExodus::save] export case file ok, time " << ti.elapsed() << "\n";

    ti.restart();
    DVLOG(2) << "[ExporterExodus::save] export geo(mesh) file\n";
    _F_writeGeoFiles();
    DVLOG(2) << "[ExporterExodus::save] export geo(mesh) file ok, time " << ti.elapsed() << "\n";

    ti.restart();
    DVLOG(2) << "[ExporterExodus::save] export variable file\n";
    _F_writeVariableFiles();
    DVLOG(2) << "[ExporterExodus::save] export variable files ok, time " << ti.elapsed() << "\n";

    ti.restart();
    DVLOG(2) << "[ExporterExodus::save] export time set\n";
    this->saveTimeSet();
    DVLOG(2) << "[ExporterExodus::save] export time set ok, time " << ti.elapsed() << "\n";
}

template<typename MeshType, int N>
void
ExporterExodus<MeshType,N>::_F_writeSoSFile() const
{
    // only on proc 0
...
template<typename MeshType, int N>
void
ExporterExodus<MeshType,N>::_F_writeCaseFile() const
{
    std::ostringstream filestr;
    filestr << this->path() << "/"
            << this->prefix() << "-"
...
template<typename MeshType, int N>
void
ExporterExodus<MeshType,N>::_F_writeGeoFiles() const
{
    namespace lambda = boost::lambda;
    ...
    while ( __ts_it != __ts_en )
    {
        ...
        if ( this->exporterGeometry() == EXPORTER_GEOMETRY_STATIC )
        {
            ...
            if ( __ts->hasMesh() )
                __ts->mesh()->accept( const_cast<ExporterExodus<MeshType,N>&>( *this ) );
            if ( (*__it)->hasMesh() && !__ts->hasMesh() )
                (*__it)->mesh()->accept( const_cast<ExporterExodus<MeshType,N>&>( *this ) );
        }
        ...
        while ( __it != __end )
        {
            ...
            if ( this->exporterGeometry() != EXPORTER_GEOMETRY_STATIC )
            {
                ...
                if ( __step->isInMemory() )
                {
                    ...
                    _M_filename =  __geofname.str();
                    __step->mesh()->accept( const_cast<ExporterExodus<MeshType,N>&>( *this ) );
                }
            }
            ++__it;
        }
...
template<typename MeshType, int N>
void
ExporterExodus<MeshType,N>::_F_writeVariableFiles() const
{
    namespace lambda = boost::lambda;
...
void
ExporterExodus<MeshType,N>::saveNodal( typename timeset_type::step_ptrtype __step, Iterator __var, Iterator en ) const
{
    while ( __var != en )
    {
        ...
        DVLOG(2) << "[ExporterExodus::saveNodal] saving " << __varfname.str() << "...\n";
        ...
        DVLOG(2) << "[ExporterExodus::saveNodal] saving " << __varfname.str() << "done\n";
        ++__var;
    }
}
template<typename MeshType, int N>
template<typename Iterator>
void
ExporterExodus<MeshType,N>::saveElement( typename timeset_type::step_ptrtype __step, Iterator __evar, Iterator __evaren ) const
{
    while ( __evar != __evaren )
    {
        ...
        __evarfname << this->path() << "/" << __evar->first
                    ...
                    << "." << std::setfill( '0' ) << std::setw( 3 ) << __step->index();
        DVLOG(2) << "[ExporterExodus::saveElement] saving " << __evarfname.str() << "...\n";
        ...
        DVLOG(2) << "[ExporterExodus::saveElement] saving " << __evarfname.str() << "done\n";
        ++__evar;
    }
}
