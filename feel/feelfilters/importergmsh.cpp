    ...
    if ( this->version() == "2.0" && std::string( __buf ) == "$MeshFormat" )
        {
            ...
            FEEL_ASSERT( theversion == "2" )( theversion )( this->version() ).error( "invalid gmsh file format version ");
            ...
            if ( this->version() == "1.0" )
            ...
            else if ( this->version() == "2.0" )
                {
                    ...
                    __is >> __ne  // elm-number
                         ...
                         >> __tag; // reg-phys
                    FEEL_ASSERT( __ntag >= 2 )( __ntag )( __tag )( __dummy ).warn( "invalid number of tags" );
...
ImporterGmsh<MeshType>::addVolume( mesh_type* mesh, std::vector<int> const& __e, int tag, GMSH_ENTITY type, mpl::int_<3> )
{
    ...
    if ( type == GMSH_TETRAHEDRON_2 )
        {
            // warning the numerotation differs between feel and gmsh
