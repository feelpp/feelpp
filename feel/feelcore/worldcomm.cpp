...
void WorldComm::init( int color, bool colormap )
{
...
bool WorldComm::operator==( WorldComm const& wc ) const
{
...
void WorldComm::initSubWorldCommSeq()
{
    std::vector<int> newIsActive( this->godSize(), false );
...
void WorldComm::showMe( std::ostream& __out ) const
{
...
void WorldComm::applyActivityOnlyOn( int _localColor ) const
{
...
int WorldComm::subWorldId( int n ) const
{
    if ( this->globalSize() == 1 )
        return 0;
...
bool WorldComm::hasSubWorlds( int n ) const
{
...
int WorldComm::numberOfSubWorlds() const
{
...
void WorldComm::setColorMap( std::vector<int> const& colormap )
{
    M_mapColorWorld = colormap;
