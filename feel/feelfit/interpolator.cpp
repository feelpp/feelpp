...
Interpolator::value_type InterpolatorP0::diff( double _x )
{
    return 0.;
}
 ...
 */
InterpolatorSpline::InterpolatorSpline( std::vector<pair_type> data, ExtrapolationType_spline r = natural, ExtrapolationType_spline l = natural )
 ...
 */
InterpolatorAkima::InterpolatorAkima( std::vector<pair_type> data )
    : super( data )
{
    SpMat A( ( this->M_data.size() - 1 ) * 4, ( this->M_data.size() - 1 ) * 4 );
    std::vector<T> coef;
