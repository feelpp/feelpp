...
Jacobi<N, T>::operator()( value_type const& x ) const
{
    ...
    else // N >= 2
    {
        ...
        for ( int k = 2; k < N + 1; ++k )
        {
            ...
            value_type a3 = ( ( two * kv + apb - two ) * ( two * kv + apb - one ) * ( two * kv + apb ) );
            value_type a4 = ( two * ( kv + M_a - one ) * ( kv + M_b - one ) * ( two * kv + apb ) );
...
template <uint16_type N, typename T>
void JacobiBatchDerivation( ublas::matrix<T>& /*res*/, T /*a*/, T /*b*/,
...
void roots( JacobiP const& p, Vector& xr )
{
...
void gaussjacobi( VectorW& wr, VectorN& xr, T a = T( 0.0 ), T b = T( 0.0 ) )
{
    typedef T value_type;
 ...
 **/
template <int N, typename T, typename VectorW, typename VectorN>
void gausslobattojacobi( VectorW& wr, VectorN& xr, T a = T( 0.0 ), T b = T( 0.0 ) )
{
    typedef T value_type;
...
void gausslobattojacobi( size_type N, VectorW& wr, VectorN& xr, T a = T( 0.0 ), T b = T( 0.0 ), bool interior = false )
{
...
template <int N, typename T, typename VectorW, typename VectorN>
void left_gaussradaujacobi( VectorW& wr, VectorN& xr, T a = T( 0.0 ), T b = T( 0.0 ) )
{
    typedef T value_type;
...
template <int N, typename T, typename VectorW, typename VectorN>
void right_gaussradaujacobi( VectorW& wr, VectorN& xr, T a = T( 0.0 ), T b = T( 0.0 ) )
{
    typedef T value_type;
