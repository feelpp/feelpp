...
template <typename T>
struct IMTetrahedra<3, T> : public IMTetrahedra<4, T>
{
};
template <typename T>
...
template <typename T>
struct IMTetrahedra<5, T> : public IMTetrahedra<6, T>
{
};
} // detail
    ...
    : public PointSetQuadrature<Simplex<Dim, 1>, Order, T>
{
  ...
  public:
    ...
    permute( int m, QVec const& q, int wi, mpl::int_<3> )
    {
        ...
        {
            FEELPP_ASSERT( 0 )
            ( m ).error( "invalid multiplicity in IMSimplex<3>" );
        }
    }
        ...
        for ( int a = 0; a <= nOrder; a++ )
        {
            for ( int b = 0; b < nOrder - a; b++ )
            {
                ...
                for ( int c = 0; c <= cMax; c++ )
                {
                    ...
                    {
                        sum += 0.5 * w[q] * pow( x[q], (double)a ) * pow( y[q], (double)b ) * pow( 1.0 - x[q] - y[q], (double)c );
                    }
            ...
            for ( int b = 0; b < p - a; b++ )
            {
                ...
                for ( int c = 0; c <= cMax; c++ )
                {
                    ...
                    for ( int d = 0; d <= dMax; d++ )
                    {
                        ...
                        for ( int q = 0; q < w.size(); q++ )
                        {
                            sum += ( 1.0 / 6.0 ) * w[q] * pow( x[q], (double)a ) * pow( y[q], (double)b ) * pow( z[q], (double)c ) * pow( 1.0 - x[q] - y[q] - z[q], (double)d );
                        }
