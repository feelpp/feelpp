...
template <typename T>
inline MatrixPetsc<T>::MatrixPetsc( WorldComm const& worldComm )
    : super( worldComm ),
      M_destroy_mat_on_exit( true )
{
}
template <typename T>
inline MatrixPetsc<T>::MatrixPetsc( datamap_ptrtype const& dmRow, datamap_ptrtype const& dmCol )
    : super( dmRow, dmCol ),
      M_destroy_mat_on_exit( true )
{
}
template <typename T>
inline MatrixPetsc<T>::MatrixPetsc( datamap_ptrtype const& dmRow, datamap_ptrtype const& dmCol, WorldComm const& worldComm )
...
template <typename T>
inline MatrixPetsc<T>::MatrixPetsc( MatrixSparse<value_type> const& M, IS& isrow, IS& iscol )
    : super(),
      M_destroy_mat_on_exit( true )
{
...
template <typename T>
inline MatrixPetsc<T>::MatrixPetsc( MatrixSparse<value_type> const& M, std::vector<int> const& rowIndex, std::vector<int> const& colIndex )
    : super(),
      M_destroy_mat_on_exit( true )
{
    MatrixPetsc<T> const* A = dynamic_cast<MatrixPetsc<T> const*>( &M );
...
template <typename T>
inline MatrixPetsc<T>::MatrixPetsc( Mat m )
...
template <typename T>
inline MatrixPetsc<T>::MatrixPetsc( Mat m, datamap_ptrtype const& dmRow, datamap_ptrtype const& dmCol, bool destroyMatOnExit )
    : super( dmRow, dmCol ),
...
template <typename T>
inline MatrixPetsc<T>::~MatrixPetsc()
{
    this->clear();
}
...
template <typename T>
inline void MatrixPetsc<T>::close() const
{
    if ( !this->isInitialized() )
        return;
...
template <typename T>
inline size_type MatrixPetsc<T>::size1() const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "MatrixPetsc<> not properly initialized" );
    ;
...
template <typename T>
inline size_type MatrixPetsc<T>::size2() const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "MatrixPetsc<> not properly initialized" );
    ;
...
template <typename T>
inline size_type MatrixPetsc<T>::rowStart() const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "MatrixPetsc<> not properly initialized" );
    ;
...
template <typename T>
inline size_type MatrixPetsc<T>::rowStop() const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "MatrixPetsc<> not properly initialized" );
    ;
...
template <typename T>
inline void MatrixPetsc<T>::set( const size_type i,
                                 const size_type j,
...
template <typename T>
inline void MatrixPetsc<T>::add( const size_type i,
                                 const size_type j,
                                 const value_type& value )
{
    FEELPP_ASSERT( this->isInitialized() )
...
template <typename T>
inline bool MatrixPetsc<T>::closed() const
{
    return super::closed();
#if 0
...
template <typename T>
void MatrixPetsc<T>::addMatrix( const ublas::matrix<value_type>& dm,
                                const std::vector<size_type>& rows,
                                const std::vector<size_type>& cols )
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "petsc matrix not initialized" );

    const size_type m = dm.size1();
    const size_type n = dm.size2();

    FEELPP_ASSERT( rows.size() == this->size1() )
        .error( "invalid row size" );
    FEELPP_ASSERT( cols.size() == this->size2() )
...
template <typename T>
void MatrixPetsc<T>::addMatrix( int* rows, int nrows,
                                int* cols, int ncols,
                                value_type* data )
{
    FEELPP_ASSERT( this->isInitialized() )
...
template <typename T>
void MatrixPetsc<T>::setDiagonal( const Vector<T>& vecDiag )
{
    if ( !vecDiag.closed() )
...
template <typename T>
void MatrixPetsc<T>::addDiagonal( const Vector<T>& vecDiag )
{
    if ( !vecDiag.closed() )
        const_cast<Vector<T>*>( &vecDiag )->close();
...
template <typename T>
void MatrixPetsc<T>::multVector( const Vector<T>& arg, Vector<T>& dest, bool transpose ) const
{
...
template <typename T>
void MatrixPetsc<T>::matMatMult( MatrixSparse<T> const& matIn, MatrixSparse<T>& matRes ) const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "petsc matrix not initialized" );
    FEELPP_ASSERT( this->size2() == matIn.size1() )
    ( this->size2() )( matIn.size1() ).error( "incompatible dimension" );
...
template <typename T>
void MatrixPetsc<T>::PtAP( MatrixSparse<value_type> const& matP, MatrixSparse<value_type>& matC ) const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "petsc matrix not initialized" );
    FEELPP_ASSERT( this->size1() == matP.size1() )
    ( this->size1() )( matP.size1() ).error( "incompatible dimension" );
    FEELPP_ASSERT( this->size2() == matP.size1() )
    ( this->size2() )( matP.size1() ).error( "incompatible dimension" );
...
template <typename T>
void MatrixPetsc<T>::PAPt( MatrixSparse<value_type> const& matP, MatrixSparse<value_type>& matC ) const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "petsc matrix not initialized" );
    FEELPP_ASSERT( this->size1() == matP.size2() )
    ( this->size1() )( matP.size2() ).error( "incompatible dimension" );
    FEELPP_ASSERT( this->size2() == matP.size2() )
    ( this->size2() )( matP.size2() ).error( "incompatible dimension" );
...
// print
template <typename T>
void MatrixPetsc<T>::printMatlab( const std::string name ) const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "petsc matrix not properly initialized" );
                                 ...
                                 bool useSameDataMap, bool checkAndFixRange ) const
{
    ...
    std::vector<size_type> rows = ( checkAndFixRange ) ? this->mapRowPtr()->buildIndexSetWithParallelMissingDof( _rows ) : _rows;
    std::vector<size_type> cols = ( checkAndFixRange && !useSameDataMap ) ? this->mapColPtr()->buildIndexSetWithParallelMissingDof( _cols ) : ( useSameDataMap ) ? rows : _cols;
...
template <typename T>
void MatrixPetsc<T>::updateSubMatrix( boost::shared_ptr<MatrixSparse<T>>& submatrix,
...
template <typename T>
void MatrixPetsc<T>::getSubMatrixPetsc( std::vector<size_type> const& rows,
...
template <typename T>
void MatrixPetsc<T>::createSubmatrix( MatrixSparse<T>& submatrix,
                                      const std::vector<size_type>& rows,
...
MatrixPetsc<T>::addMatrix( const T a_in, MatrixSparse<T> const& X_in )
{
    ...
    FEELPP_ASSERT( this->size1() == X_in.size1() )
    ( this->size1() )( X_in.size1() ).error( "incompatible dimension" );
    FEELPP_ASSERT( this->size2() == X_in.size2() )
    ( this->size2() )( X_in.size2() ).error( "incompatible dimension" );
...
template <typename T>
void MatrixPetsc<T>::scale( T const a )
{
    if ( !this->closed() )
        this->close();
...
template <typename T>
void MatrixPetsc<T>::zeroRows( std::vector<int> const& rows,
...
template <typename T>
void MatrixPetsc<T>::diagonal( Vector<value_type>& out ) const
{
    if ( !this->closed() )
        this->close();
...
template <typename T>
void MatrixPetsc<T>::transpose( MatrixSparse<value_type>& Mt, size_type options ) const
{
    Context ctx( options );
    tic();
    if ( !this->closed() )
...
template <typename T>
void MatrixPetsc<T>::symmetricPart( MatrixSparse<value_type>& Mt ) const
{
...
template <typename T>
void MatrixPetsc<T>::updateBlockMat( boost::shared_ptr<MatrixSparse<T>> const& m, std::vector<size_type> const& start_i, std::vector<size_type> const& start_j )
{
    if ( !m->closed() )
        m->close();
...
template <typename T>
bool MatrixPetsc<T>::isSymmetric( bool check ) const
{
...
template <typename T>
bool MatrixPetsc<T>::isTransposeOf( MatrixSparse<T>& Trans ) const
{
    FEELPP_ASSERT( this->size2() == Trans.size1() )
    ( this->size2() )( Trans.size1() ).error( "incompatible dimension" );
...
template <typename T>
inline void MatrixPetsc<T>::zeroEntriesDiagonal()
{
#if 1
#if 0 //trop lent!
    int start=0, stop=0, ierr=0;
...
template <typename T>
inline void MatrixPetsc<T>::getMatInfo( std::vector<double>& vec )
{
...
void MatrixPetsc<T>::threshold( void )
{
    ...
    ierr = MatGetOwnershipRange( this->M_mat, &m, &n );
    CHKERRABORT( this->comm(), ierr );
    ...
    for ( int i = m; i < n; i++ )
    {
        ...
        PetscInt* cols2;
        PetscScalar* toPut;
        ierr = MatGetRow( this->M_mat, i, &ncols, &cols, &v );
        CHKERRABORT( this->comm(), ierr );
        ...
        // apply this when finish with MatGetRow
        ierr = MatRestoreRow( this->M_mat, i, &ncols, &cols, &v );
        CHKERRABORT( this->comm(), ierr );

        //if(doIt)
        {
            ierr = MatSetValues( this->M_mat, 1, &i, ncols2, cols2, toPut, INSERT_VALUES );
            CHKERRABORT( this->comm(), ierr );
            this->close();
            delete[] toPut;
            delete[] cols2;
...
template <typename T>
inline MatrixPetscMPI<T>::MatrixPetscMPI( WorldComm const& worldComm )
    : super( worldComm )
{
}
template <typename T>
inline MatrixPetscMPI<T>::MatrixPetscMPI( datamap_ptrtype const& dmRow, datamap_ptrtype const& dmCol )
    : super( dmRow, dmCol )
{
}
template <typename T>
inline MatrixPetscMPI<T>::MatrixPetscMPI( datamap_ptrtype const& dmRow, datamap_ptrtype const& dmCol, WorldComm const& worldComm )
    : super( dmRow, dmCol, worldComm )
{
}
...
template <typename T>
inline MatrixPetscMPI<T>::MatrixPetscMPI( Mat m, datamap_ptrtype const& dmRow, datamap_ptrtype const& dmCol, bool initLocalToGlobalMapping, bool destroyMatOnExit )
...
template <typename T>
void MatrixPetscMPI<T>::initLocalToGlobalMapping()
{
    int ierr = 0;
    IS isRow;
    IS isCol;
...
template <typename T>
inline void MatrixPetscMPI<T>::set( const size_type i,
                                    const size_type j,
                                    const value_type& value )
{
    FEELPP_ASSERT( this->isInitialized() )
...
template <typename T>
inline void MatrixPetscMPI<T>::add( const size_type i,
                                    const size_type j,
                                    const value_type& value )
{
    FEELPP_ASSERT( this->isInitialized() )
...
template <typename T>
void MatrixPetscMPI<T>::addMatrix( const ublas::matrix<value_type>& dm,
                                   const std::vector<size_type>& rows,
                                   const std::vector<size_type>& cols )
{
    FEELPP_ASSERT( this->isInitialized() )
...
template <typename T>
void MatrixPetscMPI<T>::addMatrix( int* rows, int nrows,
                                   int* cols, int ncols,
                                   value_type* data )
{
    FEELPP_ASSERT( this->isInitialized() )
...
template <typename T>
void MatrixPetscMPI<T>::zeroRows( std::vector<int> const& rows,
                                  Vector<value_type> const& values,
                                  Vector<value_type>& rhs,
                                  Context const& on_context,
                                  value_type value_on_diagonal )
