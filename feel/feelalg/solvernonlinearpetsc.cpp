...
void SolverNonLinearPetsc<T>::init()
{
    ...
    if ( !this->initialized() )
    {
        ...
        case NRICHARDSON:
            check( SNESSetType( M_snes, SNESNRICHARDSON ) );
            break;

        case NKSPONLY:
            check( SNESSetType( M_snes, SNESKSPONLY ) );
            break;
        //VIRS, VISS reduced space and semi-smooth method for
        //variational inequalities
        case VINEWTONRSLS:
            check( SNESSetType( M_snes, SNESVINEWTONRSLS ) );
            break;
        case VINEWTONRSTR:
            check( SNESSetType( M_snes, SNESVINEWTONSSLS ) );
            break;
        //NGMRES Nonlinear GMRES
        case NGMRES:
            check( SNESSetType( M_snes, SNESNGMRES ) );
            break;
        //QN Quasi-Newton methods like BFGS
        case QN:
            check( SNESSetType( M_snes, SNESQN ) );
            break;
        //Shell Your method, often used as a (nonlinear) preconditioner
        case NSHELL:
            check( SNESSetType( M_snes, SNESSHELL ) );
            break;
//GS Nonlinear Gauss-Seidel sweeps
#if PETSC_VERSION_LESS_THAN( 3, 5, 0 )
        case GS:
            check( SNESSetType( M_snes, SNESGS ) );
            break;
#else
        case GS:
            check( SNESSetType( M_snes, SNESNGS ) );
            break;
#endif
        //NCG Nonlinear Conjugate Gradients
        case NCG:
            check( SNESSetType( M_snes, SNESNCG ) );
            break;
        //FAS Full approximation scheme (nonlinear multigrid)
        case FAS:
            check( SNESSetType( M_snes, SNESFAS ) );
            break;
        //MS Multi-stage smoothers (in FAS for hyperbolic problems)
        case MS:
            check( SNESSetType( M_snes, SNESMS ) );
            break;
        case NASM:
            check( SNESSetType( M_snes, SNESNASM ) );
            break;
        case ANDERSON:
            check( SNESSetType( M_snes, SNESANDERSON ) );
            break;
        case ASPIN:
            check( SNESSetType( M_snes, SNESASPIN ) );
            break;
#endif
    ...
    if ( this->getAbsoluteResidualTol() == 0 )
    {
        ...
        __absResTol = this->getAbsoluteResidualTol();
    ...
    if ( this->getRelativeResidualTol() == 0 )
    {
        ...
        __relResTol = this->getRelativeResidualTol();
    ...
    if ( this->getAbsoluteSolutionTol() == 0 )
    {
        ...
        __absSolTol = this->getAbsoluteSolutionTol();
...
template <typename T>
void SolverNonLinearPetsc<T>::setPetscNlSolverType()
{
}

template <typename T>
void SolverNonLinearPetsc<T>::setPetscKspSolverType()
{
    int ierr = 0;
...
template <typename T>
void SolverNonLinearPetsc<T>::setPetscPreconditionerType()
{
...
template <typename T>
void SolverNonLinearPetsc<T>::updateNullSpace( Mat A )
{
...
template <typename T>
void SolverNonLinearPetsc<T>::updateNearNullSpace( Mat A )
{
