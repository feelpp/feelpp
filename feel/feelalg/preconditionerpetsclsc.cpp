...
static PetscErrorCode PCLSC2Allocate_Private( PC pc )
{
    ...
    ierr = KSPCreate( PetscObjectComm( (PetscObject)pc ), &lsc->kspL );
    CHKERRQ( ierr );
    ierr = PetscObjectIncrementTabLevel( (PetscObject)lsc->kspL, (PetscObject)pc, 1 );
    CHKERRQ( ierr );
    ierr = KSPSetType( lsc->kspL, KSPPREONLY );
    CHKERRQ( ierr );
    ierr = KSPSetOptionsPrefix( lsc->kspL, ( (PetscObject)pc )->prefix );
    CHKERRQ( ierr );
    ierr = KSPAppendOptionsPrefix( lsc->kspL, "lsc_" );
    CHKERRQ( ierr );
    ierr = KSPSetFromOptions( lsc->kspL );
    CHKERRQ( ierr );
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 5, 0 )
    ierr = MatSchurComplementGetSubMatrices( pc->mat, &A, NULL, NULL, NULL, NULL );
    CHKERRQ( ierr );
#else
    ierr = MatSchurComplementGetSubmatrices( pc->mat, &A, NULL, NULL, NULL, NULL );
    CHKERRQ( ierr );
    ...
    ierr = MatGetVecs( A, &lsc->x0, &lsc->y0 );
    CHKERRQ( ierr );
    ierr = MatGetVecs( pc->pmat, &lsc->x1, NULL );
    CHKERRQ( ierr );
#else
    ierr = MatCreateVecs( A, &lsc->x0, &lsc->y0 );
    CHKERRQ( ierr );
    ierr = MatCreateVecs( pc->pmat, &lsc->x1, NULL );
    CHKERRQ( ierr );
#endif
    if ( lsc->scalediag )
    {
        ierr = VecDuplicate( lsc->x0, &lsc->scale );
        CHKERRQ( ierr );
    }
...
static PetscErrorCode PCSetUp_LSC2( PC pc )
{
    ...
    PetscFunctionBegin;
    ierr = PCLSC2Allocate_Private( pc );
    CHKERRQ( ierr );
    ...
    if ( lsc->scale )
    {
        if ( lsc->massMatrix )
        {
            ierr = MatGetDiagonal( lsc->massMatrix, lsc->scale );
            CHKERRQ( ierr );
        }
        else
        {
            Mat Ap;
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 5, 0 )
            ierr = MatSchurComplementGetSubMatrices( pc->mat, NULL, &Ap, NULL, NULL, NULL );
            CHKERRQ( ierr );
#else
            ierr = MatSchurComplementGetSubmatrices( pc->mat, NULL, &Ap, NULL, NULL, NULL );
            CHKERRQ( ierr );
            ...
            CHKERRQ( ierr ); /* Should be the mass matrix, but we don't have plumbing for that yet */
        }
        ierr = VecReciprocal( lsc->scale );
        CHKERRQ( ierr );
    }

    ierr = PetscObjectQuery( (PetscObject)pc->mat, "LSC_L", (PetscObject*)&L );
    CHKERRQ( ierr );
    if ( !L )
    {
        ierr = PetscObjectQuery( (PetscObject)pc->pmat, "LSC_L", (PetscObject*)&L );
        CHKERRQ( ierr );
    }
    ierr = PetscObjectQuery( (PetscObject)pc->pmat, "LSC_Lp", (PetscObject*)&Lp );
    CHKERRQ( ierr );
    if ( !Lp )
    {
        ierr = PetscObjectQuery( (PetscObject)pc->mat, "LSC_Lp", (PetscObject*)&Lp );
        CHKERRQ( ierr );
    }
    if ( !L )
    {
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 5, 0 )
        ierr = MatSchurComplementGetSubMatrices( pc->mat, NULL, NULL, &B, &C, NULL );
        CHKERRQ( ierr );
#else
        ierr = MatSchurComplementGetSubmatrices( pc->mat, NULL, NULL, &B, &C, NULL );
        CHKERRQ( ierr );
        ...
        if ( !lsc->L )
        {
            ...
            {
                // diag(F)^-1 * B
                ierr = MatDiagonalScale( B, lsc->scale, NULL );
                CHKERRQ( ierr );
            }
            // C* diag(F)^-1 * B
            ierr = MatMatMult( C, B, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &lsc->L );
            CHKERRQ( ierr );
        ...
        else
        {
            ...
            // C* diag(F)^-1 * B
            ierr = MatMatMult( C, B, MAT_REUSE_MATRIX, PETSC_DEFAULT, &lsc->L );
            CHKERRQ( ierr );
            // revert B and lsc->scale
            if ( lsc->scale )
            {
...
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 5, 0 )
    ierr = KSPSetOperators( lsc->kspL, L, Lp );
    CHKERRQ( ierr );
#else
    ierr = KSPSetOperators( lsc->kspL, L, Lp, SAME_PRECONDITIONER );
    CHKERRQ( ierr );
#endif
    PetscFunctionReturn( 0 );
}
...
static PetscErrorCode PCApply_LSC2( PC pc, Vec x, Vec y )
{
    ...
    PetscFunctionBegin;
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 5, 0 )
    ierr = MatSchurComplementGetSubMatrices( pc->mat, &A, NULL, &B, &C, NULL );
    CHKERRQ( ierr );
#else
    ierr = MatSchurComplementGetSubmatrices( pc->mat, &A, NULL, &B, &C, NULL );
    CHKERRQ( ierr );
#endif
    ierr = KSPSolve( lsc->kspL, x, lsc->x1 );
    CHKERRQ( ierr );
    ierr = MatMult( B, lsc->x1, lsc->x0 );
    CHKERRQ( ierr );
    if ( lsc->scale )
    {
        ierr = VecPointwiseMult( lsc->x0, lsc->x0, lsc->scale );
        CHKERRQ( ierr );
    }
    ierr = MatMult( A, lsc->x0, lsc->y0 );
    CHKERRQ( ierr );
    if ( lsc->scale )
    {
        ierr = VecPointwiseMult( lsc->y0, lsc->y0, lsc->scale );
        CHKERRQ( ierr );
    }
    ierr = MatMult( C, lsc->y0, lsc->x1 );
    CHKERRQ( ierr );
    ierr = KSPSolve( lsc->kspL, lsc->x1, y );
    CHKERRQ( ierr );
    PetscFunctionReturn( 0 );
}
...
static PetscErrorCode PCReset_LSC2( PC pc )
{
    ...
    PetscFunctionBegin;
    ierr = VecDestroy( &lsc->x0 );
    CHKERRQ( ierr );
    ierr = VecDestroy( &lsc->y0 );
    CHKERRQ( ierr );
    ierr = VecDestroy( &lsc->x1 );
    CHKERRQ( ierr );
    ierr = VecDestroy( &lsc->scale );
    CHKERRQ( ierr );
    ierr = KSPDestroy( &lsc->kspL );
    CHKERRQ( ierr );
    ierr = MatDestroy( &lsc->L );
    CHKERRQ( ierr );
    ierr = MatDestroy( &lsc->massMatrix );
    CHKERRQ( ierr );
    PetscFunctionReturn( 0 );
}
...
PETSC_EXTERN PetscErrorCode PCCreate_LSC2( PC pc )
{
    ...
    PetscFunctionBegin;
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 5, 0 )
    ierr = PetscNewLog( pc, &lsc );
    CHKERRQ( ierr );
#else
    ierr = PetscNewLog( pc, PC_LSC2, &lsc );
    CHKERRQ( ierr );
#endif
    pc->data = (void*)lsc;
