...
typedef enum { PCML_NULLSPACE_AUTO,
               PCML_NULLSPACE_USER,
               PCML_NULLSPACE_BLOCK,
               PCML_NULLSPACE_SCALAR } PCMLNullSpaceType;
...
static PetscErrorCode PCHYPRE_AMSSetCoordinateVectors( PC pc, Vec x, Vec y, Vec z )
{
    ...
    CHKERRQ( ierr );
    ierr = VecGetArray( y, &y_v );
    CHKERRQ( ierr );
    ierr = VecGetArray( z, &z_v );
    CHKERRQ( ierr );
    PetscReal* coord;
    PetscInt nloc;
    ...
    ierr = PCSetCoordinates( pc, 3, nloc, coord );
    CHKERRQ( ierr );
    ierr = VecRestoreArray( x, &x_v );
    CHKERRQ( ierr );
    ierr = VecRestoreArray( y, &y_v );
    CHKERRQ( ierr );
    ierr = VecRestoreArray( z, &z_v );
    CHKERRQ( ierr );
    delete[] coord;
    PetscFunctionReturn( 0 );
}
...
static PetscErrorCode PCFieldSplitGetSubKSP_FieldSplit_Schur( PC pc, PetscInt* n, KSP** subksp )
{
    ...
    //PetscFunctionBegin;
    ierr = PetscMalloc( jac->nsplits * sizeof( KSP ), subksp );
    CHKERRQ( ierr );
...
static PetscErrorCode PCFieldSplit_GetKSPInnerSchur( PC pc, KSP& ksp )
{
    ...
    ierr = MatSchurComplementGetKSP( jac->schur, &kspInner );
    CHKERRQ( ierr );
    if ( kspInner == NULL || kspInner == kspA )
    {
        ...
        ierr = KSPSetOperators( ksp, jac->mat[0], jac->pmat[0], pc->flag );
        CHKERRQ( ierr );
#else
        ierr = KSPSetOperators( ksp, jac->mat[0], jac->pmat[0] /*,pc->flag*/ );
        CHKERRQ( ierr );
#endif
        ierr = MatSchurComplementSetKSP( jac->schur, ksp );
        CHKERRQ( ierr );
        CHKERRQ( ierr );
    }
    else
...
static PetscErrorCode PCFieldSplit_GetKSPUpperSchur( PC pc, KSP& ksp )
{
    ...
    if ( jac->kspupper == NULL || jac->kspupper == kspA )
    {
        ...
        ierr = VecDuplicate( jac->head->x, &jac->head->z );
        CHKERRQ( ierr );
...
#if PETSC_VERSION_LESS_THAN( 3, 5, 0 )
    ierr = KSPSetOperators( jac->kspschur, jac->schur, schurMatPrecond, DIFFERENT_NONZERO_PATTERN );
    CHKERRQ( ierr );
#else
    ierr = KSPSetOperators( jac->kspschur, jac->schur, schurMatPrecond /*,DIFFERENT_NONZERO_PATTERN*/ );
    CHKERRQ( ierr );
...
static PetscErrorCode PCSetUp_Redundant( PC pc )
{
    ...
    if ( !pc->setupcalled )
    {
        ...
        if ( !red->psubcomm )
        {
            ierr = PetscSubcommCreate( comm, &red->psubcomm );
            CHKERRQ( ierr );
            ierr = PetscSubcommSetNumber( red->psubcomm, red->nsubcomm );
            CHKERRQ( ierr );
            ...
            ierr = PetscSubcommSetFromOptions( red->psubcomm );
            CHKERRQ( ierr );
            ierr = PetscLogObjectMemory( (PetscObject)pc, sizeof( PetscSubcomm ) );
            CHKERRQ( ierr );
...
/* create a new PC that processors in each subcomm have copy of */
            ...
            ierr = KSPCreate( subcomm, &red->ksp );
            CHKERRQ( ierr );
            ierr = PetscObjectIncrementTabLevel( (PetscObject)red->ksp, (PetscObject)pc, 1 );
            CHKERRQ( ierr );
            ierr = PetscLogObjectParent( (PetscObject)pc, (PetscObject)red->ksp );
            CHKERRQ( ierr );
            ierr = KSPSetType( red->ksp, KSPPREONLY );
            CHKERRQ( ierr );
            ierr = KSPGetPC( red->ksp, &red->pc );
            CHKERRQ( ierr );
            ierr = PCSetType( red->pc, PCLU );
            CHKERRQ( ierr );

            ierr = PCGetOptionsPrefix( pc, &prefix );
            CHKERRQ( ierr );
            ierr = KSPSetOptionsPrefix( red->ksp, prefix );
            CHKERRQ( ierr );
            ierr = KSPAppendOptionsPrefix( red->ksp, "redundant_" );
            CHKERRQ( ierr );
        }
        else
        {
...
#if PETSC_VERSION_LESS_THAN( 3, 6, 0 )
            ierr = MatGetRedundantMatrix( pc->pmat, red->psubcomm->n, subcomm, MAT_INITIAL_MATRIX, &red->pmats );
            CHKERRQ( ierr );
#else
            ierr = MatCreateRedundantMatrix( pc->pmat, red->psubcomm->n, subcomm, MAT_INITIAL_MATRIX, &red->pmats );
            CHKERRQ( ierr );
#endif
            ierr = KSPSetOperators( red->ksp, red->pmats, red->pmats );
            CHKERRQ( ierr );

/* get working vectors xsub and ysub */
#if PETSC_VERSION_LESS_THAN( 3, 6, 0 )
            ierr = MatGetVecs( red->pmats, &red->xsub, &red->ysub );
            CHKERRQ( ierr );
#else
            ierr = MatCreateVecs( red->pmats, &red->xsub, &red->ysub );
            CHKERRQ( ierr );
#endif
       ...
       Note: we use communicator dupcomm, not PetscObjectComm((PetscObject)pc)! */
            ierr = MatGetLocalSize( red->pmats, &mloc_sub, NULL );
            CHKERRQ( ierr );
            ierr = VecCreateMPI( red->psubcomm->dupparent, mloc_sub, PETSC_DECIDE, &red->xdup );
            CHKERRQ( ierr );
            ierr = VecCreateMPIWithArray( red->psubcomm->dupparent, 1, mloc_sub, PETSC_DECIDE, NULL, &red->ydup );
            CHKERRQ( ierr );

            /* create vecscatters */
            if ( !red->scatterin )
            { /* efficiency of scatterin is independent from psubcomm_type! */
                IS is1, is2;
                PetscInt *idx1, *idx2, i, j, k;

#if PETSC_VERSION_LESS_THAN( 3, 6, 0 )
                ierr = MatGetVecs( pc->pmat, &x, 0 );
                CHKERRQ( ierr );
#else
                ierr = MatCreateVecs( pc->pmat, &x, 0 );
                CHKERRQ( ierr );
#endif
                ierr = VecGetSize( x, &M );
                CHKERRQ( ierr );
                ierr = VecGetOwnershipRange( x, &mstart, &mend );
                CHKERRQ( ierr );
                mlocal = mend - mstart;
                ierr = PetscMalloc2( red->psubcomm->n * mlocal, &idx1, red->psubcomm->n * mlocal, &idx2 );
                CHKERRQ( ierr );
                j = 0;
                ...
                ierr = ISCreateGeneral( comm, red->psubcomm->n * mlocal, idx1, PETSC_COPY_VALUES, &is1 );
                CHKERRQ( ierr );
                ierr = ISCreateGeneral( comm, red->psubcomm->n * mlocal, idx2, PETSC_COPY_VALUES, &is2 );
                CHKERRQ( ierr );
                ierr = VecScatterCreate( x, is1, red->xdup, is2, &red->scatterin );
                CHKERRQ( ierr );
                ierr = ISDestroy( &is1 );
                CHKERRQ( ierr );
                ierr = ISDestroy( &is2 );
                ...
                ierr = ISCreateStride( comm, mlocal, mstart + red->psubcomm->color * M, 1, &is1 );
                CHKERRQ( ierr );
                ierr = ISCreateStride( comm, mlocal, mstart, 1, &is2 );
                CHKERRQ( ierr );
                ierr = VecScatterCreate( red->xdup, is1, x, is2, &red->scatterout );
                CHKERRQ( ierr );
                ierr = ISDestroy( &is1 );
                CHKERRQ( ierr );
                ierr = ISDestroy( &is2 );
                CHKERRQ( ierr );
                ierr = PetscFree2( idx1, idx2 );
                CHKERRQ( ierr );
                ierr = VecDestroy( &x );
                CHKERRQ( ierr );
            }
        ...
        { /* !red->useparallelmat */
            ierr = KSPSetOperators( red->ksp, pc->mat, pc->pmat );
            CHKERRQ( ierr );
        }
    }
    else
...
#if PETSC_VERSION_LESS_THAN( 3, 6, 0 )
            ierr = MatGetRedundantMatrix( pc->pmat, red->psubcomm->n, red->psubcomm->comm, reuse, &red->pmats );
            CHKERRQ( ierr );
#else
            ierr = MatCreateRedundantMatrix( pc->pmat, red->psubcomm->n, red->psubcomm->child, reuse, &red->pmats );
            CHKERRQ( ierr );
#endif
            ierr = KSPSetOperators( red->ksp, red->pmats, red->pmats );
            CHKERRQ( ierr );
    ...
    {
        ierr = KSPSetFromOptions( red->ksp );
        CHKERRQ( ierr );
    }
#if 0 // feelpp modif
...
template <typename T>
void PreconditionerPetsc<T>::setPrecMatrixStructure( MatrixStructure mstruct )
{
...
void SetPCType( PC& pc, const PreconditionerType& preconditioner_type, const MatSolverPackageType& matSolverPackage_type,
...
void updateOptionsDescKSP( po::options_description& _options, std::string const& prefix, std::string const& sub, bool useDefaultValue = true,
                                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                                            "maximum number of iterations" )( prefixvm( prefix, kspctx + "ksp-maxit-reuse" ).c_str(),
                                                                                                                                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                                                                                                                                              "maximum number of iterations when reuse prec/jac" )( prefixvm( prefix, kspctx + "constant-null-space" ).c_str(),
                                                                                                                                                                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                                                                                                                                                                    "set the null space to be the constant values" )( prefixvm( prefix, kspctx + "ksp-use-config-default-petsc" ).c_str(),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                      ( useDefaultValue ) ? Feel::po::value<bool>()->default_value( false ) : Feel::po::value<bool>(),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "configure ksp with default petsc options" )( prefixvm( prefix, kspctx + "gmres-restart" ).c_str(),
...
updateOptionsDescPrecBase( po::options_description& _options, std::string const& prefix, std::string const& sub, bool useDefaultValue = true, std::string const& pcType = "lu" )
{
    ...
    _options.add_options()( prefixvm( prefix, pcctx + "pc-type" ).c_str(),
                            ( useDefaultValue ) ? Feel::po::value<std::string>()->default_value( pcType ) : Feel::po::value<std::string>(),
                            "type of preconditioners (lu, cholesky, icc, ilut, ilutp, diag, id,...)" )( prefixvm( prefix, pcctx + "pc-view" ).c_str(),
                                                                                                        ( useDefaultValue ) ? Feel::po::value<bool>()->default_value( false ) : Feel::po::value<bool>(),
        ...
        ( prefixvm( prefix, pcctx + "pc-factor-mat-solver-package-type" ).c_str(),
          ...
          "sets the software that is used to perform the factorization (petsc,umfpack, spooles, petsc, superlu, superlu_dist, mumps,...)" )
            ...
            ( prefixvm( prefix, pcctx + "pc-use-config-default-petsc" ).c_str(),
              ( useDefaultValue ) ? Feel::po::value<bool>()->default_value( false ) : Feel::po::value<bool>(),
              "configure pc with defult petsc options" )( prefixvm( prefix, pcctx + "pc-factor-shift-type" ).c_str(),
...
void updateOptionsDescLU( po::options_description& _options, std::string const& prefix, std::string const& sub )
{
    std::string pcctx = ( sub.empty() ) ? "" : sub + "-";
...
//#if defined(FEELPP_HAS_MUMPS) && PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3,2,0 )
    ...
    for ( int icntl = 1; icntl <= 33; ++icntl )
    {
        ...
        if ( icntl == 7 )
            _options.add_options()( prefixvm( prefix, pcctx + mumpsOption ).c_str(),
...
void updateOptionsDescILU( po::options_description& _options, std::string const& prefix, std::string const& sub, bool useDefaultValue = true )
{
    std::string pcctx = ( sub.empty() ) ? "" : sub + "-";
    _options.add_options()( prefixvm( prefix, pcctx + "pc-factor-levels" ).c_str(),
                            ( useDefaultValue ) ? Feel::po::value<int>()->default_value( 3 ) : Feel::po::value<int>(),
                            "Sets the number of levels of fill to use for ilu" )( prefixvm( prefix, pcctx + "pc-factor-fill" ).c_str(),
...
void updateOptionsDescSOR( po::options_description& _options, std::string const& prefix, std::string const& sub, bool useDefaultValue = true )
{
    std::string pcctx = ( sub.empty() ) ? "" : sub + "-";
    _options.add_options()( prefixvm( prefix, pcctx + "pc-sor-omega" ).c_str(),
                            ( useDefaultValue ) ? Feel::po::value<double>()->default_value( 1. ) : Feel::po::value<double>(),
                            "Sets the SOR relaxation coefficient, omega" )( prefixvm( prefix, pcctx + "pc-sor-lits" ).c_str(),
                                                                            ( useDefaultValue ) ? Feel::po::value<int>()->default_value( 1 ) : Feel::po::value<int>(),
                                                                            "number of local iterations, smoothings over just variables on processor" )( prefixvm( prefix, pcctx + "pc-sor-its" ).c_str(),
                                                                                                                                                         ( useDefaultValue ) ? Feel::po::value<int>()->default_value( 1 ) : Feel::po::value<int>(),
                                                                                                                                                         "number of parallel iterations to use; each parallel iteration has lits local iterations" )( prefixvm( prefix, pcctx + "pc-sor-type" ).c_str(),
...
void updateOptionsDescGASM( po::options_description& _options, std::string const& prefix, bool useDefaultValue = true )
{
    _options.add_options()( prefixvm( prefix, "pc-gasm-type" ).c_str(), ( useDefaultValue ) ? Feel::po::value<std::string>()->default_value( "restrict" ) : Feel::po::value<std::string>(),
                            "type of gasm (basic, restrict, interpolate, none)" )( prefixvm( prefix, "pc-gasm-overlap" ).c_str(), ( useDefaultValue ) ? Feel::po::value<int>()->default_value( 1 ) : Feel::po::value<int>(),
...
void updateOptionsDescBOOMERAMG( po::options_description& _options, std::string const& prefix, bool useDefaultValue = true )
{
    _options.add_options()
        /// Documentation here : http://computation.llnl.gov/project/linear_solvers/download/hypre-2.10.0b_ref_manual.pdf
        ( prefixvm( prefix, "pc-hypre-boomeramg-max-iter" ).c_str(), Feel::po::value<int>()->default_value( 20 ), "sets the max number of iteration for boomeramg" )( prefixvm( prefix, "pc-hypre-boomeramg-tol" ).c_str(), Feel::po::value<double>()->default_value( 1e-7 ), "Convergence tolerance PER hypre call (0.0 = use a fixed number of iterations)" )( prefixvm( prefix, "pc-hypre-boomeramg-cycle-type" ).c_str(), Feel::po::value<int>()->default_value( 0 ), "Cycle type (0=V / 11==W" )( prefixvm( prefix, "pc-hypre-boomeramg-print-statistics" ).c_str(), Feel::po::value<bool>()->default_value( false ), "Print statistics" )( prefixvm( prefix, "pc-hypre-boomeramg-max-levels" ).c_str(), Feel::po::value<int>()->default_value( 25 ), "Number of levels (of grids) allowed" )( prefixvm( prefix, "pc-hypre-boomeramg-strong-threshold" ).c_str(), Feel::po::value<double>()->default_value( 0.25 ), "Threshold for being strongly connected" )( prefixvm( prefix, "pc-hypre-boomeramg-max-row-sum" ).c_str(), Feel::po::value<double>()->default_value( 0.9 ), "Maximum row sum" )( prefixvm( prefix, "pc-hypre-boomeramg-coarsen-type" ).c_str(), Feel::po::value<int>()->default_value( 6 ), "Coarsen type" )( prefixvm( prefix, "pc-hypre-boomeramg-agg-nl" ).c_str(), Feel::po::value<int>()->default_value( 1 ), "Number of levels of aggressive coarsening" )( prefixvm( prefix, "pc-hypre-boomeramg-relax-type-all" ).c_str(), Feel::po::value<int>()->default_value( 6 ), "Relax type for the up and down cycles" )( prefixvm( prefix, "pc-hypre-boomeramg-interp-type" ).c_str(), Feel::po::value<int>()->default_value( 6 ), "Interpolation type" )
...
void updateOptionsDescAMS( po::options_description& _options, std::string const& prefix, bool useDefaultValue = true )
{
    _options.add_options()
        /// Documentation here : http://computation.llnl.gov/project/linear_solvers/download/hypre-2.10.0b_ref_manual.pdf
        ( prefixvm( prefix, "pc-hypre-ams-print-level" ).c_str(), useDefaultValue ? Feel::po::value<int>()->default_value( 1 ) : Feel::po::value<int>(), "Debugging output level for AMS" )( prefixvm( prefix, "pc-hypre-ams-max-iter" ).c_str(), useDefaultValue ? Feel::po::value<int>()->default_value( 1 ) : Feel::po::value<int>(), "Maximum number of AMS multigrid iterations within PCApply" )( prefixvm( prefix, "pc-hypre-ams-cycle-type" ).c_str(), useDefaultValue ? Feel::po::value<int>()->default_value( 13 ) : Feel::po::value<int>(), "Cycle type for AMS multigrid" )( prefixvm( prefix, "pc-hypre-ams-tol" ).c_str(), useDefaultValue ? Feel::po::value<double>()->default_value( 0. ) : Feel::po::value<double>(), "Error tolerance for AMS multigrid" )( prefixvm( prefix, "pc-hypre-ams-relax-type" ).c_str(), useDefaultValue ? Feel::po::value<int>()->default_value( 2 ) : Feel::po::value<int>(), "Relaxation type for AMS smoother" )( prefixvm( prefix, "pc-hypre-ams-relax-times" ).c_str(), useDefaultValue ? Feel::po::value<int>()->default_value( 1 ) : Feel::po::value<int>(), "Number of relaxation steps for AMS smoother" )( prefixvm( prefix, "pc-hypre-ams-relax-weight" ).c_str(), useDefaultValue ? Feel::po::value<double>()->default_value( 1 ) : Feel::po::value<double>(), "Relaxation weight for AMS smoother" )( prefixvm( prefix, "pc-hypre-ams-omega" ).c_str(), useDefaultValue ? Feel::po::value<double>()->default_value( 1 ) : Feel::po::value<double>(), "SSOR coefficient for AMS smoother" )
...
void updateOptionsDescASM( po::options_description& _options, std::string const& prefix, bool useDefaultValue = true )
{
    _options.add_options()( prefixvm( prefix, "pc-asm-type" ).c_str(), ( useDefaultValue ) ? Feel::po::value<std::string>()->default_value( "restrict" ) : Feel::po::value<std::string>(),
                            "type of asm (basic, restrict, interpolate, none)" )( prefixvm( prefix, "pc-asm-overlap" ).c_str(), ( useDefaultValue ) ? Feel::po::value<int>()->default_value( 1 ) : Feel::po::value<int>(),
                                                                                  "number of overlap levels" );
}
po::options_description
...
getOptionsDescML( std::string const& prefix, std::string const& sub )
{
    ...
    // multigrid options
    _options.add_options()( prefixvm( prefix, pcctx + "mg-levels" ).c_str(), Feel::po::value<int>()->default_value( 10 /*2*/ ),
                            "number of levels including finest" )( prefixvm( prefix, pcctx + "mg-type" ).c_str(), Feel::po::value<std::string>()->default_value( "multiplicative" ),
                                                                   "Determines the form of multigrid to use: multiplicative, additive, full, kaskade " )( prefixvm( prefix, pcctx + "mg-smoothdown" ).c_str(), Feel::po::value<int>()->default_value( 1 ),
                                                                                                                                                          "number of smoothing steps before applying restriction operator" );
    // ml options
    _options.add_options()( prefixvm( prefix, pcctx + "ml-reuse-interpolation" ).c_str(), Feel::po::value<bool>()->default_value( false ),
                            "Reuse the interpolation operators when possible (cheaper, weaker when matrix entries change a lot)" )( prefixvm( prefix, pcctx + "ml-keep-agg-info" ).c_str(), Feel::po::value<bool>()->default_value( false ),
                                                                                                                                    "Allows the preconditioner to be reused, or auxilliary matrices to be generated" )( prefixvm( prefix, pcctx + "ml-reusable" ).c_str(), Feel::po::value<bool>()->default_value( false ),
                                                                                                                                                                                                                        "Store intermedaiate data structures so that the multilevel hierarchy is reusable" )( prefixvm( prefix, pcctx + "ml-old-hierarchy" ).c_str(), Feel::po::value<bool>()->default_value( false ),
    ...
    _options.add_options()( prefixvm( prefix, pcctx + "mg-levels" ).c_str(), Feel::po::value<int>()->default_value( 10 /*2*/ ),
                            "number of levels including finest" )( prefixvm( prefix, pcctx + "mg-type" ).c_str(), Feel::po::value<std::string>()->default_value( "multiplicative" ),
                                                                   "Determines the form of multigrid to use: multiplicative, additive, full, kaskade " )( prefixvm( prefix, pcctx + "mg-smoothdown" ).c_str(), Feel::po::value<int>()->default_value( 1 ),
                                                                                                                                                          "number of smoothing steps before applying restriction operator" );
    // gamg options
    _options.add_options()( prefixvm( prefix, pcctx + "gamg-type" ).c_str(), Feel::po::value<std::string>()->default_value( "agg" ),
                            "type of generalized algebraic multigrid : agg, geo " )( prefixvm( prefix, pcctx + "gamg-proc-eq-lim" ).c_str(), Feel::po::value<int>()->default_value( 50 ),
                                                                                     "number of equations to aim for on coarse grids via processor reduction" )( prefixvm( prefix, pcctx + "gamg-coarse-eq-lim" ).c_str(), Feel::po::value<int>()->default_value( 800 ),
                                                                                                                                                                 "max number of equations on coarse grids" )( prefixvm( prefix, pcctx + "gamg-threshold" ).c_str(), Feel::po::value<double>()->default_value( 0. ),
                                                                                                                                                                                                              "relative threshold to use for dropping edges in aggregation graph" )( prefixvm( prefix, pcctx + "gamg-set-sym-graph" ).c_str(), Feel::po::value<bool>()->default_value( true ),
                                                                                                                                                                                                                                                                                     "set for asymmetric matrice (if the matrix is sym, put to false)" )( prefixvm( prefix, pcctx + "gamg-reuse-interpolation" ).c_str(), Feel::po::value<bool>()->default_value( false ),
                                                                                                                                                                                                                                                                                                                                                          "reuse prolongation operator" )( prefixvm( prefix, pcctx + "gamg-verbose" ).c_str(), Feel::po::value<int>()->default_value( 0 ),
                                                                                                                                                                                                                                                                                                                                                                                           "verbose internal petsc info for gamg" )( prefixvm( prefix, pcctx + "gamg-nsmooths" ).c_str(), Feel::po::value<int>()->default_value( 1 ),
...
getOptionsDescFieldSplit( std::string const& prefix, std::string const& sub )
{
    ...
    // field split options
    _options.add_options()( prefixvm( prefix, pcctx + "fieldsplit-type" ).c_str(), Feel::po::value<std::string>()->default_value( "additive" ),
    ...
    _options.add_options()( prefixvm( prefix, pcctx + "fieldsplit-schur-fact-type" ).c_str(), Feel::po::value<std::string>()->default_value( "full" ),
                            "type of schur factorization (diag, lower, upper, full)" )( prefixvm( prefix, pcctx + "fieldsplit-schur-precondition" ).c_str(), Feel::po::value<std::string>()->default_value( "a11" ),
                                                                                        "self,user,a11" );
    ...
    _options.add_options()( prefixvm( prefixSchurInnerSolver, "use-outer-solver" ).c_str(), Feel::po::value<bool>()->default_value( true ), "use-outer-solver" );
    ...
    _options.add_options()( prefixvm( prefixSchurUpperSolver, "use-outer-solver" ).c_str(), Feel::po::value<bool>()->default_value( true ), "use-outer-solver" );
...
getOptionsDescSplitInFieldSplit( int nSplit, std::string const& prefix, std::string const& sub )
{
    ...
    for ( uint16_type i = 0; i < nSplit; ++i )
    {
        ...
        _options.add_options()( prefixvm( prefixfieldsplit, "fieldsplit-fields" ).c_str(), Feel::po::value<std::string>()->default_value( "" ),
                                "fields definition (ex: --fieldsplit-fields=0->(0,2),1->(1)" )( prefixvm( prefixfieldsplit, "fieldsplit-use-components" ).c_str(), Feel::po::value<bool>()->default_value( false ), "split also with components" );
...
getOptionsDescLSC( std::string const& prefix, std::string const& sub )
{
    ...
    _options.add_options()( prefixvm( prefixLSC, "scale-diag" ).c_str(), Feel::po::value<bool>()->default_value( false ), "scale diag" );
...
void ConfigureKSP::run( KSP& ksp ) const
{
...
void ConfigurePCHYPRE_EUCLID::run( PC& pc )
{
#if defined( PETSC_HAVE_HYPRE ) && PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 3, 0 )
...
void ConfigurePCHYPRE_BOOMERAMG::run( PC& pc )
{
#if defined( PETSC_HAVE_HYPRE ) && PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 3, 0 )
...
void ConfigurePCHYPRE_AMS::run( PC& pc )
{
#if defined( PETSC_HAVE_HYPRE ) && PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 6, 0 )
...
void ConfigurePCGASM::run( PC& pc )
{
#if ( PETSC_VERSION_MAJOR == 3 ) && ( PETSC_VERSION_MINOR >= 2 )
    /**/ if ( M_type == "restrict" )
        this->check( PCGASMSetType( pc, PC_GASM_RESTRICT ) );
    else if ( M_type == "basic" )
        this->check( PCGASMSetType( pc, PC_GASM_BASIC ) );
    else if ( M_type == "interpolate" )
        this->check( PCGASMSetType( pc, PC_GASM_INTERPOLATE ) );
    else if ( M_type == "none" )
        this->check( PCGASMSetType( pc, PC_GASM_NONE ) );
    else
        CHECK( false ) << "invalid gasm type : " << M_type << "\n";
...
void ConfigurePCASM::run( PC& pc )
{
    /**/ if ( M_type == "restrict" )
        this->check( PCASMSetType( pc, PC_ASM_RESTRICT ) );
    else if ( M_type == "basic" )
        this->check( PCASMSetType( pc, PC_ASM_BASIC ) );
    else if ( M_type == "interpolate" )
        this->check( PCASMSetType( pc, PC_ASM_INTERPOLATE ) );
    else if ( M_type == "none" )
        this->check( PCASMSetType( pc, PC_ASM_NONE ) );
    else
        CHECK( false ) << "invalid asm type : " << M_type << "\n";
      ...
      M_nBlock( 0 )
{
    ...
    if ( M_subPCfromPCtype == "block_jacobi" || M_subPCfromPCtype == "bjacobi" )
        ...
        CHECK( false ) << "invalid pctype " << M_subPCfromPCtype << "\n";
...
void ConfigureSubPC::run( KSP& ksp, ConfigureKSP const& kspConf )
{
...
void ConfigurePCML::run( PC& pc )
{
    ...
    if ( M_mgType == "multiplicative" )
        this->check( PCMGSetType( pc, PC_MG_MULTIPLICATIVE ) );
    else if ( M_mgType == "additive" )
        this->check( PCMGSetType( pc, PC_MG_ADDITIVE ) );
    else if ( M_mgType == "full" )
        this->check( PCMGSetType( pc, PC_MG_FULL ) );
    else if ( M_mgType == "kaskade" )
        this->check( PCMGSetType( pc, PC_MG_KASKADE ) );
    else
        CHECK( false ) << "invalid mgType :" << M_mgType << "\n";
}
void ConfigurePCML::configurePCMLCoarse( PC& pc )
{
...
void ConfigurePCGAMG::run( PC& pc )
{
...
void ConfigurePCGAMG::configurePCGAMGCoarse( PC& pc )
{
    std::vector<std::string> prefixOverwrite;
...
void ConfigurePCMGLevels::run( PC& pc, int level )
{
...
void ConfigurePCFieldSplit::run( PC& pc )
{
    ...
    /**/ if ( M_type == "schur" )
        theFieldSplitType = PC_COMPOSITE_SCHUR;
    else if ( M_type == "additive" )
        theFieldSplitType = PC_COMPOSITE_ADDITIVE;
    else if ( M_type == "multiplicative" )
        theFieldSplitType = PC_COMPOSITE_MULTIPLICATIVE;
    else if ( M_type == "symmetric-multiplicative" )
        theFieldSplitType = PC_COMPOSITE_SYMMETRIC_MULTIPLICATIVE;
    else if ( M_type == "special" )
        theFieldSplitType = PC_COMPOSITE_SPECIAL;
    ...
    if ( M_type == "schur" )
    {
        ...
        /**/ if ( M_schurFactType == "diag" )
            theSchurFactType = PC_FIELDSPLIT_SCHUR_FACT_DIAG;
        else if ( M_schurFactType == "lower" )
            theSchurFactType = PC_FIELDSPLIT_SCHUR_FACT_LOWER;
        else if ( M_schurFactType == "upper" )
            theSchurFactType = PC_FIELDSPLIT_SCHUR_FACT_UPPER;
        else if ( M_schurFactType == "full" )
            theSchurFactType = PC_FIELDSPLIT_SCHUR_FACT_FULL;
        ...
        /**/ if ( M_schurPrecond == "self" )
            theSchurPrecond = PC_FIELDSPLIT_SCHUR_PRE_SELF;
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 6, 0 )
        else if ( M_schurPrecond == "selfp" )
            theSchurPrecond = PC_FIELDSPLIT_SCHUR_PRE_SELFP;
#endif
        else if ( M_schurPrecond == "user" )
            theSchurPrecond = PC_FIELDSPLIT_SCHUR_PRE_USER;
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 4, 0 )
        else if ( M_schurPrecond == "a11" )
            theSchurPrecond = PC_FIELDSPLIT_SCHUR_PRE_A11;
#else
        else if ( M_schurPrecond == "a11" )
            theSchurPrecond = PC_FIELDSPLIT_SCHUR_PRE_DIAG;
#endif
...
void ConfigurePCFieldSplit::ConfigureSubKSP::run( KSP& ksp, int splitId )
{
...
void ConfigurePCRedundant::run( PC& pc )
{
#if PETSC_VERSION_GREATER_OR_EQUAL_THAN( 3, 5, 0 )
