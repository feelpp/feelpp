...
po::options_description backendtrilinos_options( std::string const& prefix )
{
    ...
    _options.add_options()
        ...
        ( ( _prefix + "trilinos-pc-type" ).c_str(), Feel::po::value<std::string>()->default_value( "block-jacobi" ), "id, domain-decomposition, block-jacobi, neumann" )( ( _prefix + "trilinos-drop" ).c_str(), Feel::po::value<double>()->default_value( 1e-3 ), "drop tolerance for preconditioners" )( ( _prefix + "trilinos-fillin" ).c_str(), Feel::po::value<int>()->default_value( 2 ), "fill-in level value for preconditioners" )( ( _prefix + "trilinos-overlap" ).c_str(), Feel::po::value<int>()->default_value( 2 ), "domain overlap value for preconditioners" )( ( _prefix + "trilinos-overlap-type" ).c_str(), Feel::po::value<std::string>()->default_value( "symmetric" ), "type of domain decomposition overlap for preconditioners" )( ( _prefix + "trilinos-order" ).c_str(), Feel::po::value<int>()->default_value( 2 ), "order of the Neumann polynomial expansion for preconditioners" )( ( _prefix + "trilinos-local-parts" ).c_str(), Feel::po::value<int>()->default_value( 4 ), "check Trilinos documentation" )( ( _prefix + "trilinos-local-solver-type" ).c_str(), Feel::po::value<std::string>()->default_value( "Amesos" ), "Local domain subsolver for Ifpack" )( ( _prefix + "trilinos-local-solver" ).c_str(), Feel::po::value<std::string>()->default_value( "Amesos_Umfpack" ), "(Amesos_KLU, Amesos_Umfpack, Amesos_Superlu ) Local domain direct subsolver for Ifpack" )

        // solver control options
        ( ( _prefix + "trilinos-restart" ).c_str(), Feel::po::value<int>()->default_value( 20 ), "number of iterations before solver restarts (gmres)" )( ( _prefix + "trilinos-verbose" ).c_str(), Feel::po::value<std::string>()->default_value( "none" ), "(none, summary, last, all) print solver iterations" )( ( _prefix + "trilinos-maxiter" ).c_str(), Feel::po::value<int>()->default_value( 1000 ), "set maximum number of iterations" )( ( _prefix + "trilinos-tolerance" ).c_str(), Feel::po::value<double>()->default_value( 2e-10 ), "set solver tolerance" )( ( _prefix + "trilinos-residual" ).c_str(), Feel::po::value<std::string>()->default_value( "rhs" ), "set solver residual calculation" );
    return _options;
}
...
// -- SETTING OF OPTIONS --
void BackendTrilinos::set_maxiter( int maxiter )
{
    M_options.set( "max_iter", maxiter );
}

void BackendTrilinos::set_tol( double tol )
{
    M_options.set( "tol", tol );
}

void BackendTrilinos::set_drop( double drop )
{
    M_options.set( "drop", drop );
    M_options.set( "fact: drop tolerance", drop );
}

void BackendTrilinos::set_overlap( int overlap )
{
    M_options.set( "overlap", overlap );
    M_options.set( "partitioner: overlap", overlap );
}

void BackendTrilinos::set_restart( int restart )
{
    M_options.set( "kspace", restart );
}

void BackendTrilinos::set_fillin( int fillin )
{
    M_options.set( "ilut_fill", fillin );
    M_options.set( "fact: level-of-fill", fillin );
}

void BackendTrilinos::set_order( int order )
{
    M_options.set( "poly_order", order );
}

void BackendTrilinos::set_overlap_type( std::string str )
{
    M_options.set( "type_overlap", str );
}

void BackendTrilinos::set_residual( std::string str )
{
    M_options.set( "conv", str );
}

void BackendTrilinos::set_localparts( int localparts )
{
    M_options.set( "partitioner: local parts", localparts );
}

void BackendTrilinos::set_solver( std::string str )
{
    if ( str == "gmres" )
...
void BackendTrilinos::set_prec_local_solver( std::string str )
{
    M_local_solver = str;
}

void BackendTrilinos::set_prec_local_solver_type( std::string str )
{
    M_local_solver_type = str;
}

void BackendTrilinos::set_prec( std::string str )
{
    if ( str == "id" )
...
void BackendTrilinos::set_verbose( const std::string verb )
{
    if ( verb == "none" )
...
void BackendTrilinos::set_options( list_type opts )
{
    M_options = opts;
}
...
void BackendTrilinos::applyMatrix( sparse_matrix_type const& _A,
