...
template <typename T>
void MatrixEigenDense<T>::init( const size_type m,
                                const size_type n,
                                const size_type /*m_l*/,
                                const size_type /*n_l*/,
                                const size_type /*nnz*/,
...
template <typename T>
void MatrixEigenDense<T>::addMatrix( int* rows, int nrows,
                                     int* cols, int ncols,
                                     value_type* data )
{
...
template <typename T>
void MatrixEigenDense<T>::scale( const T a )
{
    M_mat *= a;
}

template <typename T>
void MatrixEigenDense<T>::resize( size_type nr, size_type nc, bool /*preserve*/ )
{
    M_mat.resize( nr, nc );
}

template <typename T>
void MatrixEigenDense<T>::close() const
{
    this->setIsClosed( true );
}

template <typename T>
void MatrixEigenDense<T>::transpose( MatrixSparse<value_type>& Mt, size_type options ) const
{
    FEELPP_ASSERT( 0 )
...
template <typename T>
void MatrixEigenDense<T>::diagonal( Vector<T>& dest ) const
{
#if 0
...
template <typename T>
void MatrixEigenDense<T>::addMatrix( value_type v, MatrixSparse<value_type> const& _m )
{
    MatrixEigenDense<value_type> const* m = dynamic_cast<MatrixEigenDense<value_type> const*>( &_m );
    FEELPP_ASSERT( m != 0 )
        .error( "invalid sparse matrix type, should be MatrixEigenDense" );
...
template <typename T>
void MatrixEigenDense<T>::updateBlockMat( boost::shared_ptr<MatrixSparse<value_type>> const& m,
...
template <typename T>
void MatrixEigenDense<T>::printMatlab( const std::string filename ) const
{
...
template <typename T>
void MatrixEigenDense<T>::zeroRows( std::vector<int> const& rows,
...
template <typename T>
void MatrixEigenDense<T>::sqrt( MatrixSparse<value_type>& _m ) const
{
...
template <typename T>
void MatrixEigenDense<T>::eigenValues( std::vector<std::complex<double>>& Eingvs )
{
...
template <typename T>
void MatrixEigenDense<T>::matMatMult( MatrixSparse<T> const& In, MatrixSparse<T>& Res ) const
{
    //FEELPP_ASSERT ( this->isInitialized() ).error( "eigendense matrix not initialized" );

    FEELPP_ASSERT( this->size2() == In.size1() )
    ( this->size2() )( In.size1() ).error( "incompatible dimension" );
    ...
    FEELPP_ASSERT( X != 0 )
        .error( "invalid eigendense matrix" );
...
template <typename T>
void MatrixEigenDense<T>::matInverse( MatrixSparse<T>& Inv )
{
    //FEELPP_ASSERT ( this->isInitialized() ).error( "eigendense matrix not initialized" );

    FEELPP_ASSERT( this->size1() == Inv.size1() )
    ( this->size1() )( Inv.size1() ).error( "incompatible dimension" );
...
template <typename T>
void MatrixEigenDense<T>::applyInverseSqrt( Vector<T>& vec_in, Vector<T>& vec_out )
{
    ...
    std::sort( eigen_vals.begin(), eigen_vals.end(),
               [&]( std::complex<double> const& x, std::complex<double> const& y ) -> bool { return real( x ) < real( y ); } );
...
template <typename T>
void MatrixEigenDense<T>::createSubmatrix( MatrixSparse<T>& submatrix,
