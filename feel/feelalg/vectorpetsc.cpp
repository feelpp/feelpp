...
template <typename T>
void VectorPetsc<T>::init( datamap_ptrtype const& dm )
{
...
template <typename T>
void VectorPetsc<T>::set( const value_type& value )
{
    int ierr = 0;
...
template <typename T>
void VectorPetsc<T>::set( const size_type i, const value_type& value )
{
...
template <typename T>
void VectorPetsc<T>::setVector( int* i, int n, value_type* v )
{
...
template <typename T>
void VectorPetsc<T>::add( const size_type i, const value_type& value )
{
    DCHECK( i < size() ) << "invalid index " << i << " size : " << size();
...
template <typename T>
void VectorPetsc<T>::addVector( int* i, int n, value_type* v )
{
//FEELPP_ASSERT(n<=size())( n )( size() ).error( "invalid local index array size" );
...
template <typename T>
void VectorPetsc<T>::pointwiseOperationsImpl( Vector<T> const& xx, Vector<T> const& yy, int op )
{
    if ( !this->closed() )
        this->close();
    if ( !xx.closed() )
...
template <typename T>
void VectorPetsc<T>::pointwiseMult( Vector<T> const& xx, Vector<T> const& yy )
{
    DCHECK( this->isInitialized() ) << "VectorPetsc<> not initialized";
...
template <typename T>
void VectorPetsc<T>::pointwiseDivide( Vector<T> const& xx, Vector<T> const& yy )
{
    DCHECK( this->isInitialized() ) << "VectorPetsc<> not initialized";
    this->pointwiseOperationsImpl( xx, yy, 1 );
}
template <typename T>
void VectorPetsc<T>::zero()
{
    DCHECK( this->isInitialized() ) << "VectorPetsc<> not initialized";
    if ( !this->closed() )
        this->close();
...
template <typename T>
void VectorPetsc<T>::localize( const Vector<T>& /*V*/ )
{
    CHECK( 0 ) << "invalid call, not implemented yet";
}

template <typename T>
void VectorPetsc<T>::insert( const Vector<T>& /*V*/,
...
template <typename T>
void VectorPetsc<T>::insert( const ublas::vector<T>& /*V*/,
                             const std::vector<size_type>& /*dof_indices*/ )
{
    CHECK( 0 ) << "invalid call, not implemented yet";
}
...
template <typename T>
void VectorPetsc<T>::scale( T factor_in )
{
    if ( !this->closed() )
        this->close();
...
template <typename T>
void VectorPetsc<T>::add( const value_type& v_in )
{
    if ( !this->closed() )
        this->close();
...
template <typename T>
void VectorPetsc<T>::add( const Vector<value_type>& v )
{
    this->add( 1., v );
}
template <typename T>
void VectorPetsc<T>::add( const value_type& a_in, const Vector<value_type>& v_in )
{
    if ( !this->closed() )
        this->close();
    if ( !v_in.closed() )
...
template <typename T>
void VectorPetsc<T>::addVector( const Vector<value_type>& V_in,
                                 ...
                                 bool checkAndFixRange ) const
{
    ...
    std::vector<size_type> rows = ( checkAndFixRange ) ? this->mapPtr()->buildIndexSetWithParallelMissingDof( _rows ) : _rows;
...
template <typename T>
void VectorPetsc<T>::updateSubVector( boost::shared_ptr<Vector<T>>& subvector,
...
template <typename T>
void VectorPetsc<T>::getSubVectorPetsc( std::vector<size_type> const& rows,
                                        Vec& subvec,
                                        bool init ) const
{
    if ( !this->closed() )
...
//----------------------------------------------------------------------------------------------------//
template <typename T>
void VectorPetscMPI<T>::init( const size_type n,
...
template <typename T>
void VectorPetscMPI<T>::init( datamap_ptrtype const& dm )
{
    if ( !this->map().isCompatible( *dm ) )
        this->setMap( dm );
    this->initImpl();
...
template <typename T>
void VectorPetscMPI<T>::initImpl( const bool fast )
{
    int ierr = 0;
    ...
    int petsc_n_localGhost = static_cast<PetscInt>( this->map().nLocalGhosts() );
    FEELPP_ASSERT( petsc_n_localWithoutGhost < petsc_n )
    ( petsc_n_localWithoutGhost )( petsc_n ).warn( "invalid local size" );
...
template <typename T>
void VectorPetscMPI<T>::set( const value_type& value )
{
    if ( !this->closed() )
        this->close();
    int ierr = 0;
...
template <typename T>
void VectorPetscMPI<T>::add( const value_type& v_in )
{
    if ( !this->closed() )
        this->close();
    int ierr = 0;
...
template <typename T>
void VectorPetscMPI<T>::add( const value_type& a_in, const Vector<value_type>& v_in )
{
    if ( !this->closed() )
        this->close();
    if ( !v_in.closed() )
...
template <typename T>
void VectorPetscMPI<T>::set( size_type i, const value_type& value )
{
    int ierr = 0;
    int i_val = static_cast<int>( i );
    PetscScalar petsc_value = static_cast<PetscScalar>( value );
...
template <typename T>
void VectorPetscMPI<T>::setVector( int* i, int n, value_type* v )
{
...
template <typename T>
void VectorPetscMPI<T>::add( const size_type i, const value_type& value )
{
    int ierr = 0;
    int i_val = static_cast<int>( i );
    PetscScalar petsc_value = static_cast<PetscScalar>( value );
...
template <typename T>
void VectorPetscMPI<T>::addVector( int* i, int n, value_type* v )
{
    DCHECK( this->isInitialized() ) << "vector not initialized";
    DCHECK( n <= this->size() ) << "invalid local index array size: " << n << " > " << this->size();
...
template <typename T>
void VectorPetscMPI<T>::pointwiseMult( Vector<T> const& x, Vector<T> const& y )
{
    if ( !this->closed() )
        this->close();
    if ( !x.closed() )
...
template <typename T>
void VectorPetscMPI<T>::pointwiseDivide( Vector<T> const& x, Vector<T> const& y )
{
    if ( !this->closed() )
        this->close();
    if ( !x.closed() )
...
template <typename T>
void VectorPetscMPI<T>::pointwiseOperationOthersPetscImpl( Vector<T> const& x, Vector<T> const& y, int op )
{
    int ierr = 0;
    const VectorPetscMPIRange<T>* vecxPetscMPIRange = dynamic_cast<const VectorPetscMPIRange<T>*>( &x );
    const VectorPetscMPIRange<T>* vecyPetscMPIRange = dynamic_cast<const VectorPetscMPIRange<T>*>( &y );
...
template <typename T>
int VectorPetscMPI<T>::reciprocal()
{
    if ( !this->closed() )
        this->close();
    int ierr = 0;
...
template <typename T>
void VectorPetscMPI<T>::duplicateFromOtherPartition( Vector<T> const& vecInput )
{
...
template <typename T>
void VectorPetscMPI<T>::duplicateFromOtherPartition_run( Vector<T> const& vecInput )
{
            ...
            else if ( vecInput.map().worldComm().globalRank() == rankLocalization && inputProcIsActive_fusion[worldCommFusion.globalRank()] )
            {
                ...
                for ( size_type k = 0; k < nDataRecv; ++k )
                {
                    ...
                    if ( original_dofCluster >= vecInput.map().firstDofGlobalCluster() && original_dofCluster <= vecInput.map().lastDofGlobalCluster() )
                    {
                        ...
                        dofClusterMissing_RequestIsFind[k] = 0;
                }
...
VectorPetscMPI<T>::localSize() const
{
    FEELPP_ASSERT( this->isInitialized() )
        .error( "VectorPetsc not initialized" );
...
template <typename T>
void VectorPetscMPIRange<T>::init( datamap_ptrtype const& dm )
{
    if ( !this->map().isCompatible( *dm ) )
        this->setMap( dm );
...
template <typename T>
void VectorPetscMPIRange<T>::initRangeView( const PetscScalar arrayActive[], const PetscScalar arrayGhost[] )
{
    // clear initialized vectors
    if ( this->isInitialized() )
        this->clear();
...
template <typename T>
void VectorPetscMPIRange<T>::clear()
{
    if ( !this->isInitialized() )
        return;
...
template <typename T>
void VectorPetscMPIRange<T>::set( const value_type& value )
{
    if ( !this->closed() )
        this->close();
    int ierr = 0;
...
template <typename T>
void VectorPetscMPIRange<T>::add( const value_type& v_in )
{
    if ( !this->closed() )
        this->close();
    int ierr = 0;
...
template <typename T>
void VectorPetscMPIRange<T>::add( const value_type& a_in, const Vector<value_type>& v_in )
{
    if ( !this->closed() )
        this->close();
    if ( !v_in.closed() )
...
template <typename T>
void VectorPetscMPIRange<T>::pointwiseMult( Vector<T> const& x, Vector<T> const& y )
{
    if ( !this->closed() )
        this->close();
    if ( !x.closed() )
...
template <typename T>
void VectorPetscMPIRange<T>::pointwiseDivide( Vector<T> const& x, Vector<T> const& y )
{
    if ( !this->closed() )
        this->close();
    if ( !x.closed() )
...
template <typename T>
void VectorPetscMPIRange<T>::zero()
{
    if ( !this->closed() )
        this->close();
    int ierr = 0;
...
template <typename T>
int VectorPetscMPIRange<T>::reciprocal()
{
    if ( !this->closed() )
        this->close();
    int ierr = 0;
...
template <typename T>
void VectorPetscMPIRange<T>::localize()
{
    if ( !this->closed() )
        this->close();
