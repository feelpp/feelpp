...
template <typename T>
void VectorEigen<T>::resize( size_type s, bool preserve )
{
...
VectorEigen<T>::operator=( const Vector<value_type>& V )
{
    ...
    ( this->localSize() )( V.localSize() ).warn( "invalid vector size" );
    FEELPP_ASSERT( this->firstLocalIndex() == V.firstLocalIndex() &&
                   this->lastLocalIndex() == V.lastLocalIndex() )
    ( this->firstLocalIndex() )( this->lastLocalIndex() )( this->vec().size() )( V.firstLocalIndex() )( V.lastLocalIndex() ).warn( "may be vector invalid  copy" );
...
template <typename T>
void VectorEigen<T>::init( const size_type n,
...
template <typename T>
void VectorEigen<T>::init( datamap_ptrtype const& dm )
{
    super1::init( dm );
    this->init( dm->nDof(), dm->nLocalDofWithGhost(), false );
}
...
template <typename T>
void VectorEigen<T>::clear()
{
    M_vec.resize( 0 );
}

template <typename T>
void VectorEigen<T>::close() const
{
}

template <typename T>
void VectorEigen<T>::printMatlab( const std::string filename, bool renumber ) const
{
    ...
    if ( this->comm().rank() == 0 )
    {
        ...
        ( filename ).error( "[VectorEigen::printMatlab] ERROR: File cannot be opened for writing." );
...
template <typename T>
void VectorEigen<T>::localize( Vector<T>& v_local_in ) const
...
template <typename T>
void VectorEigen<T>::localize( const size_type first_local_idx,
...
template <typename T>
void VectorEigen<T>::localize( vector_type& v_local ) const
{
    checkInvariant();
...
template <typename T>
void VectorEigen<T>::localizeToOneProcessor( vector_type& v_local,
...
template <typename T>
void VectorEigen<T>::localizeToOneProcessor( std::vector<value_type>& v_local,
...
template <typename T>
void VectorEigen<T>::checkInvariant() const
{
    ...
    ( this->size() )( this->lastLocalIndex() )( this->firstLocalIndex() )( this->localSize() ).error( "vector invalid size" );
}
...
template <typename T>
void VectorEigen<T>::insert( const ublas::vector<T>& V,
