...
PartitionerMetis<MeshType>::partitionImpl( mesh_ptrtype mesh, rank_type np, std::vector<range_element_type> const& partitionByRange )
{
    ...
    std::vector<range_element_type> partitionByRangeUsed;
    if ( partitionByRange.empty() )
        partitionByRangeUsed.push_back( elements(mesh) );
    else
        partitionByRangeUsed = partitionByRange;
    for ( auto const& rangeMeshElt : partitionByRangeUsed )
    {
