...
void MarkerManagementDirichletBC::clearMarkerDirichletBC()
{
    M_dirichletBCType.clear();
    M_dirichletBCMarkersListByType.clear();
}

void MarkerManagementDirichletBC::setMarkerDirichletBCByNameId( std::string type, std::string markerNameId, std::list<std::string> const& markers,
                                                                ComponentType ct )
{
...
void MarkerManagementDirichletBC::addMarkerDirichletBC( std::string type, std::string markerNameId, ComponentType ct )
{
...
void MarkerManagementDirichletBC::updateForUseMarkerDirichletBC()
{
    M_dirichletBCMarkersListByType.clear();
...
bool MarkerManagementDirichletBC::hasMarkerDirichletBC( std::string type, ComponentType ct ) const
{
...
bool MarkerManagementDirichletBC::hasMarkerDirichletBCelimination( ComponentType ct ) const
{
    return this->hasMarkerDirichletBC( "elimination", ct );
}
bool MarkerManagementDirichletBC::hasMarkerDirichletBCnitsche( ComponentType ct ) const
{
    return this->hasMarkerDirichletBC( "nitsche", ct );
}
bool MarkerManagementDirichletBC::hasMarkerDirichletBClm( ComponentType ct ) const
{
    return this->hasMarkerDirichletBC( "lm", ct );
}
bool MarkerManagementDirichletBC::hasMarkerDirichletBCelimination() const
{
...
bool MarkerManagementDirichletBC::hasMarkerDirichletBCnitsche() const
{
...
bool MarkerManagementDirichletBC::hasMarkerDirichletBClm() const
{
...
MarkerManagementDirichletBC::getInfoDirichletBC() const
{
    ...
    for ( auto const& bykindbase : M_dirichletBCType )
    {
        ...
        std::string ctStr;
        if ( ct == ComponentType::X )
            ctStr = "[X]";
        else if ( ct == ComponentType::Y )
            ctStr = "[Y]";
        else if ( ct == ComponentType::Z )
            ctStr = "[Z]";
...
void MarkerManagementNeumannBC::clearMarkerNeumannBC()
{
    M_containerMarkers.clear();
}

void MarkerManagementNeumannBC::setMarkerNeumannBC( NeumannBCShape shape, std::string markerNameId, std::list<std::string> const& markers )
{
    if ( markers.empty() ) return;
    M_containerMarkers[shape][markerNameId] = markers;
}
void MarkerManagementNeumannBC::addMarkerNeumannBC( NeumannBCShape shape, std::string markerNameId )
{
    if ( markerNameId.empty() ) return;
...
MarkerManagementNeumannBC::getInfoNeumannBC() const
{
    ...
    {
        std::string shapeStr = ( markNeumanBase.first == NeumannBCShape::SCALAR ) ? "[scalar]" : ( markNeumanBase.first == NeumannBCShape::VECTORIAL ) ? "[vectorial]" : "[tensor2]";
...
void MarkerManagementNeumannEulerianFrameBC::clearMarkerNeumannEulerianFrameBC()
{
    M_containerMarkers.clear();
}

void MarkerManagementNeumannEulerianFrameBC::setMarkerNeumannEulerianFrameBC( NeumannEulerianFrameBCShape shape, std::string markerNameId, std::list<std::string> const& markers )
{
    if ( markers.empty() ) return;
    M_containerMarkers[shape][markerNameId] = markers;
}
void MarkerManagementNeumannEulerianFrameBC::addMarkerNeumannEulerianFrameBC( NeumannEulerianFrameBCShape shape, std::string markerNameId )
{
    if ( markerNameId.empty() ) return;
    M_containerMarkers[shape][markerNameId].push_back( markerNameId );
}
...
MarkerManagementNeumannEulerianFrameBC::getInfoNeumannEulerianFrameBC() const
{
    ...
    for ( auto const& markNeumanBase : M_containerMarkers )
    {
        std::string shapeStr = ( markNeumanBase.first == NeumannEulerianFrameBCShape::SCALAR ) ? "[scalar]" : ( markNeumanBase.first == NeumannEulerianFrameBCShape::VECTORIAL ) ? "[vectorial]" : "[tensor2]";
        for ( auto const& markNeuman : markNeumanBase.second )
        {
...
void MarkerManagementALEMeshBC::clearMarkerALEMeshBC()
{
    M_containerMarkers.clear();
}

void MarkerManagementALEMeshBC::setMarkerALEMeshBC( std::string type, std::list<std::string> const& markers )
{
    CHECK( type == "fixed" || type == "moving" || type == "free" ) << "error ALE type " << type;
    M_containerMarkers[type] = markers;
}
void MarkerManagementALEMeshBC::addMarkerALEMeshBC( std::string type, std::string markerName )
{
    CHECK( type == "fixed" || type == "moving" || type == "free" ) << "error ALE type " << type;
...
void MarkerManagementSlipBC::clearMarkerSlipBC()
{
    M_containerMarkers.clear();
}
void MarkerManagementSlipBC::setMarkerSlipBC( std::list<std::string> const& markers )
{
    M_containerMarkers = markers;
}
void MarkerManagementSlipBC::addMarkerSlipBC( std::string markerName )
{
...
void MarkerManagementPressureBC::clearMarkerPressureBC()
{
    M_containerMarkers.clear();
}
void MarkerManagementPressureBC::setMarkerPressureBC( std::list<std::string> const& markers )
{
    M_containerMarkers = markers;
}
void MarkerManagementPressureBC::addMarkerPressureBC( std::string markerName )
{
    if ( std::find( M_containerMarkers.begin(), M_containerMarkers.end(), markerName ) == M_containerMarkers.end() )
        M_containerMarkers.push_back( markerName );
}
...
void MarkerManagementRobinBC::clearMarkerRobinBC()
{
    M_containerMarkers.clear();
}
void MarkerManagementRobinBC::setMarkerRobinBC( std::string const& markerNameId, std::list<std::string> const& markers )
{
    M_containerMarkers[markerNameId] = markers;
}
void MarkerManagementRobinBC::addMarkerRobinBC( std::string const& markerNameId )
{
    if ( markerNameId.empty() ) return;
...
void MarkerManagementFluidStructureInterfaceBC::clearMarkerFluidStructureInterfaceBC()
{
    M_containerMarkers.clear();
}
void MarkerManagementFluidStructureInterfaceBC::setMarkerFluidStructureInterfaceBC( std::list<std::string> const& markers )
{
    M_containerMarkers = markers;
}
void MarkerManagementFluidStructureInterfaceBC::addMarkerFluidStructureInterfaceBC( std::string markerName )
{
    if ( std::find( M_containerMarkers.begin(), M_containerMarkers.end(), markerName ) == M_containerMarkers.end() )
        M_containerMarkers.push_back( markerName );
}
