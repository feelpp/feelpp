...
void ModelNumerical::addParameterInModelProperties( std::string const& symbolName, double value )
{
    M_modelProps->parameters()[symbolName] = ModelParameter( symbolName, value );
}

void ModelNumerical::setStationary( bool b )
{
    if ( M_isStationary != b )
    {
        M_isStationary = b;
...
void ModelNumerical::updateTime( double t )
{
    M_timeCurrent = t;
    M_modelProps->parameters()["t"] = ModelParameter( "current_time", M_timeCurrent );
}

void ModelNumerical::saveMSHfilePath( std::string const& fileSavePath, std::string const& meshPath ) const
{
