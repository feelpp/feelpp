...
void ModelMeasuresIO::clear()
{
    M_mapParameterData.clear();
    M_mapMeasureData.clear();
}

void ModelMeasuresIO::start()
{
...
void ModelMeasuresIO::restart( std::string const& paramKey, double val )
{
    if ( M_worldComm.isMasterRank() && !M_mapMeasureData.empty() )
    {
...
void ModelMeasuresIO::exportMeasures()
{
    if ( M_worldComm.isMasterRank() && !M_mapMeasureData.empty() )
    {
        bool hasAlreadyWrited = false;
...
void ModelMeasuresIO::setParameter( std::string const& key, double val )
{
    M_mapParameterData[key] = val;
}
void ModelMeasuresIO::setMeasure( std::string const& key, double val )
{
    M_mapMeasureData[key] = val;
}
void ModelMeasuresIO::setMeasureComp( std::string const& key, std::vector<double> const& values )
{
    if ( values.empty() )
        return;
...
void ModelMeasuresEvaluatorContext::add( std::string const& field, int ctxId, std::string const& name )
{
    M_mapFieldToMapCtxIdToName[field][ctxId] = name;
}
bool ModelMeasuresEvaluatorContext::has( std::string const& field ) const
{
...
bool ModelMeasuresEvaluatorContext::has( std::string const& field, int ctxId ) const
{
    auto itFindField = M_mapFieldToMapCtxIdToName.find( field );
    if ( itFindField == M_mapFieldToMapCtxIdToName.end() )
        return false;
...
int ModelMeasuresEvaluatorContext::ctxId( std::string const& field, std::string const& name ) const
{
    int ctxIdNull = -1;
    auto itFindField = M_mapFieldToMapCtxIdToName.find( field );
    if ( itFindField == M_mapFieldToMapCtxIdToName.end() )
...
void ModelMeasuresFlowRate::addMarker( std::string const& mark )
{
    if ( std::find( M_meshMarkers.begin(), M_meshMarkers.end(), mark ) == M_meshMarkers.end() )
        M_meshMarkers.push_back( mark );
}
void ModelMeasuresFlowRate::setDirection( std::string const& dir )
{
    CHECK( dir == "interior_normal" || dir == "exterior_normal" ) << "invalid dir " << dir;
    M_direction = dir;
}

void ModelMeasuresFlowRate::setup( pt::ptree const& ptree, std::string const& name )
{
