...
fluidMechanics_options(std::string const& prefix)
{
    ...
    fluidOptions.add_options()
        ...
        (prefixvm(prefix,"snes-fix-mean-pressure").c_str(), Feel::po::value< double >()->default_value( -1 ), "mean pressure to fix in Newton solver (default = -1, i.e. don't fix mean pressure) ")
