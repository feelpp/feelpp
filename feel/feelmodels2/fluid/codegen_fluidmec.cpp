...
namespace FeelModels
{
    ...
    FLUIDMECHANICS_CLASS_NAME::FLUIDMECHANICS_CLASS_NAME( bool __isStationary,
                                                          std::string __prefix,
    ...
    FLUIDMECHANICS_CLASS_NAME::loadConfigBCFile()
    {
        ...
        ForEachBC( bcDef, cl::dirichlet_vec,
                   /**/ if (dirichletbcType=="elimination") M_dirichletBCType["elimination"][PhysicalName].push_back(PhysicalName);/*M_dirichletBCType["elimination"].push_back(PhysicalName);*/
                   else if (dirichletbcType=="nitsche")     M_dirichletBCType["nitsche"][PhysicalName].push_back(PhysicalName);//M_dirichletBCType["nitsche"].push_back(PhysicalName);
                   else if (dirichletbcType=="lm")          M_dirichletBCType["lm"][PhysicalName].push_back(PhysicalName);//M_dirichletBCType["lm"].push_back(PhysicalName);
                   else CHECK( false ) << "invalid dirichletbcType "<< dirichletbcType << "\n";
                   //M_meshAleBCType["fixed"].push_back(PhysicalName);
                   this->addMarkerALEMeshBC("fixed",PhysicalName);
                   );
        ForEachBC( bcDef, cl::paroi_mobile,
                   //M_markersNameMovingBoundary.push_back(PhysicalName);
                   /**/ if (dirichletbcType=="elimination") M_dirichletBCType["elimination"][PhysicalName].push_back(PhysicalName);//M_dirichletBCType["elimination"].push_back(PhysicalName);
                   else if (dirichletbcType=="nitsche")     M_dirichletBCType["nitsche"][PhysicalName].push_back(PhysicalName);//M_dirichletBCType["nitsche"].push_back(PhysicalName);
                   else if (dirichletbcType=="lm")          M_dirichletBCType["lm"][PhysicalName].push_back(PhysicalName);//M_dirichletBCType["lm"].push_back(PhysicalName);
                   else CHECK( false ) << "invalid dirichletbcType "<< dirichletbcType << "\n";
                   //M_meshAleBCType["moving"].push_back(PhysicalName);
                   this->addMarkerALEMeshBC("moving",PhysicalName);
                   );
        this->updateBCMarkersListByType();

        ForEachBC( bcDef, cl::neumann_scal,
                   //M_neumannBCType.push_back(PhysicalName);
                   this->addMarkerNeumannBC(PhysicalName);
                   //M_meshAleBCType["fixed"].push_back(PhysicalName);
                   this->addMarkerALEMeshBC("fixed",PhysicalName);
                   );
        ...
        ForEachBC( bcDef, cl::fluid_outlet,
                   /**/ if (M_fluidOutletType=="free")       M_fluidOutletsBCType["free"].push_back(PhysicalName);
                   else if (M_fluidOutletType=="windkessel") M_fluidOutletsBCType["windkessel"].push_back(PhysicalName);
                   else CHECK( false ) << "invalid fluidOutletType "<< M_fluidOutletType << "\n";
                   // M_meshAleBCType is init by default after at free
                   );

        ForEachBC( bcDef, cl::fbm_dirichlet_vec,
                   //M_meshAleBCType["fixed"].push_back(PhysicalName);
                   this->addMarkerALEMeshBC("fixed",PhysicalName);
                   );
    ...
    FLUIDMECHANICS_CLASS_NAME::updateBCDirichletLagMultLinearPDE( vector_ptrtype& F ) const
    {
        ...
        if ( bcDef.hasDirichletVec() )
        {
            ...
            ForEachBC( bcDef, cl::dirichlet_vec,
                       ...
                       /**/ integrate( _range=markedfaces(this->mesh(),PhysicalName), //markedelements(this->meshDirichletLM(),PhysicalName),
