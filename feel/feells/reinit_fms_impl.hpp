...
ReinitializerFMS<FunctionSpaceType, periodicity_type>::operator()( element_type const& phi, bool useMarker2AsDoneMarker ) /*const*/
{
    ...
    auto minNewEntry = []( std::pair<heap_entry_type, size_type> a,
                           std::pair<heap_entry_type, size_type> b ) { // return the entry having the minimum abs(phi) value
