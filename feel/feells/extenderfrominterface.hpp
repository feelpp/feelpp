...
void ExtenderFromInterface<Dim, GeoOrder, Convex>::update( element_type const& phi )
{
    M_phi = phi;
    M_phi.close();
...
template <int Dim, int GeoOrder, template <uint16_type, uint16_type, uint16_type> class Convex>
template <typename Storage>
void ExtenderFromInterface<Dim, GeoOrder, Convex>::extendFromInterface( element_s_type<Storage>& field )
{
