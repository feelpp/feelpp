...
void ConvectionCrb::initModel()
{
    ...
    form2( _test=Xh, _trial=Xh, _matrix=M_Aqm[0][0] )
        += integrate ( markedfaces( mesh, "wall" ),
                       - trans( SigmaNt )*id(v)
                       - trans( SigmaN )*idt(u)
                       + nu*penalbc*trans( idt(u) )*id(v)/hFace() );
    form2( _test=Xh, _trial=Xh, _matrix=M_Aqm[0][0] )
        += integrate ( markedfaces( mesh, "cpu1" ),
                       - trans( SigmaNt )*id(v)
                       - trans( SigmaN )*idt(u)
                       + nu*penalbc*trans( idt(u) )*id(v)/hFace() );
    form2( _test=Xh, _trial=Xh, _matrix=M_Aqm[0][0] )
        += integrate ( markedfaces( mesh, "cpu2" ),
                       - trans( SigmaNt )*id(v)
                       - trans( SigmaN )*idt(u)
                       + nu*penalbc*trans( idt(u) )*id(v)/hFace() );
...
#endif
    form1( Xh, _vector=M_Fqm[0][3][0], _init=true )
        = integrate ( markedfaces( mesh, "inlet"),
    ...
    double cpu2_domain = integrate( markedfaces(mesh,"cpu2"), cst(1.) ).evaluate()(0,0);
