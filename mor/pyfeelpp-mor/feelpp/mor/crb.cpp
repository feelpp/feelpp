...
PYBIND11_MODULE( _mor, m )
{
    ...
    py::class_<ParameterSpaceX,std::shared_ptr<ParameterSpaceX>>(m,"ParameterSpace")
        .def( py::init<>() )
        .def( py::init<std::shared_ptr<ModelProperties>>(), "Initialize from ModelProperties", py::arg("ModelProperties") )
        ...
        .def("mubar", &ParameterSpaceX::mubar, "return mubar the default parameter")
