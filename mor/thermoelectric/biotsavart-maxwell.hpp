...
biotsavart_options()
{
    ...
    bo.add_options()
        ...
        ( "biotsavart.mesh-sequential", po::value<bool>()->default_value(false), "use a sequential box" )
        ;
    return bo;
}
...
BiotSavart<DimB>::BiotSavart(mesh_conductor_ptrtype const& mesh, std::set<std::string> markers)
    : M_meshCond(mesh),
      ...
      M_meshSeq(boption("biotsavart.mesh-sequential"))
{
    tic();
      ...
      M_meshSeq(boption("biotsavart.mesh-sequential"))
{
    ...
    auto comm = M_meshSeq ? Environment::worldCommSeqPtr() : Environment::worldCommPtr();
    int np = M_meshSeq ? 1 : Environment::worldCommPtr()->size();
    M_meshMgn = l.createMesh(_mesh=new mesh_box_type(comm), _name="box", _partitions=np);
      ...
      M_meshSeq(boption("biotsavart.mesh-sequential"))
{
    ...
    auto comm = M_meshSeq ? Environment::worldCommSeqPtr() : Environment::worldCommPtr();
    int np = M_meshSeq ? 1 : Environment::worldCommPtr()->size();
    M_meshMgn = s.createMesh(_mesh=new mesh_box_type(comm), _name="box", _partitions=np);
    this->M_Xh = magneticfield_space_type::New(_mesh=M_meshMgn);
      ...
      M_meshSeq(boption("biotsavart.mesh-sequential"))
{
    ...
    auto comm = M_meshSeq ? Environment::worldCommSeqPtr() : Environment::worldCommPtr();
    int np = M_meshSeq ? 1 : Environment::worldCommPtr()->size();
    M_meshMgn = c.createMesh(_mesh=new mesh_box_type(comm), _name="box", _partitions=np);
    this->M_Xh = magneticfield_space_type::New(_mesh=M_meshMgn);
