...
#include <feel/feelmodels/Rbheat/Rbheat.hpp>
...
makeAbout()
{
    Feel::AboutData about( "rbheatfem" ,
                           "rbheatfem" ,
                           "0.1",
                           "RbHeat FEM model",
 ...
 */
class RbHeatFemApp   : public Application
{
    typedef Application super;
public:

    typedef CRBModel<RbHeat> crbmodel_type;
    typedef boost::shared_ptr<crbmodel_type> crbmodel_ptrtype;

    RbHeatFemApp( int argc, char** argv, AboutData const& ad, po::options_description const& od )
        :
    ...
    {
        //M_crbmodel->solve();
        Feel::ParameterSpace<4>::Element mu( M_crbmodel->parameterSpace() );
        mu <<
           this->vm()["mu1"].as<double>(),
                this->vm()["mu2"].as<double>(),
                this->vm()["mu3"].as<double>(),
                this->vm()["mu4"].as<double>();
...
{
    Feel::RbHeatFemApp app( argc, argv, Feel::makeAbout(), Feel::makeOptions() );
    app.run();
}
