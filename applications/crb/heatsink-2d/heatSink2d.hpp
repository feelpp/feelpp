...
private:
    ...
    std::vector<std::vector<vector_ptrtype> > M_Fq_du;
    ...
    int M_Nsnap;
    Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> M_snapshots_matrix;
    Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> Mdu_snapshots_matrix;
...
void HeatSink2D::init()
{
    ...
    M_fill_snapshots_matrix=false;//by default
...
void HeatSink2D::assemble()
{
    ...
    //mass matrix
    form2( Xh, Xh, M_Mq[0], _init=true ) = integrate ( elements(mesh), idt(u)*id(v) );
    form2( Xh, Xh, M_Mq[0], _init=true ) = integrate ( elements(mesh), idt(u)*id(v) );
    M_Mq[0]->close();
    M_Mq[1]->close();
    ...
    //adjoint need to be implemented
...
double HeatSink2D::output( int output_index, parameter_type const& mu, bool export_outputs )
{
    ...
    *U = *pT;
    ...
    {
        s = M_thetaFq[output_index](1)*dot( M_Fq[output_index][1], U );
    }
