...
void
OpusHeat::initDataStructureAffineDecomposition()
{
...
void
OpusHeat::initializationField( element_ptrtype& initial_field,parameter_type const& mu )
{
    if ( super_type::is_time_dependent )
...
OpusHeat::initModel()
{
    ...
    auto mesh = loadMesh( _mesh=new OpusHeatConfig::mesh_type,
                          //_update=size_type(MESH_UPDATE_FACES_MINIMAL|MESH_NO_UPDATE_MEASURES),
                          _savehdf5=0 );
    this->setFunctionSpaces( OpusHeatConfig::space_type::New( mesh ) );
    ...
    M_surface = integrate( _range=elements( mesh ), _expr=cst( 1. ) ).evaluate()( 0,0 );
    M_bdf = bdf( _space=Xh, _vm=Environment::vm(), _name="opusheat" , _prefix="opusheat" );
...
void
OpusHeat::updateBetaQ_impl( parameter_type const& mu , double time , bool only_terms_time_dependent )
{
    ...
    this->M_betaFq[1][0] = 1./M_surface;
}



OpusHeat::super_type::betaq_type
OpusHeat::computeBetaQ( parameter_type const& mu , double time , bool only_terms_time_dependent )
{
    return computeBetaQ_impl<super_type>( mu, time, only_terms_time_dependent );
}
OpusHeat::super_type::betaq_type
OpusHeat::computeBetaQ( parameter_type const& mu )
{
    return computeBetaQ_impl<super_type>( mu );
}
void
OpusHeat::assembleData()
{
    ...
    if( true ) //boption("do-not-use-operators-free") )
    {
        ...
        auto uconv = Xh->element( vinconv*expr("-(x-0.008)*(x-0.054)*(x>0.008):x" ) );
...
double
OpusHeat::output( int output_index, parameter_type const& mu, element_type &u, bool need_to_solve , bool export_outputs )
{
    using namespace vf;
