...
namespace Feel
{
  /** \page CRB Certified Reduce Basis
   ...
   \section Geo Geometry
      \image html heat2d_geo.png "Geometry of the model" width=3cm
...
Heat2D::initModel()
{
    ...
    a1 = integrate( markedelements( mesh,"SR" ),  gradt( u )*trans( grad( v ) )  )
       - integrate( markedfaces( mesh, "BR"), (gradt(u)*id(v)+grad(v)*idt(u))*N() + 1e4*idt(u)*id(v) );
    this->addLhs( { a1 , "mu0" } );
    /// [lhs1]
    ...
    a2 = integrate( markedelements( mesh,"SL" ),  gradt( u )*trans( grad( v ) )  )
       - integrate( markedfaces( mesh, "BL"), (gradt(u)*id(v)+grad(v)*idt(u))*N() + 1e4*idt(u)*id(v) );
    this->addLhs( { a2 , "mu1" } );
    /// [lhs2]
    ...
    f0 = integrate( markedfaces( mesh,"BR" ), -expr(soption("functions.f"))*(grad(v)*N()+1e4*id(v)) );
    this->addRhs( { f0, "mu0" } );
    /// [rhs1]
    
    /// [rhs2]
    auto f1 = form1( _test=Xh );
    f1 = integrate( markedfaces( mesh,"BL" ), -expr(soption("functions.g"))*(grad(v)*N()+1e4*id(v)) );
    this->addRhs( { f1, "mu1" } );
    /// [rhs2]
    ...
    energy = integrate( markedelements( mesh, "SL" ), gradt( u )*trans( grad( v ) )  )
           - integrate(    markedfaces( mesh, "BR" ), ( (id(v)*gradt(u)+idt(u)*grad(v))*N() + 1e4*idt(u)*id(v) ))
           - integrate(    markedfaces( mesh, "BL" ), ( (id(v)*gradt(u)+idt(u)*grad(v))*N() + 1e4*idt(u)*id(v) ))
...
//CHECK( ! need_to_solve ) << "The model need to have the solution to compute the output\n";
    ...
    if ( output_index == 0 )
    {
        output  = integrate( markedfaces( mesh,"BR" ), -mu(0)*expr(soption("functions.f"))*(gradv(u)*N()+1e4*idv(u)) ).evaluate()(0,0)
                + integrate( markedfaces( mesh,"BL" ), -mu(1)*expr(soption("functions.g"))*(gradv(u)*N()+1e4*idv(u)) ).evaluate()(0,0);
    }
