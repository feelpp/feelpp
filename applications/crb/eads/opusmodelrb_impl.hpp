...
{
    //LOG(INFO) << "[OpusModelRB::computeThetaq] mu = " << mu << "\n";
    double kIC = mu( 0 );
    double D = mu( 1 );
    double Q = mu( 2 );
    double r = mu( 3 );
...
OpusModelRB<OrderU,OrderP,OrderT>::update( parameter_type const& mu , double time )
{
    ...
    *pV = vf::project( M_Th, markedelements( M_Th->mesh(), M_Th->mesh()->markerName( "AIR4" ) ),
    ...
    {
        //LOG(INFO) << "[affine decomp] scale q=" << q << " with " << M_thetaAq(q) << "\n";
    ...
    for ( int l = 0; l < Nl(); ++l )
    {
        ...
        {
            //LOG(INFO) << "[affine decomp] output " << l << " term " << q << "=" << M_thetaL[l](q) << "\n";
