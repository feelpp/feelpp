...
makeAbout()
{
    AboutData about( "laplacian-dg" ,
                     "laplacian-dg" ,
                     "0.2",
                     "nD(n=1,2,3) dG Laplacian on simplices or simplex products",
...
public:
    ...
    /*basis*/
    typedef fusion::vector<Lagrange<Order, Scalar, Discontinuous> > basis_type;
...
Laplacian<Dim, Order, RDim, Entity>::run()
{
    ...
    form2( Xh, Xh, D, _init=true, _pattern=DOF_PATTERN_COUPLED|DOF_PATTERN_NEIGHBOR );
    ...
    form2( Xh, Xh, D ) +=integrate( internalfaces(mesh),
                                    ...
                                    // penal*[u] . [v]/h_face
                                    + M_gammabc* (trans(jumpt(idt(u)))*jump(id(v)) )/hFace() );
    ...
    std::cout << "jump(u) = " <<  integrate( internalfaces(mesh),  trans(jumpv(idv(u)))*(jumpv(idv(u))) ).evaluate()(0,0) << "\n";
    std::cout << "jump(uexact) = " <<  integrate( internalfaces(mesh),  trans(jumpv(g))*(jumpv(g)) ).evaluate()(0,0) << "\n";
