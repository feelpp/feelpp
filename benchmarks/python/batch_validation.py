			...
			for p in code.params:
				if p.getName()==str(d):
					if p.getAttrValues()[1]=="discrete":
						...
						val_depend[str(d)]=map(util.strToSci,util.pickInInterval(float(p.getValues().split(':')[0]),float(p.getValues().split(':')[-1]),options.lnbr,20,options.method))
...
try:
		valide,graphfile,imgs=code.launch(val_param,val_depend,dep_outputs,options.precision,1)
except IOError:
