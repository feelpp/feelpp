...
private:
     ...
     */
    void exportResults( element_type& u, element_type& v, element_type& k, element_type& vbdy );
...
Tilted<Dim, BasisU, Entity>::run()
{
    ...
    auto ke = proj->project( _expr=k, _quad=_Q<5>() );
    ...
    {
        form1( Xh, F ) += integrate( _range=boundaryfaces( mesh ), _expr=-idv(v)*idv(ke)*grad( u )*N() );
        M_stats.put( "t.assembly.vector.dirichlet1",subt.elapsed() );
        subt.restart();
        form1( Xh, F ) += integrate( _range=boundaryfaces( mesh ), _expr=idv(ke)*penalbc*idv(v)*id( v )/hFace() );
    ...
    form2( _trial=Xh, _test=Xh, _matrix=D ) =integrate( _range=elements( mesh ),_expr=idv(ke)*( gradt( u )*trans(grad( v ) ) ) );
    ...
    if ( this->vm()[ "bctype" ].template as<int>() == 1  )
    {
        form2( Xh, Xh, D )+=integrate( _range=boundaryfaces( mesh ),_expr=-idv(ke)*( gradt( u )*N() )*id( v )-idv(ke)*trans( grad( u )*N() )*idt( v ) );
        M_stats.put( "t.assembly.matrix.dirichlet1",subt.elapsed() );
        subt.restart();
        form2( Xh, Xh, D )+=integrate( _range=boundaryfaces( mesh ),_expr=+idv(ke)*penalbc*idt( u )*id( v )/hFace() );
    ...
    {
        form2( Xh, Xh, D ) += on( _range=boundaryfaces( mesh ), _element=u, _rhs=F, _expr=idv(v) );
    ...
    t.restart();
    double u_errorL2 = integrate( _range=elements( mesh ), _expr=(idv( u )-u_exact )*( idv( u )-u_exact), _quad=_Q<5>() ).evaluate()( 0, 0 );
    double u_exactL2 = integrate( _range=elements( mesh ), _expr=u_exact*u_exact, _quad=_Q<5>() ).evaluate()( 0, 0 );
...
template<int Dim, typename BasisU, template<uint16_type,uint16_type,uint16_type> class Entity>
void
Tilted<Dim, BasisU, Entity>::exportResults( element_type& u, element_type& v, element_type& k, element_type& vbdy )
{
    if ( exporter->doExport() )
    {
        ...
        exporter->step( 0 )->add( (boost::format("u_exact_bdy-%1%")%this->level()).str(), vbdy );
        exporter->save();
    }
} // Tilted::export
} // Feel
