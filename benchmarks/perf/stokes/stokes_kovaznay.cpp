...
makeOptions()
{
    Feel::po::options_description stokesKovaznayOptions( "Stokes_Kovaznay options" );
    stokesKovaznayOptions.add_options()
    ...
    ;
    return stokesKovaznayOptions.add( Feel::feel_options() ) ;
}
...
makeAbout()
{
    Feel::AboutData about( "stokes_kovaznay" ,
                           "stokes_kovaznay" ,
                           "0.1",
...
public:
    ...
    /* *********************************************************************
     * BOTTOM : geometrie of the bottom edge
     * 1 : curve
     * 2 : straight (default)
     ************************************************************************ */
    /*
...
template<int POrder, int GeoOrder>
void
Stokes_Kovaznay<POrder,GeoOrder>::init()
{
   ...
   mesh = createGMSHMesh( _mesh=new mesh_type,
    ...
    aa+=on(_range=markedfaces(mesh,"inlet"), _rhs=ll, _element=uu,
          _expr=zero<2,1>() );
    aa+=on(_range=markedfaces(mesh,"outlet"), _rhs=ll, _element=uu,
          _expr=zero<2,1>() );
    aa+=on(_range=markedfaces(mesh,"wall2"), _rhs=ll, _element=uu,
          _expr=zero<2,1>() );
    aa+=on(_range=markedfaces(mesh,"wall1"), _rhs=ll, _element=uu,
...
template<int POrder, int GeoOrder>
void
Stokes_Kovaznay<POrder,GeoOrder>::run()
{
    this->init();
...
template<typename ExprUExact, typename ExprPExact>
void
Stokes_Kovaznay<POrder,GeoOrder>::exportResults( ExprUExact u_exact, ExprPExact p_exact,
                                                       element_type& U, element_type& V )
{
...
main( int argc, char** argv )
{
    ...
    Environment env( _argc=argc, _argv=argv,
                     _desc=makeOptions(),
                     _about=about(_name="stokes_kovaznay",
