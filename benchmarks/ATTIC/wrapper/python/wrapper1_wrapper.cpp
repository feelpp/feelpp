...
#include "wrapper.hpp"
#include "wrapper1app.hpp"

/* WARNING : Please read the following lines
 *
 * In this program, we make the assumption that the end user wishes to
 * call a function (aka NumericalMathFunction) named "wrapper1" (wrapper1 stands
 * for wrapped code).
 * In order to individualize the wrapper to the user's needs, we encourage
 * you (as the developer of this wrapper) to renamed any occurence of "wrapper1"
 * (in either case) to the real name of the function.
 * It will also avoid any confusion with other "wrapper1"s written by other entities
 * or developers.
 */
...
extern "C" {
    ...
    */
     ...
     * 'init_', 'exec_' and 'finalize_' folowed by the name of the function, here 'wrapper1'.
     *
