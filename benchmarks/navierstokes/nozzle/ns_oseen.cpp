...
int main(int argc, char**argv )
{
    ...
    auto intUz = integrate(_range=markedfaces(mesh,"inlet"), _expr=expr( option(_name="functions.g").as<std::string>() )).evaluate()(0,0) ;
    ...
    auto flow = integrate(_range=markedfaces(mesh,"inlet"), _expr=inner(expr( option(_name="functions.g").as<std::string>() )*N(),N())).evaluate()(0,0) ;
    std::cout<<"   Flow = "<< flow;
