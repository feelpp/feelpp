...
#include <life/lifemesh/surface.hpp>
...
public:
    ...
    {
        LIFE_ASSERT( r > 0 )( r ).error( "radius negative" );
        this->setCenter( c );
        this->setRadius( r );
    ...
    {
        LIFE_ASSERT( _M_radius > 0 )( _M_radius ).error( "radius negative" );
        LIFE_ASSERT( other_sphere.radius() > 0 )( other_sphere.radius() ).error( "radius negative" );

        if ( Life::distance( this->center(), other_sphere.center() ) < (this->radius() + other_sphere.radius()) )
            return true;
    ...
    bool aboveSurface (const Point& p) const
    {
        ...
        if ( Life::distance( p, this->center() ) > this->radius() )
            return true;
    ...
    bool onSurface (const Point& p) const
    {
        ...
        if ( std::abs( Life::distance( p, this->center() ) - this->radius()) < 1.e-10)
            return true;
