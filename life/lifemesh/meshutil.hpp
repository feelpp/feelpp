...
size_type findBoundaryEdges( const RegionMesh3D & mesh, TempEdgeContainer & bedges )
{
    ...
    LIFE_ASSERT( mesh.isUpdatedForUse() ).error( "mesh has not been updated for use" );
...
size_type testClosedDomain_Top( RegionMesh3D const & mesh, size_type & numBEdges )
{
    ...
    {
        LIFE_ASSERT( iv != mesh.faces().end() )( k )( mesh.numBFaces() ).error(" Trying to get not existing face" );
...
bool checkIdnumber( const MeshEntityList & list )
{
    ...
    {
        LIFE_ASSERT( l->id() == count )( l->id() )( count ).error( "wrong id" );
        ok &= ( l->id() == count );
    }
    return ok;
}
                 ...
                 TempEdgeContainer * ext_container = 0 )
{
    ...
    if ( buildboundary )
    {
              ...
              ei != bedges->end();++ei )
        {
            ...
            LIFE_ASSERT( __fit != mesh.faces().end() )( facuint16_type ).error( "invalid id for the faces" );
...
#include <life/lifemesh/sphere.hpp>
...
boundingSphere (const MeshType& mesh)
{
    ...
    const double diag = Life::distance( bbox.second, bbox.first);
...
{
    LIFE_ASSERT (mesh.numPoints() != 0).error( "mesh has no points" );
