...
struct LIFE_NO_EXPORT intersection_p
{
...
struct LIFE_NO_EXPORT contains_p
{
...
struct LIFE_NO_EXPORT contained_p
{
    ...
    bool operator()(const node_type& min2, const node_type& max2)
        {
            LIFE_ASSERT( min.size() == min2.size() &&
                         min.size() == max2.size() &&
                         max.size() == min2.size() &&
                         max.size() == max2.size() )
...
struct LIFE_NO_EXPORT has_point_p
{
    has_point_p(const node_type& P_) : P(P_) {}

    bool operator()(const node_type& min2, const node_type& max2)
        {
            LIFE_ASSERT( P.size() == min2.size() &&
                         P.size() == max2.size() )
...
LIFE_NO_EXPORT void findMatchingBoxes( RegionTree::element_base *n,
       ...
       size_type last_dir)
{
    ...
    if ( split_ok )
    {
        ...
        LIFE_ASSERT( cnt1 )( cnt1 ).error( "counter 1 is 0" );
        LIFE_ASSERT( cnt2 )( cnt2 ).error( "counter 2 is 0" );
        LIFE_ASSERT( cnt1+cnt2 >= b.size() )( cnt1 )( cnt2 )( cnt1+cnt2 )( b.size() ).error( "counter sum should be greater or equal to the number of boxes" );
        ...
        LIFE_ASSERT(cnt1 == v1.size())( cnt1 )( v1.size() ).error( "sizes should be equal" );;
        LIFE_ASSERT(cnt2 == v2.size())( cnt2 )( v2.size() ).error( "sizes should be equal" );;
...
RegionTree::build()
{
    ...
    LIFE_ASSERT( _M_root == 0 ).error( "the tree has already been built" );
    ...
    _M_root = Life::build( b, bmin, bmax, 0 );
