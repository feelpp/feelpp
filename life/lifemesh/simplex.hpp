    ...
    std::vector<uint16_type> entity( uint16_type topo_dim, uint16_type id ) const
        {
            ...
            {
                __entity[0] = __f2p[3*id];
                __entity[1] = __f2p[3*id+1];
                __entity[2] = __f2p[3*id+2];
            }
            return __entity;
        }
};
...
public:
     ...
     */
    static uint16_type e2p( uint16_type e,  uint16_type p ) { return edge_to_point_t::e2p( e, p ); }
     ...
     * the point.
     */
    static uint16_type f2p( uint16_type f,  uint16_type p ) { return face_to_point_t::f2p( f, p ); }
