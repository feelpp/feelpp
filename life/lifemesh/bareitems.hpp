...
class BareItemsHandler: public std::map<BareItem, Life::size_type, cmpBareItem<BareItem> >
{
public:
    typedef std::map<BareItem, Life::size_type, cmpBareItem<BareItem> > container;
...
BareItemsHandler<BareItem>::addIfNotThere( const BareItem & s, const size_type id )
{
    ...
    //LIFE_ASSERT( _M_id_count == id )( _M_id_count )( id ).error ( "invalid item id" );
    if ( i.second )
        {
            _M_id_count = id;
            ++_M_id_count;
