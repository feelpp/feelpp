...
#include <life/lifefilters/filterfromvtk.hpp>
...
public:
    ...
    typedef Mesh<GeoEntity<mesh_convex_type> > mesh_type;
...
PointSetToMesh<Convex, T>::visit( pointset_type* pset, mpl::int_<1> )
{
    ...
    LIFE_ASSERT( n_faces == _M_mesh->numFaces() )( n_faces )( _M_mesh->numFaces() ).error( "invalid face container size" );
