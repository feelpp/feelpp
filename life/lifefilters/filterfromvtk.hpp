 ...
 * \class FilterFromVtk
 *
 * Converts Mesh data structure from Vtk library to Life Mesh type.
 *
 * \author Gilles Steiner <gilles.steiner@epfl.ch>
 * \author Christophe Prud'homme <christophe.prudhomme@ujf-grenoble.fr>
 */
...
FilterFromVtk<MeshType>::visit( mesh_type* mesh, mpl::int_<2> )
{
    ...
    LIFE_ASSERT( n_faces == mesh->numFaces() )( n_faces )( mesh->numFaces() ).error( "invalid face container size" );
...
FilterFromVtk3D<MeshType>::visit( mesh_type* mesh, mpl::int_<3> )
{
    ...
    LIFE_ASSERT( mesh->numPoints() == __n )( __n )( mesh->numPoints() ).error( "invalid number of points" );
    size_type n_faces = 0;
