...
/*!
  ...
  * Order the order of derivation
  ...
  Life:::ADType<double,3,2, Var> __g = x/(y*z);
  std::cout << "g=" << __g << "\n";
  <pre>
...
public:
    ...
    typedef typename Life::STL::SFlatten<typename SListGenerator<Nvar,ADVariable>::list_type>::Result variable_list_type;
...
ADType<T,Nvar, 2, Var>::operator=(const ADExpr<ExprT>& expr)
{
    ...
    typedef typename Life::STL::SNoDuplicates<typename Life::STL::SFlatten<typename ADExpr<ExprT>::Var>::Result>::Result Expr;
    typedef typename Life::Meta::IF<Life::STL::SIndexOf<Expr,Variable<-1> >::value == -1,
		FOR_EACH<Expr>,
		depManager
		>::Result ForEachExpr;
...
template <
    ...
    >
Life::SDebugLifeream&
operator << ( Life::SDebugLifeream& __os, const Life:::ADType<T, Nvar, 2, Var>& a )
{
...
std::ostream&
operator << ( std::ostream& os, const Life:::ADType<T, Nvar, 2, Var>& a)
{
