                                                    ...
                                                    double *_v_til )
{
	...
	for( int i = n; i < nv; i++ )
	    if( _vN_til[ i ] < 0 )
            theta1 = std::min( theta1, -tau/2./_vN_til[ i ] ); assert( theta1 > 0 );
            ...
            if( gamma <= 0 )
                {
                    ...
                    double thetaa = ( -b + std::sqrt( b*b - 4.*a*c ) ) / (2.*a);  assert( thetaa > 0 );
                    for( int i = 0; i < nw; i++ )
                    ...
                    if( __Dot( nw, wP_til, wP_til ) + __Dot( nw, _v_til, _v_til ) > _Delta*_Delta )
                        {
                            ...
                            double b = 2. * __Dot( nw, w_til, p );
                            double c = __Dot( nw, w_til, w_til ) + __Dot( nw, _v_til, _v_til ) - _Delta*_Delta;
                            double thetab = ( -b + std::sqrt( b*b - 4.*a*c ) ) / (2.*a);  assert( thetab > 0 );
                            for( int i = 0; i < nw; i++ )
	...
	if( !( this->slack_feasible( wP_til, _v_til ) ) )
        {
            ...
            double thetac = ( -b + std::sqrt( b*b - 4.*a*c ) ) / (2.*a);  assert( thetac > 0 );
            double tau = options.tau;
            truss_exit = true;
            for( int i = n; i < nw; i++ )
