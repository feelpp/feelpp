...
public:
	...
	numerical_type   value( int __i ) const { return M_val[ __i ]; }
    numerical_type & value( int __i )       { return M_val[ __i ]; }

    gradient_type const& gradient() const { return M_grad; }
    gradient_type &      gradient()       { return M_grad; }

    numerical_gradient_type & gradient( int __i )       { return M_grad[ __i ]; }
    numerical_gradient_type   gradient( int __i ) const { return M_grad[ __i ]; }

    numerical_type & gradient( int __i, int __j )       { return M_grad[ __i](__j ); }
    numerical_type   gradient( int __i, int __j ) const { return M_grad[ __i](__j ); }

    hessian_type const& hessian() const { return M_hess; }
    hessian_type &      hessian()       { return M_hess; }

    numerical_hessian_type const& hessian( int __i ) const { return M_hess[ __i ]; }
    numerical_hessian_type &      hessian( int __i )       { return M_hess[ __i ]; }

    numerical_type & hessian( int __i, int __j, int __k )       { return M_hess[__i]( __j, __k ); }
    numerical_type   hessian( int __i, int __j, int __k ) const { return M_hess[__i]( __j, __k ); }
...
public:
	...
	*/
	...
	int n() const { return _E_n; }

	//! \return size of the matrix
	int nA() const { return _E_nA; }

	//! \return lower bound of control variable \c i
	value_type x_l( unsigned int i ) const { return M_xlower(  i  ); }

	//! \return upper bound  of control variable \c i
	value_type x_u( unsigned int i ) const { return M_xupper(  i  ); }

    vector_type const& lowerBounds() const { return M_xlower; }
    vector_type const& upperBounds() const { return M_xupper; }

    vector_type distanceToLowerBounds( vector_type const& __x ) const { return __x - M_xlower; }
    vector_type distanceToUpperBounds( vector_type const& __x ) const { return M_xupper - __x; }

	/**
	   test if __x_definitions is on
	   \return \c true if greater in absolute value to \c _S_on_flag, \c false otherwise
	*/
	bool isOn ( int __i ) const { return  __x_definitions[__i][0] >= value_type(.5); }
