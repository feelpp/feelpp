...
public :
    ...
    std::vector<weights_type> const& allfweights() const {return _M_w_face;}

    std::vector<nodes_type> const& allfpoints() const {return _M_n_face;}
                          ...
                          Expression const& f ) const
    {
        ...
        LIFE_ASSERT(  __face == ALL_FACES || ( __face >= 0 && __face < nFaces() ) )
                                  ...
                                  Expression const& f ) const
    {
        ...
        //std::cout << "integrating using " << nNodes << "\n";
        LIFE_ASSERT(  int(__face) == int(ALL_FACES) ||
                      ( int(__face) >= 0 &&
...
namespace std
{
template<class Convex, Life::uint16_type Integration_Degree, typename T>
std::ostream& operator<<( std::ostream& os,
                          Life::PointSetQuadrature<Convex,Integration_Degree,T> const& qps )
{
    os << "quadrature point set:\n"
       << "number of points: " << qps.nPoints() << "\n"
       << "points :  " << qps.points() << "\n"
       << "weights :  " << qps.weights() << "\n";
    for ( Life::uint16_type f = 0;f < qps.nFaces(); ++f )
        {
            os << " o Face " << f << "\n";
            os << qps.fpoints(f) << "\n";
            os << qps.weights(f) << "\n";
