...
namespace SmartAssert
{
inline ::Life::Assert make_assert( const char * expr)
{
    return ::Life::Assert( expr);
}
} // namespace SmartAssert

} // Life Namespace

#ifdef LIFE_SMART_ASSERT_DEBUG_MODE

#if LIFE_SMART_ASSERT_DEBUG_MODE == 1
#define LIFE_SMART_ASSERT_DEBUG
#else
#undef LIFE_SMART_ASSERT_DEBUG
#endif
...
#ifdef LIFE_SMART_ASSERT_DEBUG
// "debug" mode
#define LIFE_SMART_ASSERT( expr) \
    if ( (expr) ) ; \
    else ::Life::SmartAssert::make_assert( #expr).printContext( __FILE__, __LINE__).SMART_ASSERT_A \
    /**/
...
#define LIFE_SMART_ASSERT( expr) \
    if ( true ) ; \
    else ::Life::SmartAssert::make_assert( "").SMART_ASSERT_A \
    /**/

#endif // ifdef LIFE_SMART_ASSERT_DEBUG

// LIFE_ASSERT is a equivalent to LIFE_SMART_ASSERT
#define LIFE_ASSERT( expr) LIFE_SMART_ASSERT(expr)


#define LIFE_SMART_VERIFY( expr) \
    if ( (expr) ) ; \
    else ::Life::SmartAssert::make_assert( #expr).error().printContext( __FILE__, __LINE__).SMART_ASSERT_A \
    /**/
#define LIFE_VERIFY( expr) LIFE_SMART_VERIFY(expr)


#define SMART_ASSERT_A(x) LIFE_SMART_ASSERT_OP(x, B)
#define SMART_ASSERT_B(x) LIFE_SMART_ASSERT_OP(x, A)

#define LIFE_SMART_ASSERT_OP(x, next) \
