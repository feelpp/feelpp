...
#include <gmp.h>
#include <mpfr.h>
#include <life/lifecore/mpfr-impl.hpp>
...
class MpfrClass {
  ...
  public:
   ...
   MpfrClass (unsigned long int i,
    ...
    // Is equal to zero?
    friend bool iszero(const MpfrClass& r) { return ( (MPFR_IS_NAN(r.mpfr_rep) != 0) && (MPFR_NOTZERO(r.mpfr_rep) != 0) ); }
    ...
    operator int() const{ return to_int( *this ); }
    operator long int() const{ return to_int( *this ); }
    operator unsigned long int() const{ return to_uint( *this ); }
    operator double() const{ return to_double( *this ); }
    operator real96_type() const{ return to_ldouble( *this ); }
