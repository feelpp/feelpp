...
public:
    ...
    {
        LIFE_ASSERT(this->closed()).error( "vector is not closed" );
    ...
    {
        LIFE_ASSERT (this->isInitialized()).error( "VectorPetsc not initialized" );
     ...
     * calling VecDestroy()!
     */
    Vec vec () const { LIFE_ASSERT (_M_vec != 0).error( "invalid petsc vector" ); return _M_vec; }
...
public:
    ...
    VectorPetsc( VectorPetsc const & v)
        ...
        _M_destroy_vec_on_exit( true )
    {
        LIFE_ASSERT(v.closed()).error( "copied vector is not closed" );
                      ...
                      const bool fast)
{
    ...
    // otherwise create an MPI-enabled vector
    else
        {
            ...
            ierr = VecCreateMPI (Application::COMM_WORLD, petsc_n_local, petsc_n,
                                 &_M_vec);
            CHKERRABORT(Application::COMM_WORLD,ierr);
...
{
    LIFE_ASSERT(i<size())( i )( size() ).error( "invalid index" );
