...
{
    checkInvariant();
    LIFE_ASSERT( this->localSize() == V.localSize() )( this->localSize() )( V.localSize() ).error ( "invalid vector size" );
    LIFE_ASSERT( this->firstLocalIndex() == V.firstLocalIndex() &&
    ...
    if ( Application::processId() == 0 )
        {
            ...
            LIFE_ASSERT( file_out)( filename ).error("[VectorUblas::printMatlab] ERROR: File cannot be opened for writing.");
...

{
    ...
    LIFE_ASSERT( v_local != 0 ).error ( "dynamic_cast failed: invalid vector object" );
    ...
    LIFE_ASSERT (this->localSize() == this->size())( this->localSize() )( this->size() ).error( "invalid size in non MPI mode" );
                                  ...
                                  const std::vector<size_type>& send_list)
{
    ...
    LIFE_ASSERT (this->size() == this->localSize())( this->size() )( this->localSize() ).error("invalid local/global size" );
    LIFE_ASSERT (last_local_idx > first_local_idx)( last_local_idx )( first_local_idx ).error("invalid first/last local indices");
    LIFE_ASSERT (send_list.size() <= this->size())( send_list.size() )( this->size() ).error("invalid send list size" );
    LIFE_ASSERT (last_local_idx < this->size())( last_local_idx )( this->size() ).error( "invalid last local index" );
    Life::detail::ignore_unused_variable_warning(send_list);
                                                 ...
                                                 const size_type pid) const
{
    ...
    LIFE_ASSERT ( this->localSize() == this->size())( this->localSize() )( this->size() ).error( "invalid size in non MPI mode" );
    LIFE_ASSERT ( pid == 0  )( pid ).error( "invalid pid in non MPI mode" );
...
VectorUblas<T,Storage>::checkInvariant() const
{
    ...
    LIFE_ASSERT ( this->localSize() <= this->size() )
        ( this->size() )( this->localSize() ).error( "vector invalid size" );
    LIFE_ASSERT (_M_vec.size() == this->localSize())
        (_M_vec.size())(this->localSize()).error( "vector invalid size" );
    LIFE_ASSERT ((this->lastLocalIndex() - this->firstLocalIndex() ) == this->localSize())
        (this->size())
        (this->lastLocalIndex())
