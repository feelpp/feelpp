...
po::options_description backendtrilinos_options( std::string const& prefix )
{
    ...
    _options.add_options()
        // solver options
        ((_prefix+"trilinos-solver-type").c_str(), Life::po::value<std::string>()->default_value( "gmres" ), "cg, bicgstab, gmres, cg-cond, gmres-cond")

        // preconditioner options
        ((_prefix+"trilinos-pc-type").c_str(), Life::po::value<std::string>()->default_value( "block-jacobi" ), "id, domain-decomposition, block-jacobi, neumann")
        ((_prefix+"trilinos-drop").c_str(), Life::po::value<double>()->default_value( 1e-3 ), "drop tolerance for preconditioners")
        ((_prefix+"trilinos-fillin").c_str(), Life::po::value<int>()->default_value( 2 ), "fill-in level value for preconditioners")
        ((_prefix+"trilinos-overlap").c_str(), Life::po::value<int>()->default_value( 2 ), "domain overlap value for preconditioners")
        ((_prefix+"trilinos-overlap-type").c_str(), Life::po::value<std::string>()->default_value( "symmetric" ), "type of domain decomposition overlap for preconditioners")
        ...
        ((_prefix+"trilinos-restart").c_str(), Life::po::value<int>()->default_value( 20 ), "number of iterations before solver restarts (gmres)")
        ((_prefix+"trilinos-verbose").c_str(), Life::po::value<std::string>()->default_value( "none" ), "(none, summary, last, all) print solver iterations")
        ((_prefix+"trilinos-maxiter").c_str(), Life::po::value<int>()->default_value( 1000 ), "set maximum number of iterations")
        ((_prefix+"trilinos-tolerance").c_str(), Life::po::value<double>()->default_value( 2e-10 ), "set solver tolerance")
        ((_prefix+"trilinos-residual").c_str(), Life::po::value<std::string>()->default_value( "rhs" ), "set solver residual calculation")
        ;
    return _options;
}
...
// -- SETTING OF OPTIONS --
void
BackendTrilinos::set_maxiter ( int maxiter ) { M_options.set("max_iter", maxiter); }

void
BackendTrilinos::set_tol ( double tol ) { M_options.set("tol", tol); }
...
void
BackendTrilinos::set_restart ( int restart ) { M_options.set("kspace", restart); }
...
void
BackendTrilinos::set_order ( int order ) { M_options.set("poly_order", order );}

void
BackendTrilinos::set_overlap_type( std::string str ) { M_options.set("type_overlap", str );}

void
BackendTrilinos::set_residual ( std::string str ) { M_options.set("conv", str); }

void
BackendTrilinos::set_localparts ( int localparts ) { M_options.set("partitioner: local parts", localparts); }
...
void
BackendTrilinos::set_prec_local_solver( std::string str ) { M_local_solver = str; }

void
BackendTrilinos::set_prec_local_solver_type( std::string str ) { M_local_solver_type = str; }
...
void
BackendTrilinos::set_options( list_type opts ) { M_options = opts; }

BackendTrilinos::list_type
BackendTrilinos::get_options() { return M_options; }
