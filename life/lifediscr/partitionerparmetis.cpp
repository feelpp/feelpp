...
{
    LIFE_ASSERT( mesh.isUpdatedForUse() ).error( "invalid call, must call mesh.updateForUse first" );
                                       ...
                                       const unsigned int n_sbdmns)
{
    ...
    {
        LIFE_ASSERT (M_vtxdist.size() == n_procs+1)
    ...
    for (unsigned int proc_id=0; proc_id<Application::nProcess(); proc_id++)
        {
            ...
            for (; elem_it != elem_end; ++elem_it)
                {
                    LIFE_ASSERT ( elem_it->id() < M_forward_map.size() ).error( "invalid element id" );
                    LIFE_ASSERT ( M_forward_map[elem_it->id()] == invalid_size_type_value );
    ...
    LIFE_ASSERT (el_num       == n_active_elem)( el_num )( n_active_elem ).error( "invalid element number" );
    LIFE_ASSERT (local_el_num == n_active_local_elem)( local_el_num )( n_active_local_elem ).error( "invalid local element number" );
}
    ...
    for (; elem_it != elem_end; ++elem_it)
        {
            ...
            LIFE_ASSERT (elem->id() < M_forward_map.size()).error( "element id and forward_map incompatible" );
            LIFE_ASSERT (M_forward_map[elem->id()] != invalid_size_type_value ).error( "forward_map problem" );
                    ...
                    if (neighbor != NULL)
                        {
                            ...
                            if (neighbor->active())
                                {
                                    LIFE_ASSERT (neighbor->id() < M_forward_map.size())
                                        ( neighbor->id() )( M_forward_map.size() ).error( "problem with neighbor id and forward_map" );
                                    LIFE_ASSERT (M_forward_map[neighbor->id()] != invalid_size_type_value )
    ...
    for (; elem_it != elem_end; ++elem_it)
        {
            ...
            LIFE_ASSERT ( elem.id() < M_forward_map.size() )( elem.id() )( M_forward_map.size() ).error( "invalid size" );
            LIFE_ASSERT ( M_forward_map[elem.id()] != invalid_size_type_value )( M_forward_map[elem.id()] ).error( "invalid forward map" );
            LIFE_ASSERT (M_forward_map[elem.id()] < M_part.size())
...
//
template class PartitionerParmetis<Mesh<GeoEntity<Simplex<1,1> > > >;
template class PartitionerParmetis<Mesh<GeoEntity<Simplex<2,1> > > >;
#if LIFE_MESH_MAX_ORDER >= 2
template class PartitionerParmetis<Mesh<GeoEntity<Simplex<2,2> > > >;
#elif LIFE_MESH_MAX_ORDER >= 3
template class PartitionerParmetis<Mesh<GeoEntity<Simplex<2,3> > > >;
#elif LIFE_MESH_MAX_ORDER >= 4
template class PartitionerParmetis<Mesh<GeoEntity<Simplex<2,4> > > >;
#elif LIFE_MESH_MAX_ORDER >= 5
template class PartitionerParmetis<Mesh<GeoEntity<Simplex<2,5> > > >;
#endif


template class PartitionerParmetis<Mesh<GeoEntity<Simplex<3,1> > > >;
#if LIFE_MESH_MAX_ORDER >= 2
template class PartitionerParmetis<Mesh<GeoEntity<Simplex<3,2> > > >;
#endif

template class PartitionerParmetis<Mesh<GeoEntity<SimplexProduct<1,1> > > >;
template class PartitionerParmetis<Mesh<GeoEntity<SimplexProduct<2,1> > > >;
template class PartitionerParmetis<Mesh<GeoEntity<SimplexProduct<3,1> > > >;
