    ...
    for ( Iterator it = begin_elt; it != end_elt; ++it )
        {
            ...
            for (unsigned int n=0; n < old_elem.nPoints(); n++)
                {
                    LIFE_ASSERT (old_elem.point( n ).id() < new_node_numbers.size()).error( "invalid point id()" );
                    ...
                    if ( new_node_numbers[old_elem.point(n).id()] == invalid_size_type_value )
                        {
                            ...
                            if ( n < element_type::numVertices )
                                {
                                    LIFE_ASSERT( new_vertex[old_elem.point(n).id()] == 0 ).error( "already seen this point?" );
                                    new_vertex[old_elem.point(n).id()]=1;
                                }
                        }

                    // Define this element's connectivity on the new mesh
                    LIFE_ASSERT (new_node_numbers[old_elem.point(n).id()] < new_mesh.numPoints()).error("invalid connectivity");
