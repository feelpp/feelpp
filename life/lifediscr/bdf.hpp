...
Bdf<SpaceType>::initialize( unknowns_type const& uv0 )
{
    ...
    //LIFE_ASSERT( uv0.size() == uint16_type(_M_order) ).error( "Initial data set are not enough for the selected BDF" );
...
{
    LIFE_ASSERT( i < size_type(n + 1) ).error( "Error in specification of the time derivative coefficient for the BDF formula (out of range error)" );
...
{
    LIFE_ASSERT( i < n ).error( "Error in specification of the time derivative coefficient for the BDF formula (out of range error)" );
...
Bdf<SpaceType>::derivate( int n ) const
{
    ...
    LIFE_ASSERT( __t.size() == _M_space->nDof() )( __t.size() )( _M_space->nDof() ).error( "invalid space element size" );
    LIFE_ASSERT( __t.size() == _M_unknowns[0]->size() )( __t.size() )( _M_unknowns[0]->size() ).error( "invalid space element size" );
