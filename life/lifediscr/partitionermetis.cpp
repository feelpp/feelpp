                                      ...
                                      const uint16_type n_pieces )
{
    ...
    // contiguous range.
    {
        ...
        for (; elem_it != elem_end; ++elem_it)
            {
                LIFE_ASSERT (elem_it->id() < forward_map.size())( elem_it->id() )( forward_map.size() ).error( "invalid dimensions" );

                forward_map[elem_it->id()] = el_num++;
            }

        LIFE_ASSERT (el_num == n_elem)( el_num )( n_elem ).error( "incompatible number of elements" );
    }
        ...
        for (; elem_it != elem_end; ++elem_it)
            {
                ...
                LIFE_ASSERT (elem->id() < forward_map.size()).error( "element id and forward_map incompatible" );
                LIFE_ASSERT (forward_map[elem->id()] != invalid_size_type_value ).error( "forward_map problem" );
                ...
                LIFE_ASSERT( counter >= 1 )( elem->id() )( counter ).error( "invalid neighboring data" );
                ...
                for (uint16_type ms=0; ms < mesh.numLocalFaces(); ms++)
                    {
                        ...
                        if ( neighbor_id != invalid_size_type_value )
                            {
                                LIFE_ASSERT (neighbor_id < forward_map.size())
                                ( neighbor_id )( forward_map.size() ).error( "problem with neighbor id and forward_map" );
                                LIFE_ASSERT (forward_map[neighbor_id] != invalid_size_type_value )
        ...
        for (size_type i = 0; i < mesh.numElements(); ++i )
            {
                ...
                LIFE_ASSERT ( elem.id() < forward_map.size() )( elem.id() )( forward_map.size() ).error( "invalid size" );
                LIFE_ASSERT ( forward_map[elem.id()] != invalid_size_type_value )( forward_map[elem.id()] ).error( "invalid forward map" );;
                ...
                for (uint16_type ms=0; ms < mesh.numLocalFaces(); ms++)
                    {
                        ...
                        if ( neighbor_id != invalid_size_type_value )
                            {
                                LIFE_ASSERT ( neighbor_id < forward_map.size() )( neighbor_id )( forward_map.size() ).error( "invalid size" );
                                LIFE_ASSERT ( forward_map[neighbor_id] != invalid_size_type_value )( forward_map[neighbor_id] ).error( "invalid forward map" );
        ...
        for (; face_it != face_end; ++face_it)
            {
                ...
                if ( face.isOnBoundary() )
                    {
                        LIFE_ASSERT( face.isConnectedTo0() &&
                                     !face.isConnectedTo1() )
                ...
                LIFE_ASSERT( face.element(0).facePtr( face.pos_first() ) )
                    ( face.ad_first() )( face.pos_first() )( face.element(0).id() ).error( "invalid face in element" );
                if ( face.isConnectedTo1() )
                    LIFE_ASSERT( face.element(1).facePtr( face.pos_second() ) )
