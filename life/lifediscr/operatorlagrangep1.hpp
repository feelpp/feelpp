    ...
    _M_p2m()
{
            ...
            for( ; itl != enl; ++itl, ++elid )
                {
                    ...
                    for( int p = 0; p < image_mesh_type::element_type::numVertices; ++p )
                        {
                            ...
                            size_type ptid = boost::get<0>(this->domainSpace()->dof()->localToGlobal( it->id(),
                                                                                                      localptid_dof, 0 ));
                            LIFE_ASSERT( ptid < this->domainSpace()->nLocalDof()/domain_space_type::nComponents )( ptid )( this->domainSpace()->nLocalDof()/domain_space_type::nComponents ).warn( "invalid domain dof index" );
                            ...
                            LIFE_ASSERT( ublas::norm_2( boost::get<0>(this->domainSpace()->dof()->dofPoint( ptid ))-__pt.node()) < 1e-10 )
                                (boost::get<0>(this->domainSpace()->dof()->dofPoint( ptid )))
                                (__pt.node())
                                (elid)
                                (itl->id())
                            ...
                            //LIFE_ASSERT( ublas::norm_2( _M_mesh->point( ptid ).node(), __pt.node() ) ).error("invalid point");
#if 1
                            LIFE_ASSERT( ublas::norm_2( boost::get<0>(this->domainSpace()->dof()->dofPoint( ptid ))-elt.point(p).node()) < 1e-10 )
                                (boost::get<0>(this->domainSpace()->dof()->dofPoint( ptid )))
                                (p)
                                (elt.point(p).node())
                                (elid)
                            ...
                            LIFE_ASSERT( ublas::norm_2( elt.point(p).node()-ublas::column( elt.G(), p ) ) < 1e-10 )
                                (p)
                                (elt.point(p).node())
                                (elid)
                    ...
                    LIFE_ASSERT( meas_times > 0 )( elt.id() )( elt.G() ).warn( "negative area" );
#else
                    //LIFE_ASSERT( elt.isAnticlockwiseOriented() )( elt.id() )( elt.G() ).warn( "negative area" );
                    //LIFE_ASSERT( ublas::norm_inf( elt.G()- it->G() ) < 1e-10 )( it->G() )( elt.G() ).warn( "global: not same element" );
                    //LIFE_ASSERT( ublas::norm_inf( elt.G()- itl->G() ) < 1e-10 )( itl->G() )( elt.G() ).warn( "local: not same element" );
#endif
    ...
    for( size_type i = 0; i < this->domainSpace()->nLocalDof()/domain_space_type::nComponents; ++i )
        {
            ...
            LIFE_ASSERT(ublas::norm_2( boost::get<0>(this->domainSpace()->dof()->dofPoint( i ))-
    ...
    for( size_type i = 0; i < this->domainSpace()->nLocalDof()/domain_space_type::nComponents; ++i )
        {
            LIFE_ASSERT( boost::get<1>(this->domainSpace()->dof()->dofPoint( i )) ==
                                                   ...
                                                   mpl::bool_<true> ) const
{
    ...
    if ( domain_mesh_type::nDim == 2 &&
         ...
         it->edgePermutation( _M_pset.pointToEntity( localptid ).second ) == edge_permutation_type::REVERSE_PERMUTATION )
        {
            ...
            LIFE_ASSERT( l != invalid_uint16_type_value && ( l < domain_fe_type::nDofPerEdge ) )
                ( l )
            ...
            LIFE_ASSERT( localptid_dof != invalid_uint16_type_value &&
