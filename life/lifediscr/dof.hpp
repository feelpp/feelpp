...
public:
    ...
    typedef boost::tuple<typename node<value_type>::type, size_type, uint16_type > dof_point_type;
...
/**
     ...
     *  \param mesh mesh
     */
    void buildDofMap( mesh_type& mesh );
        ...
        if ( is_periodic )
            {
                // TODO
#if 0
                if ( M_periodic_elements_localdof.find( boost::make_tuple( ie, lc_dof ) ) )
                    _M_el_l2g[ ie][ lc_dof ] = boost::make_tuple(itdof->second, sign, true );
                else
                    _M_el_l2g[ ie][ lc_dof ] = boost::make_tuple(itdof->second, sign, false );
#endif
            }
        else
            {
...
template<typename MeshType, typename FEType, typename PeriodicityType>
void
Dof<MeshType, FEType, PeriodicityType>::buildDofMap( mesh_type& M )
{
    if ( !M_dof_indices.empty() )
        {
            generateDofPoints( M );
    ...
    std::list<element_type const*> periodic_elements;
    ...
    for (size_type processor=0; processor<n_proc; processor++)
        {
                    ...
                    for (;it_elt!=en_elt; ++it_elt )
                        {
                            ...
                            // check if we have periodic dof, if yes then store the elements which have a face
                            // marked Tag2 (see periodic.cpp)
                            if ( is_periodic )
                                {
                                    typename element_type::face_const_iterator it, en;
                                    // TODO
                                    //boost::make_tuple( it, en ) = it_elt.faces();

                                    bool found_periodic_face_in_element_tag2 = false;
                                    for( ;it != en; ++it )
                                        {
                                            // TODO
                                            //if ( (*it)->marker() == periodicity_type::tag2 )
                                                {
                                                    ...
                                                    periodic_elements.push_back( boost::addressof(__elt) );
                                                    found_periodic_face_in_element_tag2 = true;
                                                    break;
                                                }
                                            // mark the tag1 faces dof as periodic
                                            // TODO
                                             //if ( (*it)->marker() == periodicity_type::tag1 )
                                                {
                                                    // TODO : store the element and local dof id for further
                                                    // reference when inserting the associated global dof
                                                    // id of the element adjacent to the face

                                                    size_type iElAd = (*it)->ad_first();
                                                    LIFE_ASSERT( iElAd != invalid_size_type_value )( (*it)->id() ).error( "invalid face/element in face" );
                                                    Life::detail::ignore_unused_variable_warning(iElAd);

                                                    // local id of the face in its adjacent element
                                                    uint16_type iFaEl = (*it)->pos_first();
                                                    LIFE_ASSERT( iFaEl != invalid_uint16_type_value ).error ("invalid element index in face");
                                                            ...
                                                            for ( uint16_type l = 0; l < fe_type::nDofPerVertex; ++l )
                                                                {
                                                                    ...
                                                                    //M_periodic_elements_localdof[ std::make_tuple(it_elt->id(), lid,c) ] = true;
                                                                }

                                                        }
                                                }
                                        }
                                    // if a periodic face is found, then go to next element
                                    if ( found_periodic_face_in_element_tag2 )
                                        break;
                                }
                            /*
            ...
            // finally deal with the remaining work on periodic elements
            if ( is_periodic )
                {
                    // now go through the remaining elements which were flagged periodic with
                    // marker Tag2
                    typedef std::list<element_const_iterator const*> list_element_iterator;
                    // TODO
#if 0
                    list_element_iterator it = periodic_elements.begin(), en = periodic_elements.end();
                    for( ;it != en; ++it )
                        {
