...
if [ -x /usr/bin/g++-4.5 ]; then
    $COMMON,FEELPP_CXXNAME=g++-4.5,FEELPP_CXX=/usr/bin/g++-4.5,FEELPP_EXPLICIT_VECTORIZATION=SSE2
fi
if [ -x /usr/bin/g++-4.6 ]; then
    $COMMON,FEELPP_CXXNAME=g++-4.6,FEELPP_CXX=/usr/bin/g++-4.6,FEELPP_EXPLICIT_VECTORIZATION=SSE2
fi
if [ -x /usr/bin/g++-4.7 ]; then
    $COMMON,FEELPP_CXXNAME=g++-4.7,FEELPP_CXX=/usr/bin/g++-4.7,FEELPP_EXPLICIT_VECTORIZATION=SSE2
fi
if [ -x /usr/bin/g++-4.8 ]; then
    $COMMON,FEELPP_CXXNAME=g++-4.8,FEELPP_CXX=/usr/bin/g++-4.8,FEELPP_EXPLICIT_VECTORIZATION=SSE2
fi
