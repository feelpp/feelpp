...
macro ( om_add_model )
  ...
  foreach( srcs ${OM_MODEL_SRCS} )
    set( OMWRAPPER_SRCS_FULLPATH ${CMAKE_CURRENT_SOURCE_DIR}/${srcs} ${OMWRAPPER_SRCS_FULLPATH} )
  endforeach()
  ...
  find_path( OMWRAPPER_MACRO_DIR feelpp.macros.om.cmake
   ...
   if( NOT EXISTS ${OMWRAPPER_LIBDIR}/${OMWRAPPER_NAME}.c )
     execute_process( COMMAND ${OMC_COMPILER} -s ${OMWRAPPER_SRCS_FULLPATH}  WORKING_DIRECTORY ${OMWRAPPER_LIBDIR} )
   endif()

   #rewrite some parts of the makefile generated by omc
   set( OMWRAPPER_MAKEFILE ${OMWRAPPER_NAME}.makefile )
   file( READ ${OMWRAPPER_LIBDIR}/${OMWRAPPER_MAKEFILE} om_makefilestring )
   ...
   file( WRITE ${OMWRAPPER_LIBDIR}/${OMWRAPPER_MAKEFILE} ${om_makefilestring})

   #generate custom header
   string( TOUPPER ${OMWRAPPER_NAME} OM_UP_NAME )
   configure_file( ${OMWRAPPER_MACRO_DIR}/omwrapper.hpp.in ${OMWRAPPER_LIBDIR}/feel${OMWRAPPER_NAME}.hpp )
   configure_file( ${OMWRAPPER_MACRO_DIR}/omwrapper.cpp.in ${OMWRAPPER_LIBDIR}/feel${OMWRAPPER_NAME}.cpp )

   set( libname feelpp_omwrapper_${OMWRAPPER_NAME} )
   add_library( ${libname} SHARED ${OMWRAPPER_LIBDIR}/feel${OMWRAPPER_NAME}.cpp )

   set( ${OM_MODEL_LIB} ${OMWRAPPER_LIBDIR}/lib${OMWRAPPER_NAME}.so ${libname} )

   include_directories( ${OMWRAPPER_LIBDIR} )

   add_custom_command(TARGET feelpp_om_add_${OMWRAPPER_NAME}
    COMMAND ${CMAKE_COMMAND} -DOMWRAPPER_NAME=${OMWRAPPER_NAME} -DOMWRAPPER_LIBDIR=${OMWRAPPER_LIBDIR} -DOMWRAPPER_MAKEFILE=${OMWRAPPER_MAKEFILE} -P "${OMWRAPPER_MACRO_DIR}/feelpp.macros.om.cmake"
    )
