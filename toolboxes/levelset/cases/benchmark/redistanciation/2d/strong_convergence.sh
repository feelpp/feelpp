...
do
    TIMER_FILE=${ROOT_DIR}/redistanciation_2d_timer_h_${HSIZE}.csv
    CMD="${RUN_SCRIPT} ${HSIZE} ${NCORE} ${TIMER_FILE}"
    #eval "${CMD} &"
    #printf "sbatch -n ${NCORE} -N 1-1 ${EXCLUSIVE} ${EXCLUDE} --ntasks-per-core 1 --job-name=\"2D_h_${HSIZE}_np_${NCORE}_Z_redistanciation_benchmark\" --mail-type=END --mail-user=derhovsepian@math.unistra.fr -t 48:00:00${CMD}" 
    sbatch -n ${NCORE} -N 1-1 ${EXCLUSIVE} ${EXCLUDE} --ntasks-per-core 1 --job-name="2D_h_${HSIZE}_np_${NCORE}_Z_redistanciation_benchmark" --mail-type=END --mail-user=derhovsepian@math.unistra.fr -t 48:00:00 ${CMD} 
