#!/bin/bash
FM_BIN=/ssd/derhovsepian/feelpp/Release_build_benchmark_zalesak/toolboxes/levelset/cases/benchmark/redistanciation/feelpp_toolbox_benchmark_fast_marching_3d
NARGS=3
CFG_DIR=/home/u2/derhovsepian/git/feelpp/toolboxes/levelset/cases/benchmark/redistanciation/3d
...
CMD="(time mpirun -np ${NCORE} ${BIND_TO_CORE} ${FM_BIN} \
    --config-file=${CFG_DIR}/redistanciation3d.cfg \
    --levelset.mesh.filename=${CFG_DIR}/meshes/${HSIZE}/cube_p${NCORE}.json \
    --levelset.gmsh.hsize=${HSIZE} \
    --directory=toolboxes/levelset/benchmark/redistanciation/3d/h_${HSIZE} \
