class voltage_nonlinearTest3d0d
  ...
  //final Modelica.SIunits.Voltage Pi2_bis;
...
equation
  ...
  // dQ_I = -Cb*alpha*gamma*gamma*Modelica.Math.sin(gamma*time);
  ...
  Pi2 = P_I - (Rb+R1)*Q_I + R1*Cb*der(Pi1);
  
  Pi_out = Pi2 + Rout*C1*der(Pi2) - Rout/R1 * (Pi1-Pi2);
  ...
  // Pi_out = beta + alpha*Modelica.Math.sin(gamma*time)* ( 1-R1*(1-Rb)*Cb^2*gamma^2 - Rout*(1-Rb)*Cb*gamma^2*(C1+Cb) ) + alpha*gamma*Modelica.Math.cos(gamma*time)*( (1-Rb)*Cb + Rout*C1*(1-R1*(1 -Rb)*Cb^2*gamma^2) );

  //v = Pi2 + Rout*pi*Modelica.Math.cos(gamma*time)* (3*Modelica.Math.exp(-alpha2*Pi1))/(1+2*Modelica.Math.exp(-alpha2*Pi1)) - Rout*gamma*gamma*Modelica.Math.sin(gamma*time);
