...
runApplicationPoroelastic()
{
    ...
    else if ( soption( "gmsh.submesh" ).empty() )
    {
        ...
        auto cmeshElasticity = createSubmesh( mesh, markedelements(mesh,listSubmesh ), Environment::worldCommPtr() );
    ...
    else if ( soption("hdg.elasticity.gmsh.submesh").empty() )
    {
        ...
        auto cmeshPoisson = createSubmesh( mesh, markedelements(mesh,soption("gmsh.submesh")), Environment::worldCommPtr() );
