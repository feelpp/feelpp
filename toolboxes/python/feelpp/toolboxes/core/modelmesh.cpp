...
void defToolbox(py::module &m)
{
    ...
    py::class_<toolbox_t, std::shared_ptr<toolbox_t>, ModelBase>( m, pyclass_name.c_str() )
        .def( py::init([](mesh_ptr_t mesh, std::string const& prefix, py::object worldComm, ModelBaseRepository const& modelRep) {
                  worldcomm_ptr_t wc = worldComm.is_none() ? Environment::worldCommPtr() : py::cast<worldcomm_ptr_t>(worldComm);
                  return new toolbox_t(mesh, prefix, wc, modelRep);
              }),
              py::arg( "mesh" ),
              py::arg( "prefix" ) = "",
        ...
        .def(
            "updateDisplacementImposed", []( toolbox_t& t, ale_map_element_t const& d, Range<mesh_ptr_t,MESH_ELEMENTS> const& r )
            {
        ...
        .def(
            "updateDisplacementImposed", []( toolbox_t& t, ale_map_element_t const& d, Range<mesh_ptr_t,MESH_FACES> const& r )
            { t.updateDisplacementImposed( idv(d), r ); },
            py::arg( "disp" ), py::arg( "range" ),
        ...
        .def( "updateDisplacementImposedOnInitialDomain", []( toolbox_t& t, std::string keyword, ale_map_element_t const& d, Range<mesh_ptr_t,MESH_FACES> const& r  ){
