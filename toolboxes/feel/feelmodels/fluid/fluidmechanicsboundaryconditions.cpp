...
template <uint16_type Dim>
void
FluidMechanicsBoundaryConditions<Dim>::NormalStress::updateInformationObject( nl::json & p ) const
{
    auto [exprStr,compInfo] = M_mexpr.exprInformations();
    p["expr"] = exprStr;
    p["markers"] = M_markers;
...
template <uint16_type Dim>
tabulate_informations_ptr_t
FluidMechanicsBoundaryConditions<Dim>::NormalStress::tabulateInformations( nl::json const& jsonInfo, TabulateInformationProperties const& tabInfoProp )
{
    Feel::Table tabInfo;
