...
{
public:
    typedef LevelsetHeavisideExpr<LSExprType> this_type;
    ...
    //--------------------------------------------------------------------//
    // Constructors
    LevelsetHeavisideExpr( expr_levelsetphi_type const& ex, double eps )
    ...
    double thickness() const { return M_thicknessHeaviside; }
    ...
    private:
        void computeHeaviside()
        {
            for( uint16_type q = 0; q < M_locRes.size(); ++q )
            {
                ...
                if( phi < -M_epsilon )
                    M_locRes[q](0,0) = 0.;
                else if( phi > M_epsilon )
                    M_locRes[q](0,0) = 1.;
                else
                    M_locRes[q](0,0) = 0.5 * ( 1 + phi/M_epsilon + 1/M_PI*math::sin(M_PI*phi/M_epsilon) );
            }
        }
...
inline
Expr< LevelsetHeavisideExpr<LSExprT> >
levelsetHeaviside( LSExprT phi, double eps )
{
    typedef LevelsetHeavisideExpr<LSExprT> lsheaviside_t;
