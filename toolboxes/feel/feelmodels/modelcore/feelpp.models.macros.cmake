...
macro( genLibAdvection )
  PARSE_ARGUMENTS(FEELMODELS_APP
      "DIM;POLY_ORDER;CONTINUITY;POLY_SET;GEO_ORDER;POINT_SET;USE_PERIODICITY;DIFFUSION_REACTION_ORDER;DIFFUSIONCOEFF_POLY_SET;REACTIONCOEFF_POLY_SET;DIFFUSION_REACTION_CONTINUITY"
      "NO_UPDATE_MODEL_DEF;ADD_CMAKE_INSTALL"
      ${ARGN}
    )
  ...
  # point set
  if (FEELMODELS_APP_POINT_SET)
      if("${FEELMODELS_APP_POINT_SET}" STREQUAL "PointSetEquiSpaced" )
          set(ADVECTION_POINT_SET ${FEELMODELS_APP_POINT_SET} )
          unset(ADVECTION_POINT_SET_TAG) #default value PointSetEquiSpaced
      else()
          set(ADVECTION_POINT_SET ${FEELMODELS_APP_POINT_SET} )
          #set(ADVECTION_POINT_SET_TAG ${FEELMODELS_APP_POINT_SET} )
          string(REPLACE "PointSet" "" ADVECTION_POINT_SET_TAG ${FEELMODELS_APP_POINT_SET})
      endif()
  else()
      # default value
  ...
  # periodicity
  if (FEELMODELS_APP_USE_PERIODICITY)
      if ("${FEELMODELS_APP_USE_PERIODICITY}" STREQUAL "1" )
          set(ADVECTION_USE_PERIODICITY 1)
          set(ADVECTION_USE_PERIODICITY_TAG Periodic)
      elseif ("${FEELMODELS_APP_USE_PERIODICITY}" STREQUAL "0" )
          set(ADVECTION_USE_PERIODICITY 0)
          unset(ADVECTION_USE_PERIODICITY_TAG)
      else()
          message(FATAL_ERROR "USE_PERIODICITY ${FEELMODELS_APP_USE_PERIODICITY} : is not valid! It must be 0 or 1")
      endif()
  else()
      # default value (non-periodic)
      set(ADVECTION_USE_PERIODICITY 0)
      unset(ADVECTION_USE_PERIODICITY_TAG)
  endif()
  #######################################################
  ...
  if(FEELMODELS_APP_REACTIONCOEFF_POLY_SET)
      ...
      elseif("${FEELMODELS_APP_REACTIONCOEFF_POLY_SET}" STREQUAL "Vectorial" )
          set(ADVECTION_REACTIONCOEFF_POLY_SET_TAG v)
  ...
  if (FEELMODELS_APP_DIFFUSION_REACTION_CONTINUITY)
      if ("${FEELMODELS_APP_DIFFUSION_REACTION_CONTINUITY}" STREQUAL "Continuous" )
          ...
          set(ADVECTION_DIFFUSION_REACTION_TAG DRP${ADVECTION_ORDER_DIFFUSION_REACTION})
    ...
    # default value
    set(ADVECTION_USE_CONTINUOUS_DIFFUSION_REACTION 1)
    set(ADVECTION_DIFFUSION_REACTION_TAG DRP${ADVECTION_ORDER_DIFFUSION_REACTION}) # default value P${ORDER_POLY}c
  endif()
  #######################################################
  ...
  set(FEELMODELS_MODEL_SPECIFIC_NAME_SUFFIX ${ADVECTION_DIM}dP${ADVECTION_ORDERPOLY}${ADVECTION_CONTINUITY_TAG}${ADVECTION_POLY_SET_TAG}G${ADVECTION_ORDERGEO}${ADVECTION_POINT_SET_TAG}${ADVECTION_DIFFUSION_REACTION_TAG}${ADVECTION_PERIODICITY_TAG} )
...
macro( genLibLevelset )
    PARSE_ARGUMENTS(FEELMODELS_APP
        "DIM;LEVELSET_ORDER;GEO_ORDER;POINT_SET;USE_PERIODICITY;ADVECTION_DIFFUSION_REACTION_ORDER;ADVECTION_DIFFUSION_REACTION_CONTINUITY;ADD_CMAKE_INSTALL_BIS"
        "NO_UPDATE_MODEL_DEF;ADD_CMAKE_INSTALL"
        ${ARGN}
        )
    ...
    if (FEELMODELS_APP_POINT_SET)
        if("${FEELMODELS_APP_POINT_SET}" STREQUAL "PointSetEquiSpaced" )
            set(LEVELSET_POINT_SET ${FEELMODELS_APP_POINT_SET} )
            unset(LEVELSET_POINT_SET_TAG) #default value PointSetEquiSpaced
        else()
            set(LEVELSET_POINT_SET ${FEELMODELS_APP_POINT_SET} )
            #set(LEVELSET_POINT_SET_TAG ${FEELMODELS_APP_POINT_SET} )
            string(REPLACE "PointSet" "" LEVELSET_POINT_SET_TAG ${FEELMODELS_APP_POINT_SET})
        endif()
    else()
        # default value
        set(LEVELSET_POINT_SET PointSetEquiSpaced)
        unset(LEVELSET_POINT_SET_TAG) #default value PointSetEquiSpaced
    endif()
    #######################################################
    if (FEELMODELS_APP_USE_PERIODICITY)
    ...
    genLibAdvection(
        ...
        POINT_SET       ${FEELMODELS_APP_POINT_SET} 
        ...
        USE_PERIODICITY ${LEVELSET_USE_PERIODICITY}
        #ADD_CMAKE_INSTALL ${LIBBASE_ADD_CMAKE_INSTALL}
        )
    ...
    set(FEELMODELS_MODEL_SPECIFIC_NAME_SUFFIX ${LEVELSET_DIM}dP${LEVELSET_ORDERPOLY}G${LEVELSET_ORDERGEO}${LEVELSET_POINT_SET_TAG}${LEVELSET_USE_PERIODICITY_TAG} )
...
macro(genLibMultiFluid)
  PARSE_ARGUMENTS(FEELMODELS_APP
      "DIM;FLUID_U_ORDER;FLUID_P_ORDER;FLUID_P_CONTINUITY;GEO_ORDER;FLUID_DENSITY_VISCOSITY_CONTINUITY;FLUID_DENSITY_VISCOSITY_ORDER;LEVELSET_ORDER;LEVELSET_DIFFUSION_REACTION_ORDER;LEVELSET_DIFFUSION_REACTION_CONTINUITY;POINT_SET;USE_PERIODICITY"
    "ADD_CMAKE_INSTALL"
    ${ARGN}
    )
