    ...
    auto symbolsExpr( std::string const& prefix_symbol = "" ) const
        {
            ...
            {
                std::cout << "compute se " << name << std::endl;
...
template <typename IndexType>
class ModelMeshes : protected std::map<std::string,ModelMesh<IndexType>>
{
    using index_type = IndexType;
...
ModelMeshes<IndexType>::setup( pt::ptree const& pt )
{
    for ( auto const& item : pt )
    {
        ...
        std::cout << "ModelMeshes= " << meshName << std::endl;
        ModelMesh<IndexType> me;
